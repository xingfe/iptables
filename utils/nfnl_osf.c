...
static struct xt_osf_opt IANA_opts[] = {
    ...
    {
        .kind = 15, .length = 1,
    }, /* TCP Alternate Checksum Data. Length is not defined */
    {
        .kind = 16, .length = 1,
    },
    {
...
static void ulog(const char *f, ...) {
    ...
    fprintf(osf_log_stream, "%s.%lu %ld ", str, tv.tv_usec,
            syscall(__NR_gettid));
...
static void xt_osf_parse_opt(struct xt_osf_opt *opt, __u16 *optnum, char *obuf,
                             int olen) {
    int i, op;
    char *ptr, wc;
    unsigned long val;
...
static int osf_load_line(char *buffer, int len, int del) {
    ...
    nfnl_addattr_l(nmh, sizeof(buf), OSF_ATTR_FINGER, &f,
...
int main(int argc, char *argv[]) {
    ...
    while ((ch = getopt(argc, argv, "f:dh")) != -1) {
        ...
        case 'd':
            ...
            break;
        default:
            fprintf(stderr, "Usage: %s -f fingerprints -d <del rules> -h\n",
                    argv[0]);
            return -1;
        }
    }
