...
static void xtopt_parse_markmask(struct xt_option_call *cb) {
    ...
    if (*end == '/' && !xtables_strtoui(end + 1, &end, &mask, 0, UINT32_MAX))
        xt_params->exit_err(PARAMETER_PROBLEM,
...
static void xtopt_parse_sysloglevel(struct xt_option_call *cb) {
    static const struct syslog_level log_names[] = {
        /* must be sorted */
    ...
    if (!xtables_strtoui(cb->arg, NULL, &num, 0, 7)) {
        ...
        if (e == NULL)
            xt_params->exit_err(PARAMETER_PROBLEM, "log level \"%s\" unknown\n",
                                cb->arg);
        num = e->level;
    }
...
static void xtopt_parse_host(struct xt_option_call *cb) {
    ...
    if (ret != 0)
        xt_params->exit_err(PARAMETER_PROBLEM, "getaddrinfo: %s\n",
                            gai_strerror(ret));
    ...
    for (p = res; p != NULL; p = p->ai_next) {
        if (adcount == 0) {
            ...
            memcpy(&cb->val.haddr, xtables_sa_host(p->ai_addr, p->ai_family),
                   xtables_sa_hostlen(p->ai_family));
            ++adcount;
            continue;
        }
        if (memcmp(&cb->val.haddr, xtables_sa_host(p->ai_addr, p->ai_family),
    ...
    if (cb->entry->flags & XTOPT_PUT)
        ...
        memcpy(XTOPT_MKPTR(cb), &cb->val.haddr, sizeof(cb->val.haddr));
}
...
static void xtopt_parse_mport(struct xt_option_call *cb) {
    ...
    if (entry->flags & XTOPT_PUT)
        memcpy(XTOPT_MKPTR(cb), cb->val.port_range,
               sizeof(uint16_t) * (cb->nvals <= maxiter ? cb->nvals : maxiter));
    free(lo_arg);
}
...
static void xtopt_parse_ethermac(struct xt_option_call *cb) {
    ...
    if (*end != '\0' || end - arg > 2)
        goto out;
    if (cb->entry->flags & XTOPT_PUT)
        memcpy(XTOPT_MKPTR(cb), cb->val.ethermac, sizeof(cb->val.ethermac));
    return;
out:
    ...
    for (; entry->name != NULL; ++entry) {
        ...
        if (xtopt_psize[entry->type] != -1 &&
            ...
            xt_params->exit_err(OTHER_PROBLEM,
                                ...
                                name, entry->name, xtopt_psize[entry->type],
                                entry->size);
    }
}
...
void xtables_option_tpcall(unsigned int c, char **argv, bool invert,
    ...
    if (cb.entry == NULL)
        xtables_error(OTHER_PROBLEM, "Extension does not know id %u\n", c);
    cb.arg = optarg;
    cb.invert = invert;
    cb.ext_name = t->name;
    cb.data = t->t->data;
 ...
 */
static void xtables_option_fcheck2(const char *name,
                                   const struct xt_option_entry *entry,
...
void xtables_options_fcheck(const char *name, unsigned int xflags,
    ...
    for (entry = table; entry->name != NULL; ++entry) {
        if (entry->flags & XTOPT_MAND && !(xflags & (1 << entry->id)))
            xt_params->exit_err(PARAMETER_PROBLEM,
