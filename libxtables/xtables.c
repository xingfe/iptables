...
void basic_exit_err(enum xtables_exittype status, const char *msg, ...) {
    ...
    va_start(args, msg);
    fprintf(stderr, "%s v%s: ", xt_params->program_name,
...
static const struct xtables_afinfo afinfo_bridge = {
    .libprefix = "libebt_", .family = NFPROTO_BRIDGE,
};

static const struct xtables_afinfo afinfo_arp = {
    .libprefix = "libarpt_", .family = NFPROTO_ARP,
};
 ...
 * undefined contents.
 */
bool xtables_strtoul(const char *s, char **end, uintmax_t *value, uintmax_t min,
                     uintmax_t max) {
    uintmax_t v;
    const char *p;
    char *my_end;
    ...
    do {
        ...
        for (prefix = all_prefixes; *prefix != NULL; ++prefix) {
            ...
            if (is_target)
                ...
                ptr = xtables_find_match(name, XTF_DONT_LOAD, NULL);
...
static bool extension_cmp(const char *name1, const char *name2,
    ...
    if (strcmp(name1, name2) == 0 &&
        (family == afinfo->family || family == NFPROTO_UNSPEC))
        return true;
...
struct xtables_match *xtables_find_match(const char *name,
    ...
    if (strlen(name) >= XT_EXTENSION_MAXNAMELEN)
        xtables_error(PARAMETER_PROBLEM,
                      "Invalid match name \"%s\" (%u chars max)", name,
                      XT_EXTENSION_MAXNAMELEN - 1);

    /* This is ugly as hell. Nonetheless, there is no way of changing
     * this without hurting backwards compatibility */
    if ((strcmp(name, "icmpv6") == 0) || (strcmp(name, "ipv6-icmp") == 0) ||
        (strcmp(name, "icmp6") == 0))
        name = icmp6;
    ...
    if (!ptr && tryload != XTF_DONT_LOAD && tryload != XTF_DURING_LOAD) {
        ptr = load_extension(xtables_libdir, afinfo->libprefix, name, false);
    ...
    if (!ptr && (tryload == XTF_LOAD_MUST_SUCCEED)) {
        xt_params->exit_err(PARAMETER_PROBLEM, "Couldn't find match `%s'\n",
                            name);
    }
#endif
...
struct xtables_target *xtables_find_target(const char *name,
    ...
    /* Standard target? */
    if (strcmp(name, "") == 0 || strcmp(name, XTC_LABEL_ACCEPT) == 0 ||
    ...
    if (ptr == NULL && tryload == XTF_LOAD_MUST_SUCCEED) {
        xt_params->exit_err(PARAMETER_PROBLEM, "Couldn't find target `%s'\n",
                            name);
    }
#endif
...
static void xtables_check_options(const char *name, const struct option *opt) {
    for (; opt->name != NULL; ++opt)
        if (opt->val < 0 || opt->val >= XT_OPTION_OFFSET_SCALE) {
            fprintf(stderr,
                    "%s: Extension %s uses invalid "
                    "option value %d\n",
                    xt_params->program_name, name, opt->val);
            exit(1);
        }
}
...
void xtables_register_match(struct xtables_match *me) {
    ...
    if (strcmp(me->version, XTABLES_VERSION) != 0) {
        fprintf(stderr,
                ...
                "but \"%s\" is required.\n",
                xt_params->program_name, me->name, me->version,
                XTABLES_VERSION);
        exit(1);
    }
    ...
    if (me->family >= NPROTO) {
        fprintf(stderr, "%s: BUG: match %s has invalid protocol family\n",
                xt_params->program_name, me->name);
        exit(1);
    }
 ...
 * preferred.
 */
static int xtables_mt_prefer(bool a_alias, unsigned int a_rev,
                             unsigned int a_fam, bool b_alias,
...
static int xtables_match_prefer(const struct xtables_match *a,
    ...
    return xtables_mt_prefer(a->real_name != NULL, a->revision, a->family,
                             b->real_name != NULL, b->revision, b->family);
}
...
static void xtables_fully_register_pending_match(struct xtables_match *me) {
    ...
    if (old) {
        ...
        if (compare > 0 && compatible_match_revision(rn, old->revision))
            return;
    ...
    if (me->size != XT_ALIGN(me->size)) {
        fprintf(stderr, "%s: match `%s' has invalid size %u.\n",
                xt_params->program_name, me->name, (unsigned int)me->size);
        exit(1);
    }
...
void xtables_register_target(struct xtables_target *me) {
    ...
    if (me->family >= NPROTO) {
        fprintf(stderr, "%s: BUG: target %s has invalid protocol family\n",
                xt_params->program_name, me->name);
        exit(1);
    }
...
static void xtables_fully_register_pending_target(struct xtables_target *me) {
    ...
    if (old) {
        ...
        /* Now we have two (or more) options, check compatibility. */
        rn = (old->real_name != NULL) ? old->real_name : old->name;
        if (compare > 0 && compatible_target_revision(rn, old->revision))
            return;
...
void xtables_param_act(unsigned int status, const char *p1, ...) {
    ...
    case XTF_ONLY_ONCE:
        ...
        xt_params->exit_err(PARAMETER_PROBLEM,
                            "%s: \"%s\" option may only be specified once", p1,
                            p2);
        break;
    case XTF_NO_INVERT:
        p2 = va_arg(args, const char *);
    ...
    case XTF_BAD_VALUE:
        ...
        xt_params->exit_err(PARAMETER_PROBLEM,
                            "%s: Bad value for \"%s\" option: \"%s\"", p1, p2,
                            p3);
        break;
    case XTF_ONE_ACTION:
        b = va_arg(args, unsigned int);
...
static struct in_addr *ipparse_hostnetwork(const char *name,
                                           unsigned int *naddrs) {
...
static struct in_addr *parse_ipmask(const char *mask) {
    ...
    if (!xtables_strtoui(mask, NULL, &bits, 0, 32))
        xt_params->exit_err(PARAMETER_PROBLEM, "invalid mask `%s' specified",
                            mask);
    if (bits != 0) {
    ...
    *maskpp = xtables_malloc(sizeof(struct in_addr) * count);
    ...
    for (i = 0; i < count; ++i) {
        ...
        if (next != NULL)
            ...
            len = strlen(loop);
        if (len > sizeof(buf) - 1)
            xt_params->exit_err(PARAMETER_PROBLEM, "Hostname too long");
        ...
        if (n > 1) {
            count += n - 1;
            *addrpp = xtables_realloc(*addrpp, sizeof(struct in_addr) * count);
            *maskpp = xtables_realloc(*maskpp, sizeof(struct in_addr) * count);
            for (j = 0; j < n; ++j)
                /* for each new addr */
...
static struct in6_addr *host_to_ip6addr(const char *name, unsigned int *naddr) {
...
static struct in6_addr *ip6parse_hostnetwork(const char *name,
                                             unsigned int *naddrs) {
...
void xtables_ip6parse_multiple(const char *name, struct in6_addr **addrpp,
    ...
    *maskpp = xtables_malloc(sizeof(struct in6_addr) * count);
    ...
    for (i = 0; i < count /*NB: count can grow*/; ++i) {
        ...
        if (n > 1) {
            count += n - 1;
            *addrpp = xtables_realloc(*addrpp, sizeof(struct in6_addr) * count);
            *maskpp = xtables_realloc(*maskpp, sizeof(struct in6_addr) * count);
            for (j = 0; j < n; ++j)
                /* for each new addr */
                memcpy(*addrpp + i + j, addrp + j, sizeof(*addrp));
            for (j = 1; j < n; ++j)
...
uint16_t xtables_parse_protocol(const char *s) {
    ...
    xt_params->exit_err(PARAMETER_PROBLEM, "unknown protocol \"%s\" specified",
                        s);
    return -1;
}
...
const char *xt_xlate_get(struct xt_xlate *xl) { return xl->buf.data; }
