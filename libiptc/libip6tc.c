...
static int ipv6_prefix_length(const struct in6_addr *a) {
    int l, i;
    for (l = 0; l < 128; l++) {
        if (BIT6(a, l) == 0)
            break;
...
static int dump_entry(struct ip6t_entry *e, struct xtc_handle *const handle) {
    size_t i;
    char buf[40];
    int len;
...
static unsigned char *is_same(const STRUCT_ENTRY *a, const STRUCT_ENTRY *b,
    ...
    if (memcmp(&a->ipv6.src, &b->ipv6.src, sizeof(struct in6_addr)) ||
        ...
        a->ipv6.proto != b->ipv6.proto || a->ipv6.tos != b->ipv6.tos ||
        a->ipv6.flags != b->ipv6.flags || a->ipv6.invflags != b->ipv6.invflags)
        return NULL;
...
static inline int unconditional(const struct ip6t_ip6 *ipv6) {
    unsigned int i;
...
static void do_check(struct xtc_handle *h, unsigned int line) {
    ...
    if (strcmp(h->info.name, "filter") == 0) {
        assert(h->info.valid_hooks ==
               (1 << NF_IP6_LOCAL_IN | 1 << NF_IP6_FORWARD |
                1 << NF_IP6_LOCAL_OUT));
    ...
    for (i = 0; i < NUMHOOKS; i++) {
        ...
        if (!(h->info.valid_hooks & (1 << i)))
            continue;
        assert(h->info.underflow[i] == get_chain_end(h, h->info.hook_entry[i]));
