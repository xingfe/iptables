 ...
 * 	- performance optimization, sponsored by Astaro AG
 *(http://www.astaro.com/) don't rebuild the chain cache after every operation,
 *instead fix it up after a ruleset change. 2004-Aug-18: Harald Welte
 *<laforge@netfilter.org>: - further performance work: total reimplementation
 *of libiptc. - libiptc now has a real internal (linked-list) represntation of
 *the ruleset and a parser/compiler from/to this internal representation -
...
static const char *hooknames[] = {
        [HOOK_PRE_ROUTING] = "PREROUTING",   [HOOK_LOCAL_IN] = "INPUT",
        [HOOK_FORWARD] = "FORWARD",          [HOOK_LOCAL_OUT] = "OUTPUT",
...
static inline void set_changed(struct xtc_handle *h) { h->changed = 1; }

#ifdef IPTC_DEBUG
static void do_check(struct xtc_handle *h, unsigned int line);
#define CHECK(h)                                                               \
    do {                                                                       \
        if (!getenv("IPTC_NO_CHECK"))                                          \
            do_check((h), __LINE__);                                           \
    } while (0)
#else
#define CHECK(h)
...
static inline int iptcb_get_number(const STRUCT_ENTRY *i,
...
static inline int iptcb_get_entry_n(STRUCT_ENTRY *i, unsigned int number,
                                    unsigned int *pos, STRUCT_ENTRY **pe) {
    if (*pos == number) {
        *pe = i;
        return 1;
    }
...
static inline STRUCT_ENTRY *iptcb_get_entry(struct xtc_handle *h,
                                            unsigned int offset) {
    return (STRUCT_ENTRY *)((char *)h->entries->entrytable + offset);
}

static unsigned int iptcb_entry2index(struct xtc_handle *const h,
                                      const STRUCT_ENTRY *seek) {
    unsigned int pos = 0;
...
static inline STRUCT_ENTRY *iptcb_offset2entry(struct xtc_handle *h,
                                               unsigned int offset) {
    return (STRUCT_ENTRY *)((void *)h->entries->entrytable + offset);
}

static inline unsigned long iptcb_entry2offset(struct xtc_handle *const h,
                                               const STRUCT_ENTRY *e) {
    return (void *)e - (void *)h->entries->entrytable;
}

static inline unsigned int iptcb_offset2index(struct xtc_handle *const h,
                                              unsigned int offset) {
    return iptcb_entry2index(h, iptcb_offset2entry(h, offset));
}

/* Returns 0 if not hook entry, else hooknumber + 1 */
static inline unsigned int iptcb_ent_is_hook_entry(STRUCT_ENTRY *e,
                                                   struct xtc_handle *h) {
    unsigned int i;
 ...
 *
 */
static struct list_head *__iptcc_bsearch_chain_index(const char *name,
                                                     unsigned int offset,
                                                     unsigned int *idx,
...
loop:
    ...
    debug("bsearch Index[%d] name:%s ", pos, handle->chain_index[pos]->name);
...
static struct list_head *iptcc_bsearch_chain_index(const char *name,
                                                   unsigned int *idx,
                                                   struct xtc_handle *handle) {
...
static struct list_head *iptcc_bsearch_chain_offset(unsigned int offset,
                                                    unsigned int *idx,
                                                    struct xtc_handle *handle) {
...
static int iptcc_chain_index_alloc(struct xtc_handle *h) {
    ...
    array_elems =
        (h->num_chains / list_length) + (h->num_chains % list_length ? 1 : 0);
 ...
 */
static int iptcc_chain_index_delete_chain(struct chain_head *c,
                                          struct xtc_handle *h) {
...
static struct chain_head *iptcc_find_chain_by_offset(struct xtc_handle *handle,
                                                     unsigned int offset) {
    struct list_head *pos;
...
static int __iptcc_p_del_policy(struct xtc_handle *h, unsigned int num) {
    ...
    if (h->chain_iterator_cur) {
        ...
        struct rule_head *pr =
            (struct rule_head *)h->chain_iterator_cur->rules.prev;
        ...
        h->chain_iterator_cur->counter_map.maptype = COUNTER_MAP_ZEROED;
...
static int cache_add_entry(STRUCT_ENTRY *e, struct xtc_handle *h,
                           STRUCT_ENTRY **prev, unsigned int *num) {
    ...
    if (strcmp(GET_TARGET(e)->u.user.name, ERROR_TARGET) == 0) {
        ...
        struct chain_head *c =
            iptcc_alloc_chain_head((char *)hooknames[builtin - 1], builtin);
...
static int parse_table(struct xtc_handle *h) {
    ...
    ENTRY_ITERATE(h->entries->entrytable, h->entries->size, cache_add_entry, h,
                  &prev, &num);
...
#define IPTCB_CHAIN_START_SIZE                                                 \
    (sizeof(STRUCT_ENTRY) + ALIGN(sizeof(struct xt_error_target)))
...
#define IPTCB_CHAIN_FOOT_SIZE                                                  \
    (sizeof(STRUCT_ENTRY) + ALIGN(sizeof(STRUCT_STANDARD_TARGET)))
...
static inline int iptcc_compile_rule(struct xtc_handle *h, STRUCT_REPLACE *repl,
                                     struct rule_head *r) {
    /* handle jumps */
    if (r->type == IPTCC_R_JUMP) {
        STRUCT_STANDARD_TARGET *t;
...
static int iptcc_compile_chain(struct xtc_handle *h, STRUCT_REPLACE *repl,
                               struct chain_head *c) {
    int ret;
    struct rule_head *r;
...
static int iptcc_compile_chain_offsets(struct xtc_handle *h,
                                       struct chain_head *c,
                                       unsigned int *offset,
                                       unsigned int *num) {
    struct rule_head *r;
...
static struct xtc_handle *alloc_handle(const char *tablename, unsigned int size,
...
retry:
    ...
    if ((h = alloc_handle(info.name, info.size, info.num_entries)) == NULL) {
        close(sockfd);
        return NULL;
    }
    ...
    {
        int fd =
            open("/tmp/libiptc-so_get_entries.blob", O_CREAT | O_WRONLY, 0644);
        if (fd >= 0) {
...
void TC_FREE(struct xtc_handle *h) {
    ...
    list_for_each_entry_safe(c, tmp, &h->chains, list) {
        ...
        list_for_each_entry_safe(r, rtmp, &c->rules, list) { free(r); }
...
static inline int print_match(const STRUCT_ENTRY_MATCH *m) {
    printf("Match name: `%s'\n", m->u.user.name);
    return 0;
}
...
const char *TC_FIRST_CHAIN(struct xtc_handle *handle) {
    struct chain_head *c =
        list_entry(handle->chains.next, struct chain_head, list);
...
const STRUCT_ENTRY *TC_NEXT_RULE(const STRUCT_ENTRY *prev,
    ...
    r = list_entry(handle->rule_iterator_cur->list.next, struct rule_head,
                   list);
...
const char *TC_GET_POLICY(const char *chain, STRUCT_COUNTERS *counters,
                          struct xtc_handle *handle) {
    struct chain_head *c;
...
static int iptcc_map_target(struct xtc_handle *const handle,
                            struct rule_head *r, bool dry_run) {
    STRUCT_ENTRY *e = r->entry;
...
int TC_INSERT_ENTRY(const IPT_CHAINLABEL chain, const STRUCT_ENTRY *e,
                    unsigned int rulenum, struct xtc_handle *handle) {
    struct chain_head *c;
    struct rule_head *r;
...
int TC_REPLACE_ENTRY(const IPT_CHAINLABEL chain, const STRUCT_ENTRY *e,
                     unsigned int rulenum, struct xtc_handle *handle) {
    struct chain_head *c;
...
static inline int match_different(const STRUCT_ENTRY_MATCH *a,
...
static inline int target_same(struct rule_head *a, struct rule_head *b,
...
static unsigned char *is_same(const STRUCT_ENTRY *a, const STRUCT_ENTRY *b,
                              unsigned char *matchmask);

/* find the first rule in `chain' which matches `fw' and remove it unless
 * dry_run is set */
static int delete_entry(const IPT_CHAINLABEL chain, const STRUCT_ENTRY *origfw,
    ...
    list_for_each_entry(i, &c->rules, list) {
        ...
        if (i == handle->rule_iterator_cur) {
            handle->rule_iterator_cur = list_entry(
                handle->rule_iterator_cur->list.prev, struct rule_head, list);
        }
...
int TC_DELETE_NUM_ENTRY(const IPT_CHAINLABEL chain, unsigned int rulenum,
                        struct xtc_handle *handle) {
    struct chain_head *c;
    struct rule_head *r;
...
int TC_FLUSH_ENTRIES(const IPT_CHAINLABEL chain, struct xtc_handle *handle) {
    ...
    list_for_each_entry_safe(r, tmp, &c->rules, list) { iptcc_delete_rule(r); }
...
int TC_ZERO_COUNTER(const IPT_CHAINLABEL chain, unsigned int rulenum,
                    struct xtc_handle *handle) {
    struct chain_head *c;
    struct rule_head *r;
...
int TC_SET_COUNTER(const IPT_CHAINLABEL chain, unsigned int rulenum,
                   STRUCT_COUNTERS *counters, struct xtc_handle *handle) {
    struct chain_head *c;
    struct rule_head *r;
    STRUCT_ENTRY *e;
...
int TC_CREATE_CHAIN(const IPT_CHAINLABEL chain, struct xtc_handle *handle) {
    ...
    /* find_label doesn't cover built-in targets: DROP, ACCEPT,
       QUEUE, RETURN. */
    if (iptcc_find_label(chain, handle) || strcmp(chain, LABEL_DROP) == 0 ||
        strcmp(chain, LABEL_ACCEPT) == 0 || strcmp(chain, LABEL_QUEUE) == 0 ||
...
int TC_GET_REFERENCES(unsigned int *ref, const IPT_CHAINLABEL chain,
                      struct xtc_handle *handle) {
    struct chain_head *c;
...
/* Renames a chain. */
int TC_RENAME_CHAIN(const IPT_CHAINLABEL oldname, const IPT_CHAINLABEL newname,
                    struct xtc_handle *handle) {
    struct chain_head *c;
    ...
    /* find_label doesn't cover built-in targets: DROP, ACCEPT,
       QUEUE, RETURN. */
    if (iptcc_find_label(newname, handle) || strcmp(newname, LABEL_DROP) == 0 ||
...
int TC_SET_POLICY(const IPT_CHAINLABEL chain, const IPT_CHAINLABEL policy,
                  STRUCT_COUNTERS *counters, struct xtc_handle *handle) {
    struct chain_head *c;
...
*/
static void subtract_counters(STRUCT_COUNTERS *answer, const STRUCT_COUNTERS *a,
...
static void counters_map_zeroed(STRUCT_COUNTERS_INFO *newcounters,
     ...
     */
    subtract_counters(&newcounters->counters[idx], &repl->counters[mappos],
                      counters);
...
static void counters_map_set(STRUCT_COUNTERS_INFO *newcounters,
    ...
    memcpy(&newcounters->counters[idx], counters, sizeof(STRUCT_COUNTERS));
...
int TC_COMMIT(struct xtc_handle *handle) {
    ...
    counterlen =
        sizeof(STRUCT_COUNTERS_INFO) + sizeof(STRUCT_COUNTERS) * new_number;

    /* These are the old counters we will get from kernel */
    repl->counters = malloc(sizeof(STRUCT_COUNTERS) * handle->info.num_entries);
    if (!repl->counters) {
        errno = ENOMEM;
        goto out_free_repl;
    }
    ...
    {
        int fd =
            open("/tmp/libiptc-so_set_replace.blob", O_CREAT | O_WRONLY, 0644);
        if (fd >= 0) {
    ...
    list_for_each_entry(c, &handle->chains, list) {
        ...
        if (iptcc_is_builtin(c)) {
            ...
            case COUNTER_MAP_NORMAL_MAP:
                counters_normal_map(newcounters, repl, c->foot_index,
                                    c->counter_map.mappos);
                break;
            case COUNTER_MAP_ZEROED:
                counters_map_zeroed(newcounters, repl, c->foot_index,
                                    c->counter_map.mappos, &c->counters);
                break;
        ...
        list_for_each_entry(r, &c->rules, list) {
            ...
            case COUNTER_MAP_NORMAL_MAP:
                counters_normal_map(newcounters, repl, r->index,
                                    r->counter_map.mappos);
                break;

            case COUNTER_MAP_ZEROED:
                counters_map_zeroed(newcounters, repl, r->index,
                                    r->counter_map.mappos, &r->entry->counters);
                break;
...
const char *TC_STRERROR(int err) {
    ...
    struct table_struct {
        ...
        {TC_DELETE_CHAIN, EMLINK, "Can't delete chain with references left"},
        ...
        {TC_SET_POLICY, ENOENT, "Bad built-in chain name"},
        {TC_SET_POLICY, EINVAL, "Bad policy name"},
    ...
    for (i = 0; i < sizeof(table) / sizeof(struct table_struct); i++) {
        if ((!table[i].fn || table[i].fn == iptc_fn) && table[i].err == err)
