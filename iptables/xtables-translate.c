...
int xlate_matches(const struct iptables_command_state *cs,
                  struct xt_xlate *xl) {
...
bool xlate_find_match(const struct iptables_command_state *cs,
                      const char *p_name) {
    struct xtables_rule_match *matchp;
...
static int nft_rule_xlate_add(struct nft_handle *h,
    ...
    if (append) {
        xt_xlate_add(xl, "add rule %s %s %s ", family2str[h->family], p->table,
                     p->chain);
    } else {
        xt_xlate_add(xl, "insert rule %s %s %s ", family2str[h->family],
                     p->table, p->chain);
    }
...
static int
xlate(struct nft_handle *h, struct nft_xt_cmd_parse *p,
      struct iptables_command_state *cs, struct xtables_args *args, bool append,
      int (*cb)(struct nft_handle *h, const struct nft_xt_cmd_parse *p,
    ...
    for (i = 0; i < args->s.naddrs; i++) {
        ...
        case AF_INET:
            ...
            break;
        case AF_INET6:
            memcpy(&cs->fw6.ipv6.src, &args->s.addr.v6[i],
                   sizeof(struct in6_addr));
            memcpy(&cs->fw6.ipv6.smsk, &args->s.mask.v6[i],
                   sizeof(struct in6_addr));
            for (j = 0; j < args->d.naddrs; j++) {
                memcpy(&cs->fw6.ipv6.dst, &args->d.addr.v6[j],
                       sizeof(struct in6_addr));
                memcpy(&cs->fw6.ipv6.dmsk, &args->d.mask.v6[j],
                       sizeof(struct in6_addr));
                ret = cb(h, p, cs, append);
            }
            break;
...
static int commit(struct nft_handle *h) { return 1; }
...
static int xlate_chain_set(struct nft_handle *h, const char *table,
    ...
    printf("add chain %s %s %s { type %s ", family2str[h->family], table, chain,
           type);
...
static int xtables_restore_xlate_main(int family, const char *progname,
    ...
    printf("# Translated by %s v%s on %s", argv[0], IPTABLES_VERSION,
           ctime(&now));
...
int xtables_ip4_xlate_main(int argc, char *argv[]) {
    return xtables_xlate_main(NFPROTO_IPV4, "iptables-translate", argc, argv);
}

int xtables_ip6_xlate_main(int argc, char *argv[]) {
    return xtables_xlate_main(NFPROTO_IPV6, "ip6tables-translate", argc, argv);
}

int xtables_ip4_xlate_restore_main(int argc, char *argv[]) {
    return xtables_restore_xlate_main(NFPROTO_IPV4,
                                      "iptables-translate-restore", argc, argv);
}

int xtables_ip6_xlate_restore_main(int argc, char *argv[]) {
    return xtables_restore_xlate_main(
        NFPROTO_IPV6, "ip6tables-translate-restore", argc, argv);
}
