...
static struct xtables_match *find_proto(const char *pname,
...
static bool should_load_proto(struct iptables_command_state *cs) {
    if (cs->protocol == NULL)
        return false;
    if (find_proto(cs->protocol, XTF_DONT_LOAD, cs->options & OPT_NUMERIC,
                   NULL) == NULL)
        return true;
...
struct xtables_match *load_proto(struct iptables_command_state *cs) {
    if (!should_load_proto(cs))
        return NULL;
    return find_proto(cs->protocol, XTF_TRY_LOAD, cs->options & OPT_NUMERIC,
                      &cs->matches);
}
...
int command_default(struct iptables_command_state *cs,
    ...
    if (cs->target != NULL &&
        ...
        xtables_option_tpcall(cs->c, cs->argv, cs->invert, cs->target, &cs->fw);
        return 0;
    }
    ...
    if (m != NULL) {
        ...
        if (m->x6_options != NULL)
            gl->opts = xtables_options_xfrm(gl->orig_opts, gl->opts,
                                            m->x6_options, &m->option_offset);
        else
            gl->opts = xtables_merge_options(gl->orig_opts, gl->opts,
    ...
    while (1) {
        ...
        if (++i % 10 == 0) {
            fprintf(stderr,
                    ...
                    "still %lds %ldus time ahead to have a chance to grab the "
                    "lock...\n",
...
int parse_wait_time(int argc, char *argv[]) {
    ...
    if (optarg) {
        if (sscanf(optarg, "%i", &wait) != 1)
            xtables_error(PARAMETER_PROBLEM, "wait seconds not numeric");
...
void parse_wait_interval(int argc, char *argv[],
    ...
    if (ret == 1) {
        if (usec > 999999)
            xtables_error(PARAMETER_PROBLEM,
                          "too long usec wait %u > 999999 usec", usec);
...
inline bool xs_has_arg(int argc, char *argv[]) {
    return optind < argc && argv[optind][0] != '-' && argv[optind][0] != '!';
}
