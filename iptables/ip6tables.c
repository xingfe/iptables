...
static const char optflags[] = {'n', 's', 'd', 'p', 'j', 'v',
                                'x', 'i', 'o', '0', 'c'};
...
void ip6tables_exit_error(enum xtables_exittype status, const char *msg, ...)
...
static void __attribute__((noreturn)) exit_tryhelp(int status) {
    if (line != -1)
...
static void exit_printhelp(const struct xtables_rule_match *matches) {
    printf("%s v%s\n\n"
           ...
           prog_name, prog_vers, prog_name, prog_name, prog_name, prog_name,
    ...
    printf(
        ...
        "				Delete rule rulenum (1 = first) from "
        "chain\n"
        "  --insert  -I chain [rulenum]\n"
        "				Insert in chain as rulenum (default "
        "1=first)\n"
        "  --replace -R chain rulenum\n"
        "				Replace rule rulenum (1 = first) in "
        "chain\n"
        "  --list    -L [chain [rulenum]]\n"
        "				List the rules in a chain or all "
        "chains\n"
        "  --list-rules -S [chain [rulenum]]\n"
        "				Print the rules in a chain or all "
        "chains\n"
        "  --flush   -F [chain]		Delete all rules in  chain or "
        "all chains\n"
        ...
        "Options:\n"
        "    --ipv4	-4		Error (line is ignored by "
        "ip6tables-restore)\n"
        "    --ipv6	-6		Nothing (line is ignored by "
        "iptables-restore)\n"
        "[!] --protocol	-p proto	protocol: by number or name, "
        "eg. `tcp'\n"
        ...
        /*"[!] --fragment	-f		match second or further
           fragments only\n"*/
...
static int is_exthdr(uint16_t proto) {
    return (proto == IPPROTO_ROUTING || proto == IPPROTO_FRAGMENT ||
            proto == IPPROTO_AH || proto == IPPROTO_DSTOPTS);
}
...
static int parse_rulenumber(const char *rule) {
    ...
    if (!xtables_strtoui(rule, NULL, &rulenum, 1, INT_MAX))
        xtables_error(PARAMETER_PROBLEM, "Invalid rule number `%s'", rule);

    return rulenum;
}

static void parse_chain(const char *chainname) {
    const char *ptr;
    ...
    if (xtables_find_target(chainname, XTF_TRY_LOAD))
        xtables_error(PARAMETER_PROBLEM, "chain name may not clash "
                                         "with target name\n");

    for (ptr = chainname; *ptr; ptr++)
        if (isspace(*ptr))
            xtables_error(PARAMETER_PROBLEM, "Invalid chain name `%s'",
                          chainname);
}
...
static const char *parse_target(const char *targetname) {
    ...
    if (strlen(targetname) < 1)
        xtables_error(PARAMETER_PROBLEM, "Invalid target name (too short)");

    if (strlen(targetname) >= XT_EXTENSION_MAXNAMELEN)
        xtables_error(PARAMETER_PROBLEM,
                      "Invalid target name `%s' (%u chars max)", targetname,
                      XT_EXTENSION_MAXNAMELEN - 1);

    for (ptr = targetname; *ptr; ptr++)
        if (isspace(*ptr))
            xtables_error(PARAMETER_PROBLEM, "Invalid target name `%s'",
                          targetname);
    return targetname;
}

static void set_option(unsigned int *options, unsigned int option,
                       uint8_t *invflg, int invert) {
    if (*options & option)
    ...
    *options |= option;
    ...
    if (invert) {
        ...
        if (!inverse_for_options[i])
            xtables_error(PARAMETER_PROBLEM, "cannot have ! before -%c",
                          opt2char(option));
        *invflg |= inverse_for_options[i];
    }
}

static void print_header(unsigned int format, const char *chain,
...
static int print_match(const struct xt_entry_match *m,
                       const struct ip6t_ip6 *ip, int numeric) {
...
static void print_firewall(const struct ip6t_entry *fw, const char *targname,
    ...
    if (!ip6tc_is_chain(targname, handle))
        ...
        target = xtables_find_target(XT_STANDARD_TARGET, XTF_LOAD_MUST_SUCCEED);
...
static void print_firewall_line(const struct ip6t_entry *fw,
...
static int append_entry(const xt_chainlabel chain, struct ip6t_entry *fw,
                        unsigned int nsaddrs, const struct in6_addr saddrs[],
                        const struct in6_addr smasks[], unsigned int ndaddrs,
                        const struct in6_addr daddrs[],
                        const struct in6_addr dmasks[], int verbose,
                        struct xtc_handle *handle) {
    unsigned int i, j;
    int ret = 1;
...
static int replace_entry(const xt_chainlabel chain, struct ip6t_entry *fw,
                         unsigned int rulenum, const struct in6_addr *saddr,
                         const struct in6_addr *smask,
                         const struct in6_addr *daddr,
                         const struct in6_addr *dmask, int verbose,
                         struct xtc_handle *handle) {
    fw->ipv6.src = *saddr;
    fw->ipv6.dst = *daddr;
    fw->ipv6.smsk = *smask;
...
static int insert_entry(const xt_chainlabel chain, struct ip6t_entry *fw,
                        unsigned int rulenum, unsigned int nsaddrs,
...
static unsigned char *make_delete_mask(const struct xtables_rule_match *matches,
    ...
    mask = xtables_calloc(1, size + XT_ALIGN(sizeof(struct xt_entry_target)) +
                                 target->size);
    ...
    memset(mptr, 0xFF,
           XT_ALIGN(sizeof(struct xt_entry_target)) + target->userspacesize);

    return mask;
}

static int delete_entry(const xt_chainlabel chain, struct ip6t_entry *fw,
                        unsigned int nsaddrs, const struct in6_addr saddrs[],
                        const struct in6_addr smasks[], unsigned int ndaddrs,
                        const struct in6_addr daddrs[],
                        const struct in6_addr dmasks[], int verbose,
...
static int check_entry(const xt_chainlabel chain, struct ip6t_entry *fw,
                       unsigned int nsaddrs, const struct in6_addr *saddrs,
                       const struct in6_addr *smasks, unsigned int ndaddrs,
                       const struct in6_addr *daddrs,
                       const struct in6_addr *dmasks, bool verbose,
                       struct xtc_handle *handle,
                       struct xtables_rule_match *matches,
                       const struct xtables_target *target) {
    unsigned int i, j;
...
int for_each_chain6(int (*fn)(const xt_chainlabel, int, struct xtc_handle *),
    ...
    for (i = 0; i < chaincount; i++) {
        if (!builtinstoo &&
            ip6tc_builtin(chains + i * sizeof(xt_chainlabel), handle) == 1)
            continue;
...
int flush_entries6(const xt_chainlabel chain, int verbose,
                   struct xtc_handle *handle) {
    if (!chain)
...
static int zero_entries(const xt_chainlabel chain, int verbose,
                        struct xtc_handle *handle) {
    if (!chain)
...
int delete_chain6(const xt_chainlabel chain, int verbose,
                  struct xtc_handle *handle) {
    if (!chain)
...
static int list_entries(const xt_chainlabel chain, int rulenum, int verbose,
    ...
    for (this = ip6tc_first_chain(handle); this;
        ...
        while (i) {
            ...
            if (!rulenum || num == rulenum)
                print_firewall(i, ip6tc_get_target(i, handle), num, format,
                               handle);
    ...
    if (proto) {
        ...
        for (i = 0; xtables_chain_protos[i].name != NULL; ++i)
            if (xtables_chain_protos[i].num == proto) {
                printf("%s -p %s", invertstr, xtables_chain_protos[i].name);
                return;
            }
...
static int print_match_save(const struct xt_entry_match *e,
    ...
    if (match) {
        printf(" -m %s", match->alias ? match->alias(e) : e->u.user.name);
        ...
        if (e->u.match_size) {
            fprintf(stderr, "Can't find library for match `%s'\n",
                    e->u.user.name);
            exit(1);
        }
    }
...
void print_rule6(const struct ip6t_entry *e, struct xtc_handle *h,
    ...
    if (counters > 0)
        printf("[%llu:%llu] ", (unsigned long long)e->counters.pcnt,
    ...
    if (e->ipv6.flags & IP6T_F_TOS)
        printf("%s -? %d", e->ipv6.invflags & IP6T_INV_TOS ? " !" : "",
               e->ipv6.tos);
    ...
    if (counters < 0)
        printf(" -c %llu %llu", (unsigned long long)e->counters.pcnt,
               (unsigned long long)e->counters.bcnt);
    ...
    if (t->u.user.name[0]) {
        ...
        if (target->save && t->u.user.revision == target->revision)
             ...
             * how to print it */
            if (t->u.target_size != sizeof(struct xt_entry_target)) {
                fprintf(stderr,
                        "Target `%s' is missing "
                        "save function\n",
                        t->u.user.name);
...
static int list_rules(const xt_chainlabel chain, int rulenum, int counters,
                      struct xtc_handle *handle) {
    ...
    if (!rulenum)
        for (this = ip6tc_first_chain(handle); this;
            ...
            if (ip6tc_builtin(this, handle)) {
                ...
                printf("-P %s %s", this,
                       ip6tc_get_policy(this, &count, handle));
                if (counters)
                    printf(" -c %llu %llu", (unsigned long long)count.pcnt,
...
static struct ip6t_entry *generate_entry(const struct ip6t_entry *fw,
                                         struct xtables_rule_match *matches,
...
static void command_match(struct iptables_command_state *cs) {
    ...
    if (cs->invert)
        xtables_error(PARAMETER_PROBLEM, "unexpected ! flag before --match");
...
int do_command6(int argc, char *argv[], char **table,
    ...
    while ((cs.c = getopt_long(argc, argv,
                               "-:A:C:D:R:I:L::S::M:F::Z::N:X::E:P:Vh::o:p:s:d:"
                               "j:i:bvw::W::nt:m:xc:g:46",
                               opts, NULL)) != -1) {
        switch (cs.c) {
        /*
         * Command selection
        ...
        case 'C':
            add_command(&command, CMD_CHECK, CMD_NONE, cs.invert);
            chain = optarg;
            break;

        case 'D':
            add_command(&command, CMD_DELETE, CMD_NONE, cs.invert);
            chain = optarg;
        ...
        case 'R':
            add_command(&command, CMD_REPLACE, CMD_NONE, cs.invert);
            chain = optarg;
            if (xs_has_arg(argc, argv))
                rulenum = parse_rulenumber(argv[optind++]);
            else
                xtables_error(PARAMETER_PROBLEM, "-%c requires a rule number",
                              cmd2char(CMD_REPLACE));
            break;

        case 'I':
            add_command(&command, CMD_INSERT, CMD_NONE, cs.invert);
            chain = optarg;
            if (xs_has_arg(argc, argv))
                rulenum = parse_rulenumber(argv[optind++]);
            else
        ...
        case 'L':
            add_command(&command, CMD_LIST, CMD_ZERO | CMD_ZERO_NUM, cs.invert);
            if (optarg)
                chain = optarg;
        ...
        case 'S':
            add_command(&command, CMD_LIST_RULES, CMD_ZERO | CMD_ZERO_NUM,
                        cs.invert);
            if (optarg)
                chain = optarg;
            else if (xs_has_arg(argc, argv))
        ...
        case 'F':
            add_command(&command, CMD_FLUSH, CMD_NONE, cs.invert);
            if (optarg)
                chain = optarg;
            else if (xs_has_arg(argc, argv))
                chain = argv[optind++];
        ...
        case 'N':
            parse_chain(optarg);
            add_command(&command, CMD_NEW_CHAIN, CMD_NONE, cs.invert);
            chain = optarg;
            break;

        case 'X':
            add_command(&command, CMD_DELETE_CHAIN, CMD_NONE, cs.invert);
            if (optarg)
                chain = optarg;
            else if (xs_has_arg(argc, argv))
                chain = argv[optind++];
        ...
        case 'E':
            add_command(&command, CMD_RENAME_CHAIN, CMD_NONE, cs.invert);
            chain = optarg;
            if (xs_has_arg(argc, argv))
                newname = argv[optind++];
            else
        ...
        case 'P':
            add_command(&command, CMD_SET_POLICY, CMD_NONE, cs.invert);
            chain = optarg;
            if (xs_has_arg(argc, argv))
                policy = argv[optind++];
            else
        ...
        case 'h':
            ...
            if (!cs.matches && cs.protocol)
                xtables_find_match(cs.protocol, XTF_TRY_LOAD, &cs.matches);
        ...
        case 'p':
            ...
            if (cs.fw6.ipv6.proto == 0 && (cs.fw6.ipv6.invflags & XT_INV_PROTO))
                xtables_error(PARAMETER_PROBLEM,
        ...
        case 'i':
            ...
            set_option(&cs.options, OPT_VIANAMEIN, &cs.fw6.ipv6.invflags,
                       cs.invert);
            xtables_parse_interface(optarg, cs.fw6.ipv6.iniface,
        ...
        case 'o':
            ...
            set_option(&cs.options, OPT_VIANAMEOUT, &cs.fw6.ipv6.invflags,
                       cs.invert);
            xtables_parse_interface(optarg, cs.fw6.ipv6.outiface,
                                    cs.fw6.ipv6.outiface_mask);
            break;

        case 'v':
            if (!verbose)
                set_option(&cs.options, OPT_VERBOSE, &cs.fw6.ipv6.invflags,
                           cs.invert);
            verbose++;
            break;
        ...
        case 'w':
            if (restore) {
                xtables_error(PARAMETER_PROBLEM, "You cannot use `-w' from "
                                                 "ip6tables-restore");
            }
        ...
        case 'c':
            ...
            if (sscanf(bcnt, "%llu", &cnt) != 1)
                xtables_error(PARAMETER_PROBLEM, "-%c byte counter not numeric",
                              opt2char(OPT_COUNTERS));
    ...
    if (cs.invert)
        xtables_error(PARAMETER_PROBLEM, "nothing appropriate following !");

    if (command &
        (CMD_REPLACE | CMD_INSERT | CMD_DELETE | CMD_APPEND | CMD_CHECK)) {
    ...
    if (shostnetworkmask)
        xtables_ip6parse_multiple(shostnetworkmask, &saddrs, &smasks, &nsaddrs);

    if (dhostnetworkmask)
        xtables_ip6parse_multiple(dhostnetworkmask, &daddrs, &dmasks, &ndaddrs);
    ...
    if (command == CMD_APPEND || command == CMD_DELETE ||
        ...
        /* If they didn't specify a target, or it's a chain
           name, use standard. */
        if (!cs.target &&
            (strlen(cs.jumpto) == 0 || ip6tc_is_chain(cs.jumpto, *handle))) {
            size_t size;
 ...
 */
    ...
    switch (command) {
    case CMD_APPEND:
        ret = append_entry(chain, e, nsaddrs, saddrs, smasks, ndaddrs, daddrs,
                           dmasks, cs.options & OPT_VERBOSE, *handle);
        break;
    case CMD_DELETE:
        ret = delete_entry(chain, e, nsaddrs, saddrs, smasks, ndaddrs, daddrs,
                           dmasks, cs.options & OPT_VERBOSE, *handle,
                           cs.matches, cs.target);
        break;
    case CMD_DELETE_NUM:
        ret = ip6tc_delete_num_entry(chain, rulenum - 1, *handle);
        break;
    case CMD_CHECK:
        ret = check_entry(chain, e, nsaddrs, saddrs, smasks, ndaddrs, daddrs,
                          dmasks, cs.options & OPT_VERBOSE, *handle, cs.matches,
                          cs.target);
        break;
    case CMD_REPLACE:
        ret = replace_entry(chain, e, rulenum - 1, saddrs, smasks, daddrs,
                            dmasks, cs.options & OPT_VERBOSE, *handle);
        break;
    case CMD_INSERT:
        ret = insert_entry(chain, e, rulenum - 1, nsaddrs, saddrs, smasks,
                           ndaddrs, daddrs, dmasks, cs.options & OPT_VERBOSE,
                           *handle);
        break;
    case CMD_FLUSH:
    ...
    case CMD_LIST | CMD_ZERO_NUM:
        ret = list_entries(chain, rulenum, cs.options & OPT_VERBOSE,
                           cs.options & OPT_NUMERIC, cs.options & OPT_EXPANDED,
                           cs.options & OPT_LINENUMBERS, *handle);
    ...
    case CMD_LIST_RULES | CMD_ZERO_NUM:
        ret = list_rules(chain, rulenum, cs.options & OPT_VERBOSE, *handle);
        if (ret && (command & CMD_ZERO))
            ret = zero_entries(chain, cs.options & OPT_VERBOSE, *handle);
        if (ret && (command & CMD_ZERO_NUM))
            ret = ip6tc_zero_counter(chain, rulenum, *handle);
    ...
    case CMD_RENAME_CHAIN:
        ...
        break;
    case CMD_SET_POLICY:
        ret = ip6tc_set_policy(
            chain, policy, cs.options & OPT_COUNTERS ? &cs.fw6.counters : NULL,
            *handle);
        break;
    default:
