...
static const char cmdflags[] = {'I', 'D', 'D', 'R', 'A', 'L',
                                'F', 'Z', 'N', 'X', 'P', 'E'};
...
static struct option original_opts[] = {{"append", 1, 0, 'A'},
                                        {"delete", 1, 0, 'D'},
                                        {"insert", 1, 0, 'I'},
                                        {"replace", 1, 0, 'R'},
                                        {"list", 2, 0, 'L'},
...
static void exit_printhelp(void) {
    ...
    printf(
        ...
        "chain\n"
        "  --list    -L [chain]		List the rules in a chain or "
        "all chains\n"
    ...
    for (i = 0; i < NUMOPCODES; i++)
        printf(" %d = %s\n", i + 1, opcodes[i]);
    printf(" hardware type string: 1 = Ethernet\n"
...
static void add_command(unsigned int *cmd, const int newcmd,
                        const unsigned int othercmds, int invert) {
    if (invert)
...
static int check_inverse(const char option[], int *invert, int *optidx,
                         int argc) {
    if (option && strcmp(option, "!") == 0) {
        if (*invert)
            xtables_error(PARAMETER_PROBLEM, "Multiple `!' flags not allowed");
        *invert = TRUE;
        if (optidx) {
            *optidx = *optidx + 1;
            if (argc && *optidx > argc)
...
static struct in_addr *parse_mask(char *mask) {
    ...
    if (string_to_number(mask, 0, 32, &bits) == -1)
        xtables_error(PARAMETER_PROBLEM, "invalid mask `%s' specified", mask);
    if (bits != 0) {
...
static void parse_interface(const char *arg, char *vianame,
                            unsigned char *mask) {
...
static const char *parse_target(const char *targetname) {
    ...
    if (strlen(targetname) + 1 > sizeof(arpt_chainlabel))
        xtables_error(PARAMETER_PROBLEM,
                      "Invalid target name `%s' (%zu chars max)", targetname,
                      sizeof(arpt_chainlabel) - 1);
...
static int list_entries(struct nft_handle *h, const char *chain,
                        const char *table, int rulenum, int verbose,
...
static struct xtables_target *command_jump(struct arpt_entry *fw,
    ...
    if (target->x6_options != NULL)
        opts = xtables_options_xfrm(arptables_globals.orig_opts, opts,
                                    target->x6_options, &target->option_offset);
    else
        opts =
            xtables_merge_options(arptables_globals.orig_opts, opts,
                                  target->extra_opts, &target->option_offset);

    return target;
}

static int append_entry(struct nft_handle *h, const char *chain,
                        const char *table, struct arptables_command_state *cs,
                        int rulenum, unsigned int nsaddrs,
                        const struct in_addr saddrs[], unsigned int ndaddrs,
                        const struct in_addr daddrs[], bool verbose,
                        bool append) {
    unsigned int i, j;
    int ret = 1;
    ...
    for (i = 0; i < nsaddrs; i++) {
        ...
        for (j = 0; j < ndaddrs; j++) {
            ...
            if (append) {
                ...
                ret = nft_rule_insert(h, chain, table, cs, rulenum, verbose);
            }
        }
    }
...
static int replace_entry(const char *chain, const char *table,
                         struct arptables_command_state *cs,
                         unsigned int rulenum, const struct in_addr *saddr,
                         const struct in_addr *daddr, bool verbose,
                         struct nft_handle *h) {
...
static int delete_entry(const char *chain, const char *table,
                        struct arptables_command_state *cs,
...
int do_commandarp(struct nft_handle *h, int argc, char *argv[], char **table) {
    ...
    while ((c = getopt_long(
                argc, argv,
                "-A:D:R:I:L::M:F::Z::N:X::E:P:Vh::o:p:s:d:j:l:i:vnt:m:c:", opts,
                NULL)) != -1) {
        switch (c) {
        /*
         * Command selection
        ...
        case 'L':
            add_command(&command, CMD_LIST, CMD_ZERO, invert);
            if (optarg)
                chain = optarg;
        ...
        case 'X':
            add_command(&command, CMD_DELETE_CHAIN, CMD_NONE, invert);
            if (optarg)
                chain = optarg;
            else if (xs_has_arg(argc, argv))
                chain = argv[optind++];
        ...
        case 'E':
            add_command(&command, CMD_RENAME_CHAIN, CMD_NONE, invert);
            chain = optarg;
            if (xs_has_arg(argc, argv))
                newname = argv[optind++];
            else
        ...
        case 'P':
            add_command(&command, CMD_SET_POLICY, CMD_NONE, invert);
            chain = optarg;
            if (xs_has_arg(argc, argv))
                policy = argv[optind++];
            else
        ...
        case 's':
            ...
            set_option(&options, OPT_S_IP, &cs.fw.arp.invflags, invert);
            shostnetworkmask = argv[optind - 1];
            break;

        case 'd':
            check_inverse(optarg, &invert, &optind, argc);
            set_option(&options, OPT_D_IP, &cs.fw.arp.invflags, invert);
            dhostnetworkmask = argv[optind - 1];
            break;

        case 2: /* src-mac */
            check_inverse(optarg, &invert, &optind, argc);
            set_option(&options, OPT_S_MAC, &cs.fw.arp.invflags, invert);
            if (getmac_and_mask(argv[optind - 1], cs.fw.arp.src_devaddr.addr,
                                cs.fw.arp.src_devaddr.mask))
        ...
        case 3: /* dst-mac */
            check_inverse(optarg, &invert, &optind, argc);
            set_option(&options, OPT_D_MAC, &cs.fw.arp.invflags, invert);

            if (getmac_and_mask(argv[optind - 1], cs.fw.arp.tgt_devaddr.addr,
                                cs.fw.arp.tgt_devaddr.mask))
                xtables_error(PARAMETER_PROBLEM, "Problem with specified "
                                                 "destination mac");
            break;
        ...
        case 'l': /* hardware length */
            check_inverse(optarg, &invert, &optind, argc);
            set_option(&options, OPT_H_LENGTH, &cs.fw.arp.invflags, invert);
        ...
        /*
                                check_inverse(optarg, &invert, &optind, argc);
                                set_option(&options, OPT_P_LENGTH,
           &cs.fw.arp.invflags, invert);

                                getlength_and_mask(argv[optind - 1],
           &cs.fw.arp.arpln, &cs.fw.arp.arpln_mask); break;
        */

        case 4: /* opcode */
            check_inverse(optarg, &invert, &optind, argc);
            set_option(&options, OPT_OPCODE, &cs.fw.arp.invflags, invert);
        ...
        case 5: /* h-type */
            check_inverse(optarg, &invert, &optind, argc);
            set_option(&options, OPT_H_TYPE, &cs.fw.arp.invflags, invert);
        ...
        case 6: /* proto-type */
            check_inverse(optarg, &invert, &optind, argc);
            set_option(&options, OPT_P_TYPE, &cs.fw.arp.invflags, invert);
        ...
        case 'j':
            set_option(&options, OPT_JUMP, &cs.fw.arp.invflags, invert);
        ...
        case 'i':
            check_inverse(optarg, &invert, &optind, argc);
            set_option(&options, OPT_VIANAMEIN, &cs.fw.arp.invflags, invert);
            parse_interface(argv[optind - 1], cs.fw.arp.iniface,
        ...
        case 'o':
            check_inverse(optarg, &invert, &optind, argc);
            set_option(&options, OPT_VIANAMEOUT, &cs.fw.arp.invflags, invert);
            parse_interface(argv[optind - 1], cs.fw.arp.outiface,
        ...
        case 'v':
            if (!verbose)
                set_option(&options, OPT_VERBOSE, &cs.fw.arp.invflags, invert);
            verbose++;
            break;
        ...
        case 'n':
            set_option(&options, OPT_NUMERIC, &cs.fw.arp.invflags, invert);
            break;
        ...
        case '0':
            set_option(&options, OPT_LINENUMBERS, &cs.fw.arp.invflags, invert);
            break;
        ...
        case 'c':
            ...
            set_option(&options, OPT_COUNTERS, &cs.fw.arp.invflags, invert);
            pcnt = optarg;
            if (xs_has_arg(argc, argv))
                bcnt = argv[optind++];
            else
        ...
        default:
            if (cs.target) {
                xtables_option_tpcall(c, argv, invert, cs.target, &cs.fw);
            }
            break;
        }
        invert = FALSE;
    ...
    if (shostnetworkmask)
        parse_hostnetworkmask(shostnetworkmask, &saddrs, &(cs.fw.arp.smsk),
                              &nsaddrs);

    if (dhostnetworkmask)
        parse_hostnetworkmask(dhostnetworkmask, &daddrs, &(cs.fw.arp.tmsk),
                              &ndaddrs);
    ...
    if (command == CMD_APPEND || command == CMD_DELETE ||
        command == CMD_INSERT || command == CMD_REPLACE) {
    ...
    switch (command) {
    case CMD_APPEND:
        ret = append_entry(h, chain, *table, &cs, 0, nsaddrs, saddrs, ndaddrs,
                           daddrs, options & OPT_VERBOSE, true);
        break;
    case CMD_DELETE:
        ret = delete_entry(chain, *table, &cs, nsaddrs, saddrs, ndaddrs, daddrs,
    ...
    case CMD_DELETE_NUM:
        ...
        break;
    case CMD_REPLACE:
        ret = replace_entry(chain, *table, &cs, rulenum - 1, saddrs, daddrs,
                            options & OPT_VERBOSE, h);
        break;
    case CMD_INSERT:
        ret = append_entry(h, chain, *table, &cs, rulenum - 1, nsaddrs, saddrs,
                           ndaddrs, daddrs, options & OPT_VERBOSE, false);
        break;
    case CMD_LIST:
        ret =
            list_entries(h, chain, *table, rulenum, options & OPT_VERBOSE,
                         options & OPT_NUMERIC,
                         /*options&OPT_EXPANDED*/ 0, options & OPT_LINENUMBERS);
        break;
    case CMD_FLUSH:
