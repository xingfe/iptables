...
static int nft_ipv6_add(struct nftnl_rule *r, void *data) {
    ...
    if (cs->fw6.ipv6.proto != 0) {
        ...
        add_proto(r, offsetof(struct ip6_hdr, ip6_nxt), 1, cs->fw6.ipv6.proto,
                  op);
    }

    if (!IN6_IS_ADDR_UNSPECIFIED(&cs->fw6.ipv6.src)) {
        op = nft_invflags2cmp(cs->fw6.ipv6.invflags, IPT_INV_SRCIP);
        add_addr(r, offsetof(struct ip6_hdr, ip6_src), &cs->fw6.ipv6.src,
                 &cs->fw6.ipv6.smsk, sizeof(struct in6_addr), op);
    }
    if (!IN6_IS_ADDR_UNSPECIFIED(&cs->fw6.ipv6.dst)) {
        op = nft_invflags2cmp(cs->fw6.ipv6.invflags, IPT_INV_DSTIP);
        add_addr(r, offsetof(struct ip6_hdr, ip6_dst), &cs->fw6.ipv6.dst,
                 &cs->fw6.ipv6.dmsk, sizeof(struct in6_addr), op);
    }
...
static bool nft_ipv6_is_same(const void *data_a, const void *data_b) {
    ...
    return is_same_interfaces(
        a->fw6.ipv6.iniface, a->fw6.ipv6.outiface, a->fw6.ipv6.iniface_mask,
        a->fw6.ipv6.outiface_mask, b->fw6.ipv6.iniface, b->fw6.ipv6.outiface,
        b->fw6.ipv6.iniface_mask, b->fw6.ipv6.outiface_mask);
}
...
static void nft_ipv6_parse_payload(struct nft_xt_ctx *ctx, struct nftnl_expr *e,
                                   void *data) {
    struct iptables_command_state *cs = data;
...
static void print_ipv6_addr(const struct iptables_command_state *cs,
    ...
    if (IN6_IS_ADDR_UNSPECIFIED(&cs->fw6.ipv6.src) && !(format & FMT_NUMERIC))
        printf(FMT("%-19s ", "%s "), "anywhere");
    else {
        if (format & FMT_NUMERIC)
            strcpy(buf, xtables_ip6addr_to_numeric(&cs->fw6.ipv6.src));
        else
            strcpy(buf, xtables_ip6addr_to_anyname(&cs->fw6.ipv6.src));
        strcat(buf, xtables_ip6mask_to_numeric(&cs->fw6.ipv6.smsk));
        printf(FMT("%-19s ", "%s "), buf);
    }

    fputc(cs->fw6.ipv6.invflags & IP6T_INV_DSTIP ? '!' : ' ', stdout);
    if (IN6_IS_ADDR_UNSPECIFIED(&cs->fw6.ipv6.dst) && !(format & FMT_NUMERIC))
        printf(FMT("%-19s ", "-> %s"), "anywhere");
    else {
        if (format & FMT_NUMERIC)
            strcpy(buf, xtables_ip6addr_to_numeric(&cs->fw6.ipv6.dst));
        else
            strcpy(buf, xtables_ip6addr_to_anyname(&cs->fw6.ipv6.dst));
...
static void nft_ipv6_print_firewall(struct nftnl_rule *r, unsigned int num,
    ...
    print_firewall_details(&cs, cs.jumpto, cs.fw6.ipv6.flags,
                           cs.fw6.ipv6.invflags, cs.fw6.ipv6.proto, num,
                           format);
...
static void nft_ipv6_post_parse(int command, struct iptables_command_state *cs,
    ...
    memcpy(cs->fw6.ipv6.iniface_mask, args->iniface_mask,
           IFNAMSIZ * sizeof(unsigned char));

    strncpy(cs->fw6.ipv6.outiface, args->outiface, IFNAMSIZ);
    memcpy(cs->fw6.ipv6.outiface_mask, args->outiface_mask,
           IFNAMSIZ * sizeof(unsigned char));
    ...
    if (args->shostnetworkmask)
        xtables_ip6parse_multiple(args->shostnetworkmask, &args->s.addr.v6,
                                  &args->s.mask.v6, &args->s.naddrs);
    if (args->dhostnetworkmask)
        xtables_ip6parse_multiple(args->dhostnetworkmask, &args->d.addr.v6,
                                  &args->d.mask.v6, &args->d.naddrs);
...
static bool nft_ipv6_rule_find(struct nft_family_ops *ops, struct nftnl_rule *r,
                               void *data) {
    struct iptables_command_state *cs = data;
...
static int nft_ipv6_xlate(const void *data, struct xt_xlate *xl) {
    ...
    if (cs->fw6.ipv6.proto != 0) {
        const struct protoent *pent = getprotobynumber(cs->fw6.ipv6.proto);
        char protonum[strlen("255") + 1];

        if (!xlate_find_match(cs, pent->p_name)) {
            snprintf(protonum, sizeof(protonum), "%u", cs->fw6.ipv6.proto);
