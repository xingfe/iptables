...
static ssize_t mnl_nft_socket_sendmsg(const struct mnl_socket *nl) {
    ...
    list_for_each_entry(batch_page, &batch_page_list, head) {
        ...
        i++;
#ifdef NL_DEBUG
        mnl_nlmsg_fprintf(stdout, mnl_nlmsg_batch_head(batch_page->batch),
...
static int batch_add(struct nft_handle *h, enum obj_update_type type,
                     void *ptr) {
...
static struct nftnl_chain *nft_chain_builtin_alloc(struct builtin_table *table,
...
int nft_chain_add(struct nft_handle *h, struct nftnl_chain *c, uint16_t flags) {
    ...
    nlh = nftnl_chain_nlmsg_build_hdr(buf, NFT_MSG_NEWCHAIN, h->family,
                                      NLM_F_ACK | flags | NLM_F_CREATE, h->seq);
...
static struct builtin_chain *nft_chain_builtin_find(struct builtin_table *t,
                                                    const char *chain) {
    int i;
    bool found = false;
...
static void nft_chain_builtin_init(struct nft_handle *h,
    ...
    for (i = 0; i < NF_IP_NUMHOOKS && table->chains[i].name != NULL; i++) {
        ...
        c = nft_chain_list_find(list, table->name, table->chains[i].name);
        if (c != NULL)
            continue;
...
static bool mnl_batch_supported(struct nft_handle *h) {
    ...
    nftnl_set_nlmsg_build_hdr(mnl_nlmsg_batch_current(h->batch), NFT_MSG_NEWSET,
    ...
    ret = mnl_socket_recvfrom(h->nl, buf, sizeof(buf));
    while (ret > 0) {
        ret = mnl_cb_run(buf, ret, 0, mnl_socket_get_portid(h->nl), NULL, NULL);
        if (ret <= 0)
            break;
...
static int __add_match(struct nftnl_expr *e, struct xt_entry_match *m) {
    ...
    nftnl_expr_set(e, NFTNL_EXPR_MT_NAME, m->u.user.name,
...
int add_comment(struct nftnl_rule *r, const char *comment) {
    ...
    if (!nftnl_udata_put_strz(udata, UDATA_TYPE_COMMENT, comment))
        return -ENOMEM;
    nftnl_rule_set_data(r, NFTNL_RULE_USERDATA, nftnl_udata_buf_data(udata),
...
static struct nftnl_rule *nft_rule_new(struct nft_handle *h, const char *chain,
...
int nft_rule_append(struct nft_handle *h, const char *chain, const char *table,
...
void nft_rule_print_save(const void *data, struct nftnl_rule *r,
...
int nft_chain_save(struct nft_handle *h, struct nftnl_chain_list *list,
    ...
    while (c != NULL) {
        const char *chain_table = nftnl_chain_get_str(c, NFTNL_CHAIN_TABLE);
...
int nft_rule_save(struct nft_handle *h, const char *table, bool counters) {
    ...
    while (r != NULL) {
        const char *rule_table = nftnl_rule_get_str(r, NFTNL_RULE_TABLE);
...
static void __nft_rule_flush(struct nft_handle *h, const char *table,
                             const char *chain) {
    struct nftnl_rule *r;
...
int nft_chain_user_add(struct nft_handle *h, const char *chain,
                       const char *table) {
    struct nftnl_chain *c;
    int ret;
...
bool nft_table_find(struct nft_handle *h, const char *tablename) {
    ...
    while (t != NULL) {
        const char *this_tablename = nftnl_table_get(t, NFTNL_TABLE_NAME);
...
int nft_for_each_table(struct nft_handle *h,
                       int (*func)(struct nft_handle *h, const char *tablename,
                                   bool counters),
                       bool counters) {
    int ret = 1;
    struct nftnl_table_list *list;
    ...
    t = nftnl_table_list_iter_next(iter);
    while (t != NULL) {
        const char *tablename = nftnl_table_get(t, NFTNL_TABLE_NAME);
...
int nft_table_purge_chains(struct nft_handle *h, const char *this_table,
    ...
    while (chain_obj != NULL) {
        const char *table = nftnl_chain_get_str(chain_obj, NFTNL_CHAIN_TABLE);
...
static struct nftnl_rule *nft_rule_find(struct nft_handle *h,
                                        struct nftnl_rule_list *list,
                                        const char *chain, const char *table,
                                        void *data, int rulenum) {
    struct nftnl_rule *r;
    struct nftnl_rule_list_iter *iter;
    int rule_ctr = 0;
    bool found = false;
    ...
    while (r != NULL) {
        const char *rule_table = nftnl_rule_get_str(r, NFTNL_RULE_TABLE);
        const char *rule_chain = nftnl_rule_get_str(r, NFTNL_RULE_CHAIN);

        if (strcmp(table, rule_table) != 0 || strcmp(chain, rule_chain) != 0) {
...
int nft_rule_delete(struct nft_handle *h, const char *chain, const char *table,
                    void *data, bool verbose) {
    int ret = 0;
    struct nftnl_rule *r;
    struct nftnl_rule_list *list;
...
static int nft_rule_add(struct nft_handle *h, const char *chain,
...
int nft_rule_insert(struct nft_handle *h, const char *chain, const char *table,
                    void *data, int rulenum, bool verbose) {
    struct nftnl_rule_list *list;
    struct nftnl_rule *r;
    uint64_t handle = 0;
    ...
    if (rulenum > 0) {
        ...
        if (r == NULL) {
             ...
             */
            r = nft_rule_find(h, list, chain, table, data, rulenum - 1);
            if (r != NULL) {
                flush_rule_cache(h);
                return nft_rule_append(h, chain, table, data, 0, verbose);
            }
...
int nft_rule_replace(struct nft_handle *h, const char *chain, const char *table,
                     void *data, int rulenum, bool verbose) {
    int ret = 0;
    struct nftnl_rule *r;
    struct nftnl_rule_list *list;
    ...
    if (r != NULL) {
        DEBUGP("replacing rule with handle=%llu\n",
               (unsigned long long)nftnl_rule_get_u64(r, NFTNL_RULE_HANDLE));

        ret =
            nft_rule_append(h, chain, table, data,
                            nftnl_rule_get_u64(r, NFTNL_RULE_HANDLE), verbose);
    } else
        errno = ENOENT;
...
static int __nft_rule_list(struct nft_handle *h, const char *chain,
...
int nft_rule_list(struct nft_handle *h, const char *chain, const char *table,
    ...
    if (chain && rulenum) {
        __nft_rule_list(h, chain, table, rulenum, format, ops->print_firewall);
        return 1;
    }
    ...
    while (c != NULL) {
        ...
        ops->print_header(format, chain_name, policy_name[policy], &ctrs,
                          basechain, refs);

        __nft_rule_list(h, chain_name, table, rulenum, format,
                        ops->print_firewall);
...
static void list_save(struct nftnl_rule *r, unsigned int num,
                      unsigned int format) {
    struct iptables_command_state cs = {};
...
static int nftnl_rule_list_chain_save(struct nft_handle *h, const char *chain,
                                      const char *table,
                                      struct nftnl_chain_list *list,
                                      int counters) {
    struct nftnl_chain_list_iter *iter;
...
int nft_rule_zero_counters(struct nft_handle *h, const char *chain,
    ...
    ret = nft_rule_append(h, chain, table, &cs,
                          nftnl_rule_get_u64(r, NFTNL_RULE_HANDLE), false);
...
static void nft_compat_table_batch_add(struct nft_handle *h, uint16_t type,
    ...
    nlh = nftnl_table_nlmsg_build_hdr(mnl_nlmsg_batch_current(h->batch), type,
                                      h->family, flags, seq);
...
static void nft_compat_chain_batch_add(struct nft_handle *h, uint16_t type,
    ...
    nlh = nftnl_chain_nlmsg_build_hdr(mnl_nlmsg_batch_current(h->batch), type,
                                      h->family, flags, seq);
...
static void nft_compat_rule_batch_add(struct nft_handle *h, uint16_t type,
    ...
    nlh = nftnl_rule_nlmsg_build_hdr(mnl_nlmsg_batch_current(h->batch), type,
                                     h->family, flags, seq);
...
static int nft_action(struct nft_handle *h, int action) {
    ...
    list_for_each_entry_safe(n, tmp, &h->obj_list, head) {
        ...
        case NFT_COMPAT_TABLE_ADD:
            nft_compat_table_batch_add(h, NFT_MSG_NEWTABLE, NLM_F_CREATE, seq++,
                                       n->table);
            break;
        case NFT_COMPAT_CHAIN_ADD:
            nft_compat_chain_batch_add(h, NFT_MSG_NEWCHAIN, NLM_F_CREATE, seq++,
                                       n->chain);
            break;
        case NFT_COMPAT_CHAIN_USER_ADD:
            nft_compat_chain_batch_add(h, NFT_MSG_NEWCHAIN, NLM_F_EXCL, seq++,
                                       n->chain);
            break;
        case NFT_COMPAT_CHAIN_USER_DEL:
            nft_compat_chain_batch_add(h, NFT_MSG_DELCHAIN, 0, seq++, n->chain);
            break;
        case NFT_COMPAT_CHAIN_UPDATE:
            nft_compat_chain_batch_add(h, NFT_MSG_NEWCHAIN,
                                       h->restore ? NLM_F_CREATE : 0, seq++,
                                       n->chain);
            break;
        case NFT_COMPAT_CHAIN_RENAME:
            nft_compat_chain_batch_add(h, NFT_MSG_NEWCHAIN, 0, seq++, n->chain);
            break;
        ...
        case NFT_COMPAT_RULE_INSERT:
            nft_compat_rule_batch_add(h, NFT_MSG_NEWRULE, NLM_F_CREATE, seq++,
                                      n->rule);
            break;
        case NFT_COMPAT_RULE_REPLACE:
            nft_compat_rule_batch_add(h, NFT_MSG_NEWRULE,
                                      NLM_F_CREATE | NLM_F_REPLACE, seq++,
                                      n->rule);
            break;
        case NFT_COMPAT_RULE_DELETE:
        case NFT_COMPAT_RULE_FLUSH:
            nft_compat_rule_batch_add(h, NFT_MSG_DELRULE, 0, seq++, n->rule);
            break;
        }
...
int nft_abort(struct nft_handle *h) { return nft_action(h, NFT_COMPAT_ABORT); }
...
int nft_compatible_revision(const char *name, uint8_t rev, int opt) {
    ...
    if (opt == IPT_SO_GET_REVISION_MATCH || opt == IP6T_SO_GET_REVISION_MATCH)
        type = 0;
    else
        type = 1;
...
const char *nft_strerror(int err) {
    ...
    static struct table_struct {
        ...
        {nft_chain_user_del, EMLINK, "Can't delete chain with references left"},
        {nft_chain_user_add, EEXIST, "Chain already exists"},
        {nft_rule_add, E2BIG, "Index of insertion too big"},
        {nft_rule_check, ENOENT,
    ...
    for (i = 0; i < sizeof(table) / sizeof(struct table_struct); i++) {
        if ((!table[i].fn || table[i].fn == nft_fn) && table[i].err == err)
...
int nft_xtables_config_load(struct nft_handle *h, const char *filename,
    ...
    if (xtables_config_parse(filename, table_list, chain_list) < 0) {
        if (errno == ENOENT) {
            xtables_config_perror(
                flags, "configuration file `%s' does not exists\n", filename);
        } else {
    ...
    while ((table = nftnl_table_list_iter_next(titer)) != NULL) {
        table_family = nftnl_table_get_u32(table, NFTNL_TABLE_FAMILY);
        ...
        if (batch_table_add(h, NFT_COMPAT_TABLE_ADD, table) < 0) {
            if (errno == EEXIST) {
                xtables_config_perror(
                    flags, "table `%s' already exists, skipping\n",
    ...
    while ((chain = nftnl_chain_list_iter_next(citer)) != NULL) {
        chain_family = nftnl_chain_get_u32(chain, NFTNL_CHAIN_TABLE);
        ...
        xtables_config_perror(
            flags, "chain `%s' in table `%s' has been created\n",
            (char *)nftnl_chain_get(chain, NFTNL_CHAIN_NAME),
            (char *)nftnl_chain_get(chain, NFTNL_CHAIN_TABLE));
    }
...
int nft_chain_zero_counters(struct nft_handle *h, const char *chain,
    ...
    c = nftnl_chain_list_iter_next(iter);
    while (c != NULL) {
        const char *chain_name = nftnl_chain_get(c, NFTNL_CHAIN_NAME);
        const char *chain_table = nftnl_chain_get(c, NFTNL_CHAIN_TABLE);
...
static const char *supported_exprs[NFT_COMPAT_EXPR_MAX] = {
    "match", "target",  "payload", "meta",
    "cmp",   "bitwise", "counter", "immediate"};
