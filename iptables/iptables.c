...
static int print_match(const struct xt_entry_match *m, const struct ipt_ip *ip,
                       int numeric) {
...
static void print_firewall(const struct ipt_entry *fw, const char *targname,
...
static int append_entry(const xt_chainlabel chain, struct ipt_entry *fw,
                        ...
                        const struct in_addr dmasks[], int verbose,
                        struct xtc_handle *handle) {
    unsigned int i, j;
    int ret = 1;
...
static int replace_entry(const xt_chainlabel chain, struct ipt_entry *fw,
                         unsigned int rulenum, const struct in_addr *saddr,
                         const struct in_addr *smask,
                         const struct in_addr *daddr,
                         const struct in_addr *dmask, int verbose,
                         struct xtc_handle *handle) {
...
static int insert_entry(const xt_chainlabel chain, struct ipt_entry *fw,
...
static int check_entry(const xt_chainlabel chain, struct ipt_entry *fw,
                       unsigned int nsaddrs, const struct in_addr *saddrs,
                       const struct in_addr *smasks, unsigned int ndaddrs,
                       const struct in_addr *daddrs,
                       const struct in_addr *dmasks, bool verbose,
                       struct xtc_handle *handle,
                       struct xtables_rule_match *matches,
                       const struct xtables_target *target) {
    unsigned int i, j;
...
int for_each_chain4(int (*fn)(const xt_chainlabel, int, struct xtc_handle *),
    ...
    for (i = 0; i < chaincount; i++) {
        if (!builtinstoo &&
            iptc_builtin(chains + i * sizeof(xt_chainlabel), handle) == 1)
            continue;
...
int flush_entries4(const xt_chainlabel chain, int verbose,
                   struct xtc_handle *handle) {
    if (!chain)
...
int delete_chain4(const xt_chainlabel chain, int verbose,
                  struct xtc_handle *handle) {
    if (!chain)
    ...
    for (this = iptc_first_chain(handle); this;
        ...
        while (i) {
            ...
            if (!rulenum || num == rulenum)
                print_firewall(i, iptc_get_target(i, handle), num, format,
                               handle);
...
#define IP_PARTS_NATIVE(n)                                                     \
    (unsigned int)((n) >> 24) & 0xFF, (unsigned int)((n) >> 16) & 0xFF,        \
        (unsigned int)((n) >> 8) & 0xFF, (unsigned int)((n)&0xFF)
...
static void print_ip(const char *prefix, uint32_t ip, uint32_t mask,
                     int invert) {
    ...
    printf("%s %s %u.%u.%u.%u", invert ? " !" : "", prefix, IP_PARTS(ip));
...
int do_command4(int argc, char *argv[], char **table,
    ...
    while ((cs.c = getopt_long(argc, argv,
        ...
        case 'p':
            ...
            if (cs.fw.ip.proto == 0 && (cs.fw.ip.invflags & XT_INV_PROTO))
                xtables_error(PARAMETER_PROBLEM,
        ...
        case 'i':
            ...
            set_option(&cs.options, OPT_VIANAMEIN, &cs.fw.ip.invflags,
                       cs.invert);
            xtables_parse_interface(optarg, cs.fw.ip.iniface,
        ...
        case 'o':
            ...
            set_option(&cs.options, OPT_VIANAMEOUT, &cs.fw.ip.invflags,
                       cs.invert);
            xtables_parse_interface(optarg, cs.fw.ip.outiface,
        ...
        case 'v':
            if (!verbose)
                set_option(&cs.options, OPT_VERBOSE, &cs.fw.ip.invflags,
                           cs.invert);
            verbose++;
            break;
        ...
        case 'n':
            set_option(&cs.options, OPT_NUMERIC, &cs.fw.ip.invflags, cs.invert);
            break;
    ...
    if (!*handle)
        xtables_error(VERSION_PROBLEM,
                      "can't initialize iptables table `%s': %s", *table,
                      iptc_strerror(errno));
    ...
    if (command == CMD_APPEND || command == CMD_DELETE ||
        ...
        /* If they didn't specify a target, or it's a chain
           name, use standard. */
        if (!cs.target &&
            (strlen(cs.jumpto) == 0 || iptc_is_chain(cs.jumpto, *handle))) {
            size_t size;
 ...
 */
    ...
    case CMD_SET_POLICY:
        ret = iptc_set_policy(
            chain, policy, cs.options & OPT_COUNTERS ? &cs.fw.counters : NULL,
            *handle);
        break;
    default:
