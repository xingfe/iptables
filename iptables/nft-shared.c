...
void add_addr(struct nftnl_rule *r, int offset, void *data, void *mask,
...
void add_proto(struct nftnl_rule *r, int offset, size_t len, uint8_t proto,
               uint32_t op) {
    add_payload(r, offset, len, NFT_PAYLOAD_NETWORK_HEADER);
...
void nft_rule_to_iptables_command_state(struct nftnl_rule *r,
    ...
    if (nftnl_rule_is_set(r, NFTNL_RULE_USERDATA)) {
        ...
        match = xtables_find_match("comment", XTF_TRY_LOAD, &cs->matches);
        if (match == NULL)
            return;
...
void print_firewall_details(const struct iptables_command_state *cs,
                            ...
                            uint8_t invflags, uint8_t proto, unsigned int num,
                            unsigned int format) {
    if (format & FMT_LINENUMBERS)
        printf(FMT("%-4u ", "%u "), num);
...
static void print_iface(char letter, const char *iface,
                        const unsigned char *mask, int inv) {
    unsigned int i;
...
void save_firewall_details(const struct iptables_command_state *cs,
    ...
    if (iniface != NULL) {
        print_iface('i', iniface, iniface_mask, invflags & IPT_INV_VIA_IN);
    }
    if (outiface != NULL) {
        print_iface('o', outiface, outiface_mask, invflags & IPT_INV_VIA_OUT);
    }
...
void save_counters(uint64_t pcnt, uint64_t bcnt) {
    printf("[%llu:%llu] ", (unsigned long long)pcnt, (unsigned long long)bcnt);
}

void save_matches_and_target(struct xtables_rule_match *m,
                             struct xtables_target *target, const char *jumpto,
                             uint8_t flags, const void *fw) {
    struct xtables_rule_match *matchp;

    for (matchp = m; matchp; matchp = matchp->next) {
        if (matchp->match->alias) {
            printf("-m %s", matchp->match->alias(matchp->match->m));
        } else
...
bool compare_matches(struct xtables_rule_match *mt1,
    ...
    for (mp1 = mt1, mp2 = mt2; mp1 && mp2; mp1 = mp1->next, mp2 = mp2->next) {
        ...
        if (memcmp(m1->data, m2->data, mp1->match->userspacesize) != 0) {
