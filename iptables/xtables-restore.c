...
static void print_usage(const char *name, const char *version) {
    fprintf(
        stderr,
        "Usage: %s [-c] [-v] [-t] [-h] [-n] [-T table] [-M command] [-4] [-6]\n"
        "	   [ --counters ]\n"
        "	   [ --verbose ]\n"
        "	   [ --test ]\n"
        "	   [ --help ]\n"
...
void xtables_restore_parse(struct nft_handle *h, struct nft_xt_restore_parse *p,
                           struct nft_xt_restore_cb *cb, int argc,
                           char *argv[]) {
    char buffer[10240];
    int in_table = 0;
            ...
            if (strcmp(policy, "-") != 0) {
                ...
                if (cb->chain_set &&
                    ...
                    xtables_error(OTHER_PROBLEM,
                                  "Can't set policy `%s'"
                                  " on `%s' line %u: %s\n",
                                  policy, chain, line, ops->strerror(errno));
                }
...
static int xtables_restore_main(int family, const char *progname, int argc,
                                char *argv[]) {
    struct nft_handle h = {
        .family = family, .restore = true,
    };
    int c;
    ...
    while ((c = getopt_long(argc, argv, "bcvthnM:T:46", options, NULL)) != -1) {
        ...
        case 't':
            ...
            break;
        case 'h':
            print_usage("xtables-restore", IPTABLES_VERSION);
            exit(0);
        case 'n':
            noflush = 1;
            break;
        ...
        case '6':
            ...
            break;
        default:
            fprintf(stderr, "Try `xtables-restore -h' for more information.\n");
            exit(1);
        }
    }
...
int xtables_ip4_restore_main(int argc, char *argv[]) {
    return xtables_restore_main(NFPROTO_IPV4, "iptables-restore", argc, argv);
}

int xtables_ip6_restore_main(int argc, char *argv[]) {
    return xtables_restore_main(NFPROTO_IPV6, "ip6tables-restore", argc, argv);
}
