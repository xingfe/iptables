...
int iptables_restore_main(int argc, char *argv[]) {
    ...
    while ((c = getopt_long(argc, argv, "bcvVthnwWM:T:", options, NULL)) !=
        ...
        case 't':
            ...
            break;
        case 'h':
            print_usage("iptables-restore", IPTABLES_VERSION);
            exit(0);
        case 'n':
            noflush = 1;
            break;
    ...
    while (fgets(buffer, sizeof(buffer), in)) {
        ...
        if (buffer[0] == '\n')
            ...
            if (noflush == 0) {
                ...
                for_each_chain4(flush_entries4, verbose, 1, handle);

                DEBUGP("Deleting all user-defined chains "
                       "of table '%s'\n",
                       table);
                for_each_chain4(delete_chain4, verbose, 0, handle);
            }
            ...
            ret = do_command4(newargc, newargv, &newargv[2], &handle, true);
