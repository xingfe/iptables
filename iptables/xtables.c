...
static int add_entry(const char *chain, const char *table,
                     struct iptables_command_state *cs, int rulenum, int family,
                     const struct addr_mask s, const struct addr_mask d,
        ...
        if (family == AF_INET) {
            ...
            for (j = 0; j < d.naddrs; j++) {
                ...
                if (append) {
                    ...
                    ret =
                        nft_rule_insert(h, chain, table, cs, rulenum, verbose);
                }
            }
        } else if (family == AF_INET6) {
            memcpy(&cs->fw6.ipv6.src, &s.addr.v6[i], sizeof(struct in6_addr));
            memcpy(&cs->fw6.ipv6.smsk, &s.mask.v6[i], sizeof(struct in6_addr));
            for (j = 0; j < d.naddrs; j++) {
                memcpy(&cs->fw6.ipv6.dst, &d.addr.v6[j],
                       sizeof(struct in6_addr));
                memcpy(&cs->fw6.ipv6.dmsk, &d.mask.v6[j],
                       sizeof(struct in6_addr));
                if (append) {
                    ret = nft_rule_append(h, chain, table, cs, 0, verbose);
                } else {
...
static int replace_entry(const char *chain, const char *table,
                         ...
                         unsigned int rulenum, int family,
                         const struct addr_mask s, const struct addr_mask d,
...
static int delete_entry(const char *chain, const char *table,
                        struct iptables_command_state *cs, int family,
                        const struct addr_mask s, const struct addr_mask d,
                        bool verbose, struct nft_handle *h) {
    unsigned int i, j;
    int ret = 1;
...
static int check_entry(const char *chain, const char *table,
                       struct iptables_command_state *cs, int family,
                       const struct addr_mask s, const struct addr_mask d,
                       bool verbose, struct nft_handle *h) {
    unsigned int i, j;
...
static int list_rules(struct nft_handle *h, const char *chain,
...
void do_parse(struct nft_handle *h, int argc, char *argv[],
    ...
    while ((cs->c = getopt_long(argc, argv,
                                "-:A:C:D:R:I:L::S::M:F::Z::N:X::E:P:Vh::o:p:s:"
                                "d:j:i:fbvw::W::nt:m:xc:g:46",
                                opts, NULL)) != -1) {
        switch (cs->c) {
        /*
         * Command selection
         */
        case 'A':
            add_command(&p->command, CMD_APPEND, CMD_NONE, cs->invert);
            p->chain = optarg;
            break;

        case 'C':
            add_command(&p->command, CMD_CHECK, CMD_NONE, cs->invert);
            p->chain = optarg;
            break;

        case 'D':
            add_command(&p->command, CMD_DELETE, CMD_NONE, cs->invert);
            p->chain = optarg;
        ...
        case 'R':
            add_command(&p->command, CMD_REPLACE, CMD_NONE, cs->invert);
            p->chain = optarg;
            if (xs_has_arg(argc, argv))
                p->rulenum = parse_rulenumber(argv[optind++]);
            else
        ...
        case 'I':
            add_command(&p->command, CMD_INSERT, CMD_NONE, cs->invert);
            p->chain = optarg;
            if (xs_has_arg(argc, argv))
                p->rulenum = parse_rulenumber(argv[optind++]);
            else
        ...
        case 'L':
            add_command(&p->command, CMD_LIST, CMD_ZERO | CMD_ZERO_NUM,
                        cs->invert);
            if (optarg)
                p->chain = optarg;
        ...
        case 'S':
            add_command(&p->command, CMD_LIST_RULES, CMD_ZERO | CMD_ZERO_NUM,
                        cs->invert);
            if (optarg)
                p->chain = optarg;
            else if (xs_has_arg(argc, argv))
        ...
        case 'F':
            add_command(&p->command, CMD_FLUSH, CMD_NONE, cs->invert);
            if (optarg)
                p->chain = optarg;
            else if (xs_has_arg(argc, argv))
                p->chain = argv[optind++];
        ...
        case 'Z':
            add_command(&p->command, CMD_ZERO, CMD_LIST | CMD_LIST_RULES,
                        cs->invert);
            if (optarg)
                p->chain = optarg;
            else if (xs_has_arg(argc, argv))
        ...
        case 'N':
            ...
            if (xtables_find_target(optarg, XTF_TRY_LOAD))
                xtables_error(PARAMETER_PROBLEM, "chain name may not clash "
                                                 "with target name\n");
            add_command(&p->command, CMD_NEW_CHAIN, CMD_NONE, cs->invert);
            p->chain = optarg;
            break;

        case 'X':
            add_command(&p->command, CMD_DELETE_CHAIN, CMD_NONE, cs->invert);
            if (optarg)
                p->chain = optarg;
            else if (xs_has_arg(argc, argv))
                p->chain = argv[optind++];
        ...
        case 'E':
            add_command(&p->command, CMD_RENAME_CHAIN, CMD_NONE, cs->invert);
            p->chain = optarg;
            if (xs_has_arg(argc, argv))
        ...
        case 'P':
            add_command(&p->command, CMD_SET_POLICY, CMD_NONE, cs->invert);
            p->chain = optarg;
            if (xs_has_arg(argc, argv))
         ...
         */
        case 'p':
            set_option(&cs->options, OPT_PROTOCOL, &args->invflags, cs->invert);
        ...
        case 's':
            set_option(&cs->options, OPT_SOURCE, &args->invflags, cs->invert);
...
#ifdef IPT_F_GOTO
        case 'g':
            set_option(&cs->options, OPT_JUMP, &args->invflags, cs->invert);
            args->goto_set = true;
            cs->jumpto = parse_target(optarg);
            break;
#endif
        ...
        case 'i':
            ...
            set_option(&cs->options, OPT_VIANAMEIN, &args->invflags,
                       cs->invert);
            xtables_parse_interface(optarg, args->iniface, args->iniface_mask);
            break;
        ...
        case 'o':
            ...
            set_option(&cs->options, OPT_VIANAMEOUT, &args->invflags,
                       cs->invert);
            xtables_parse_interface(optarg, args->outiface,
                                    args->outiface_mask);
            break;
        ...
        case 'f':
            ...
            set_option(&cs->options, OPT_FRAGMENT, &args->invflags, cs->invert);
            args->flags |= IPT_F_FRAG;
            break;

        case 'v':
            if (!p->verbose)
                set_option(&cs->options, OPT_VERBOSE, &args->invflags,
                           cs->invert);
            p->verbose++;
            break;
        ...
        case 'n':
            set_option(&cs->options, OPT_NUMERIC, &args->invflags, cs->invert);
            break;
        ...
        case 'x':
            set_option(&cs->options, OPT_EXPANDED, &args->invflags, cs->invert);
            break;
        ...
        case 'c':
            set_option(&cs->options, OPT_COUNTERS, &args->invflags, cs->invert);
            args->pcnt = optarg;
    ...
    if (p->command == CMD_APPEND || p->command == CMD_DELETE ||
        p->command == CMD_CHECK || p->command == CMD_INSERT ||
        ...
        if (strcmp(p->chain, "PREROUTING") == 0 ||
            ...
            if (cs->options & OPT_VIANAMEOUT)
                xtables_error(PARAMETER_PROBLEM, "Can't use -%c with %s\n",
                              opt2char(OPT_VIANAMEOUT), p->chain);
        }
        ...
        if (strcmp(p->chain, "POSTROUTING") == 0 ||
            ...
            if (cs->options & OPT_VIANAMEIN)
                xtables_error(PARAMETER_PROBLEM, "Can't use -%c with %s\n",
                              opt2char(OPT_VIANAMEIN), p->chain);
        }
...
int do_commandx(struct nft_handle *h, int argc, char *argv[], char **table,
    ...
    switch (p.command) {
    case CMD_APPEND:
        ret = add_entry(p.chain, p.table, &cs, 0, h->family, args.s, args.d,
                        cs.options & OPT_VERBOSE, h, true);
        break;
    case CMD_DELETE:
        ret = delete_entry(p.chain, p.table, &cs, h->family, args.s, args.d,
                           cs.options & OPT_VERBOSE, h);
        break;
    case CMD_DELETE_NUM:
        ret =
            nft_rule_delete_num(h, p.chain, p.table, p.rulenum - 1, p.verbose);
        break;
    case CMD_CHECK:
        ret = check_entry(p.chain, p.table, &cs, h->family, args.s, args.d,
                          cs.options & OPT_VERBOSE, h);
        break;
    case CMD_REPLACE:
        ret = replace_entry(p.chain, p.table, &cs, p.rulenum - 1, h->family,
                            args.s, args.d, cs.options & OPT_VERBOSE, h);
        break;
    case CMD_INSERT:
        ret = add_entry(p.chain, p.table, &cs, p.rulenum - 1, h->family, args.s,
                        args.d, cs.options & OPT_VERBOSE, h, false);
        break;
    case CMD_FLUSH:
    ...
    case CMD_ZERO:
        ...
        break;
    case CMD_ZERO_NUM:
        ret = nft_rule_zero_counters(h, p.chain, p.table, p.rulenum - 1);
        break;
    case CMD_LIST:
    case CMD_LIST | CMD_ZERO:
    case CMD_LIST | CMD_ZERO_NUM:
        if (nft_is_ruleset_compatible(h) == 1) {
            printf("ERROR: You're using nft features that cannot be mapped to "
                   "iptables, please keep using nft.\n");
            exit(EXIT_FAILURE);
        }

        ret = list_entries(h, p.chain, p.table, p.rulenum,
                           cs.options & OPT_VERBOSE, cs.options & OPT_NUMERIC,
