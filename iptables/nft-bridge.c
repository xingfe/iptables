...
static void ebt_print_mac_and_mask(const unsigned char *mac,
...
static void add_logical_iniface(struct nftnl_rule *r, char *iface,
                                uint32_t op) {
    int iface_len;
...
static void add_logical_outiface(struct nftnl_rule *r, char *iface,
                                 uint32_t op) {
    int iface_len;
 ...
 */
static int _add_action(struct nftnl_rule *r,
...
static void nft_bridge_parse_meta(struct nft_xt_ctx *ctx, struct nftnl_expr *e,
                                  void *data) {
    struct ebtables_command_state *cs = data;
    struct ebt_entry *fw = &cs->fw;
    uint8_t flags = 0;
    ...
    iface = parse_meta(e, ctx->meta.key, fw->in, fw->in_mask, fw->out,
                       fw->out_mask, &flags);
    if (!iface)
        goto out;
...
static void nft_bridge_parse_target(struct xtables_target *t, void *data) {
    ...
    /* harcoded names :-( */
    if (strcmp(t->name, "log") == 0 || strcmp(t->name, "nflog") == 0) {
    ...
    for (m = cs.match_list; m; m = m->next) {
        if (m->ismatch) {
            ...
            if (matchp->print != NULL) {
                matchp->print(&cs.fw, matchp->m, format & FMT_NUMERIC);
            }
        } else {
            watcherp = m->u.watcher;
            if (watcherp->print != NULL) {
                watcherp->print(&cs.fw, watcherp->t, format & FMT_NUMERIC);
            }
        }
    }
...
static bool nft_bridge_is_same(const void *data_a, const void *data_b) {
    ...
    return is_same_interfaces((char *)a->in, (char *)a->out, a->in_mask,
                              a->out_mask, (char *)b->in, (char *)b->out,
                              b->in_mask, b->out_mask);
}

static bool nft_bridge_rule_find(struct nft_family_ops *ops,
