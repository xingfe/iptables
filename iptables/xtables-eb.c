...
unsigned char eb_msk_type_bridge_group[ETH_ALEN] = {255, 255, 255,
                                                    255, 255, 255};
...
static int ebt_check_inverse2(const char option[], int argc, char **argv) {
    ...
    if (strcmp(option, "!") == 0) {
        if (ebt_invert == 1)
            xtables_error(PARAMETER_PROBLEM, "Double use of '!' not allowed");
        if (optind >= argc)
            optarg = NULL;
        else
            optarg = argv[optind];
...
static const char *parse_target(const char *targetname) {
    ...
    if (strlen(targetname) + 1 > EBT_CHAIN_MAXNAMELEN)
        xtables_error(PARAMETER_PROBLEM, "Invalid target '%s' (%d chars max)",
...
static int append_entry(struct nft_handle *h, const char *chain,
                        const char *table, struct ebtables_command_state *cs,
                        int rule_nr, bool verbose, bool append) {
    int ret = 1;
...
static int delete_entry(struct nft_handle *h, const char *chain,
                        const char *table, struct ebtables_command_state *cs,
                        int rule_nr, int rule_nr_end, bool verbose) {
    int ret = 1;
    ...
    if (rule_nr == -1)
        ...
        do {
            ret = nft_rule_delete_num(h, chain, table, rule_nr, verbose);
            rule_nr++;
...
static void print_help(const struct xtables_target *t,
    ...
    printf("Usage:\n"
           ...
           "--delete -D chain rulenum     : delete rule at position rulenum "
           "from chain\n"
           ...
           "chain\n"
           "--list   -L [chain]           : list the rules in a chain or in "
           "all chains\n"
           "--flush  -F [chain]           : delete all rules in chain or in "
           "all chains\n"
           "--init-table                  : replace the kernel table with the "
           "initial table\n"
           "--zero   -Z [chain]           : put counters on zero in chain or "
           "in all chains\n"
           "--policy -P chain target      : change policy on chain to target\n"
           "--new-chain -N chain          : create a user defined chain\n"
           "--rename-chain -E old new     : rename a chain\n"
           "--delete-chain -X [chain]     : delete a user defined chain\n"
           "--atomic-commit               : update the kernel w/t table "
           "contained in <FILE>\n"
           ...
           "          pcnt bcnt           : set the counters of the to be "
           "added rule\n"
           "--modprobe -M program         : try to insert modules using this "
           "program\n"
           "--concurrent                  : use a file lock to support "
           "concurrent scripts\n"
...
static int list_rules(struct nft_handle *h, const char *chain,
                      const char *table, int rule_nr, int verbose, int numeric,
                      int expanded, int linenumbers, int counters) {
    unsigned int format;
...
static int parse_change_counters_rule(int argc, char **argv, int *rule_nr,
                                      int *rule_nr_end, int exec_style,
    ...
    if (optind + 2 < argc &&
        (argv[optind + 2][0] != '-' ||
         (argv[optind + 2][1] >= '0' && argv[optind + 2][1] <= '9'))) {
        if (optind + 3 != argc)
            xtables_error(
                PARAMETER_PROBLEM,
    ...
    return ret;
invalid:
    xtables_error(PARAMETER_PROBLEM, "Packet counter '%s' invalid",
                  argv[optind]);
}
...
static void ebt_add_match(struct xtables_match *m,
    ...
    if (newm == NULL)
        xtables_error(OTHER_PROBLEM, "Unable to add match %s", m->name);
...
int do_commandeb(struct nft_handle *h, int argc, char *argv[], char **table) {
    ...
    while ((c = getopt_long(argc, argv,
        ...
        case 'X': /* Delete chain */
            ...
            if (c == 'E') {
                if (optind >= argc)
                    ...
                    xtables_error(PARAMETER_PROBLEM,
                                  "No extra options allowed with -E");
                ...
                ret = nft_chain_user_rename(h, chain, *table, argv[optind]);
                if (ret != 0 && errno == ENOENT)
                    xtables_error(PARAMETER_PROBLEM,
                                  "Chain '%s' doesn't exists", chain);
                ...
                if ((chcounter = parse_change_counters_rule(
                         argc, argv, &rule_nr, &rule_nr_end, exec_style,
                         &cs)) == -1)
                    return -1;
            } else if (c == 'I') {
                if (optind >= argc ||
            ...
            handle_P:
                ...
                for (i = 0; i < NUM_STANDARD_TARGETS; i++)
                    if (!strcmp(argv[optind], nft_ebt_standard_target(i))) {
                        ...
                        if (-i - 1 == EBT_CONTINUE)
                            xtables_error(PARAMETER_PROBLEM,
                                          "Wrong policy '%s'", argv[optind]);
                        break;
                    }
                if (i == NUM_STANDARD_TARGETS)
                    xtables_error(PARAMETER_PROBLEM, "Unknown policy '%s'",
                                  argv[optind]);
                optind++;
            }
            break;
        ...
        case 'h': /* Help */
            ...
            while (optind < argc) {
                ...
                if (!(t = xtables_find_target(argv[optind], XTF_TRY_LOAD)))
                    xtables_error(PARAMETER_PROBLEM, "Extension '%s' not found",
                                  argv[optind]);
                if (flags & OPT_JUMP)
                    xtables_error(PARAMETER_PROBLEM, "Sorry, you can only see "
                                                     "help for one target "
                                                     "extension at a time");
                flags |= OPT_JUMP;
                cs.target = t;
        ...
        case 'c': /* Set counters */
            ...
            if (c == 'i') {
                ...
                if (selected_chain > 2 && selected_chain < NF_BR_BROUTING)
                    xtables_error(PARAMETER_PROBLEM, "Use -i only in INPUT, "
                                                     "FORWARD, PREROUTING and "
                                                     "BROUTING chains");
                ...
                if (selected_chain > 2 && selected_chain < NF_BR_BROUTING)
                    xtables_error(PARAMETER_PROBLEM,
                                  "Use --logical-in only in INPUT, FORWARD, "
                ...
                if (selected_chain < 2 || selected_chain == NF_BR_BROUTING)
                    xtables_error(PARAMETER_PROBLEM, "Use -o only in OUTPUT, "
                ...
                if (selected_chain < 2 || selected_chain == NF_BR_BROUTING)
                    xtables_error(PARAMETER_PROBLEM, "Use --logical-out only "
                ...
                if (ebt_get_mac_and_mask(optarg, cs.fw.sourcemac,
                                         cs.fw.sourcemsk))
                    xtables_error(PARAMETER_PROBLEM,
                ...
                if (ebt_check_inverse2(optarg, argc, argv))
                    xtables_error(PARAMETER_PROBLEM, "Unexpected '!' after -c");
                ...
                if (*buffer != '\0')
                    xtables_error(PARAMETER_PROBLEM,
                                  "Packet counter '%s' invalid", optarg);
                cs.counters.bcnt = strtoull(argv[optind], &buffer, 10);
                if (*buffer != '\0')
                    xtables_error(PARAMETER_PROBLEM,
            ...
            if (*buffer != '\0') {
                ...
                if (!ent)
                    xtables_error(
                        PARAMETER_PROBLEM,
                        "Problem with the specified Ethernet protocol '%s', perhaps "_PATH_ETHERTYPES
                        " is missing",
                        optarg);
        ...
        case 4: /* Lc */
            ...
            if (command != 'L')
                xtables_error(PARAMETER_PROBLEM, "Use --Lc with -L");
            flags |= LIST_C;
            break;
        case 5: /* Ln */
#ifdef SILENT_DAEMON
            ...
            if (command != 'L')
                xtables_error(PARAMETER_PROBLEM, "Use --Ln with -L");
            if (flags & LIST_X)
                xtables_error(PARAMETER_PROBLEM,
        ...
        case 6: /* Lx */
            ...
            if (command != 'L')
                xtables_error(PARAMETER_PROBLEM, "Use --Lx with -L");
            if (flags & LIST_N)
                xtables_error(PARAMETER_PROBLEM,
                              "--Lx is not compatible with --Ln");
            flags |= LIST_X;
        ...
        case 12: /* Lmac2 */
            ...
            if (command != 'L')
                xtables_error(PARAMETER_PROBLEM, "Use --Lmac2 with -L");
            flags |= LIST_MAC2;
            break;
        case 8: /* atomic-commit */
                /*			if (exec_style == EXEC_STYLE_DAEMON)
                                                ebt_print_error2("--atomic-commit is not
                   supported in daemon mode");         replace->command = c;         if
                   (OPT_COMMANDS)         ebt_print_error2("Multiple commands are         not allowed");
                   replace->flags |= OPT_COMMAND;         if (!replace->filename)
        ...
        /*	if (exec_style == EXEC_STYLE_DAEMON) {
                        if (c == 7) {
                                ebt_print_error2("--atomic-init is not supported
           in daemon mode"); } else if (c == 10) ebt_print_error2("--atomic-save
           is not supported in daemon mode"); ebt_print_error2("--init-table is
        ...
        case 1:
            if (!strcmp(optarg, "!"))
                ...
                xtables_error(PARAMETER_PROBLEM, "Bad argument : '%s'", optarg);
        ...
        default:
            ...
            if (cs.target != NULL && cs.target->parse != NULL) {
                ...
                if (cs.target->parse(c - opt_offset, argv, ebt_invert,
                                     &cs.target->tflags, NULL, &cs.target->t))
                    goto check_extension;
            }

            /* Is it a match_option? */
            for (m = xtables_matches; m; m = m->next) {
                if (m->parse(c - m->option_offset, argv, ebt_invert, &m->mflags,
                             NULL, &m->m)) {
                    ebt_add_match(m, &cs);
                    goto check_extension;
                }
            ...
            for (w = xtables_targets; w; w = w->next) {
                if (w->parse(c - w->option_offset, argv, ebt_invert, &w->tflags,
                             NULL, &w->t)) {
        ...
        /*
        if (w == NULL && c == '?')
                ebt_print_error2("Unknown argument: '%s'", argv[optind - 1],
        (char)optopt, (char)c); else if (w == NULL) { if (!strcmp(t->name,
        "standard")) ebt_print_error2("Unknown argument: don't forget the -t
        option"); else ebt_print_error2("Target-specific option does not
        correspond with specified target");
        }
        if (ebt_errormsg[0] != '\0')
                return -1;
        if (w->used == 0) {
    ...
    /* Do the final checks */
    if (command == 'A' || command == 'I' || command == 'D' || command == 'C') {
    ...
    if (command == 'P') {
        ...
        ret = list_rules(
            h, chain, *table, rule_nr, flags & OPT_VERBOSE, flags & OPT_NUMERIC,
            /*flags&OPT_EXPANDED*/ 0, flags & LIST_N, flags & LIST_C);
    ...
    if (flags & OPT_ZERO) {
        ...
        ret = append_entry(h, chain, *table, &cs, 0, flags & OPT_VERBOSE, true);
    } else if (command == 'I') {
        ret = append_entry(h, chain, *table, &cs, rule_nr - 1,
                           flags & OPT_VERBOSE, false);
    } else if (command == 'D') {
        ret = delete_entry(h, chain, *table, &cs, rule_nr - 1, rule_nr_end,
                           flags & OPT_VERBOSE);
    } /*else if (replace->command == 'C') {
            ebt_change_counters(replace, new_entry, rule_nr, rule_nr_end,
    &(new_entry->cnt_surplus), chcounter); if (ebt_errormsg[0] != '\0') return
    -1;
    }*/
