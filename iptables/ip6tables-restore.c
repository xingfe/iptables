...
static void print_usage(const char *name, const char *version) {
    fprintf(stderr,
            "Usage: %s [-c] [-v] [-V] [-t] [-h] [-n] [-w secs] [-W usecs] [-T "
            "table] [-M command]\n"
            "	   [ --counters ]\n"
            "	   [ --verbose ]\n"
            "	   [ --version]\n"
...
int ip6tables_restore_main(int argc, char *argv[]) {
    ...
    while ((c = getopt_long(argc, argv, "bcvVthnwWM:T:", options, NULL)) !=
        ...
        case 't':
            ...
            break;
        case 'h':
            print_usage("ip6tables-restore", IPTABLES_VERSION);
            exit(0);
        case 'n':
            noflush = 1;
            break;
    ...
    while (fgets(buffer, sizeof(buffer), in)) {
        ...
        if (buffer[0] == '\n')
            ...
            if (noflush == 0) {
                ...
                for_each_chain6(flush_entries6, verbose, 1, handle);

                DEBUGP("Deleting all user-defined chains "
                       "of table '%s'\n",
                       table);
                for_each_chain6(delete_chain6, verbose, 0, handle);
            }
            ...
            if (strcmp(policy, "-") != 0) {
                ...
                if (!ops->set_policy(chain, policy, &count, handle))
            ...
            ret = do_command6(newargc, newargv, &newargv[2], &handle, true);
