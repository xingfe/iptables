 ...
 * based on iptables-restore (C) 2000-2002 by Harald Welte
 * <laforge@gnumonks.org> based on previous code from Rusty Russell
...
static int parse_counters(char *string, struct xt_counters *ctr) {
    ...
    if (string != NULL) {
        pcnt = &ctr->pcnt;
        bcnt = &ctr->bcnt;
        return (sscanf(string, "[%llu:%llu]", (unsigned long long *)pcnt,
...
static void openChain(char *chain, char *policy, struct xt_counters *ctr,
                      char close) {
    closeChain();
...
static void saveChain(char *chain, char *policy, struct xt_counters *ctr) {
...
static int isTarget(char *arg) {
    return ((arg) &&
            (strcmp((arg), "-j") == 0 || strcmp((arg), "--jump") == 0 ||
             strcmp((arg), "-g") == 0 || strcmp((arg), "--goto") == 0));
}

// is it a terminating target like -j ACCEPT, etc
// (or I guess -j SNAT in nat table, but we don't check for that yet
static int isTerminatingTarget(char *arg) {
    return ((arg) &&
            (strcmp((arg), "ACCEPT") == 0 || strcmp((arg), "DROP") == 0 ||
             strcmp((arg), "QUEUE") == 0 || strcmp((arg), "RETURN") == 0));
}

// part=-1 means do conditions, part=1 means do rules, part=0 means do both
static void do_rule_part(char *leveltag1, char *leveltag2, int part, int argc,
...
#define OPEN_LEVEL(LEVEL, TAG)                                                 \
    do {                                                                       \
        ...
        if (leveltag##LEVEL) {                                                 \
            ...
            printf("%s<%s ", (leveli##LEVEL), (level##LEVEL));                 \
    } while (0)
    ...
    while (arg < argc) {
        ...
        if (part == -1 && !argvattr[arg] && (isTarget(argv[arg]))) {
            ...
            spacer = " ";
                   ...
                   existsChain(argv[arg + 1]) && (2 + arg >= argc)) {
            if (!((1 + arg) < argc))
            ...
            if (strcmp(argv[arg], "-g") == 0 ||
                ...
                level2 = NULL;
                   ...
                   (isTarget(argv[arg]) || strcmp(argv[arg], "-m") == 0 ||
            ...
            if ((arg + 1) >= argc ||
                (!argvattr[arg + 1] &&
                 (isTarget(argv[arg + 1]) || strcmp(argv[arg + 1], "-m") == 0 ||
                  strcmp(argv[arg + 1], "--module") == 0))) {
                printf(" />\n");
                level1 = NULL;
            } else {
                printf(">\n");
    ...
    while (new < newargc && old < oldargc) {
        if (isTarget(oldargv[old]) && isTarget(newargv[new])) {
            ...
            if (((strcmp((oldargv[old]), "-j") == 0 ||
                 ...
                 old + 1 < oldargc && isTerminatingTarget(oldargv[old + 1])) ||
...
static void do_rule(char *pcnt, char *bcnt, int argc, char *argv[],
                    int argvattr[]) {
...
int iptables_xml_main(int argc, char *argv[]) {
    ...
    if (optind == argc - 1) {
        ...
        if (!in) {
            fprintf(stderr, "Can't open %s: %s", argv[optind], strerror(errno));
            exit(1);
        }
    } else if (optind < argc) {
    ...
    while (fgets(buffer, sizeof(buffer), in)) {
        ...
        if ((strcmp(buffer, "COMMIT\n") == 0) && (curTable[0])) {
            ...
            if (!policy) {
                xtables_error(PARAMETER_PROBLEM, "%s: line %u policy invalid\n",
                              prog_name, line);
                exit(1);
            }
            ...
            for (curchar = parsestart; *curchar; curchar++) {
                ...
                if (*curchar == ' ' || *curchar == '\t' || *curchar == '\n') {
                    ...
                    *(param_buffer + param_len) = '\0';
                    ...
                    if (newargc >= 2 && 0 == strcmp(newargv[newargc - 2], "-A"))
