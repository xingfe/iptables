...
static int nft_ipv4_add(struct nftnl_rule *r, void *data) {
    ...
    if (cs->fw.ip.proto != 0) {
        ...
        add_proto(r, offsetof(struct iphdr, protocol), 1, cs->fw.ip.proto, op);
    }

    if (cs->fw.ip.src.s_addr != 0) {
        op = nft_invflags2cmp(cs->fw.ip.invflags, IPT_INV_SRCIP);
        add_addr(r, offsetof(struct iphdr, saddr), &cs->fw.ip.src.s_addr,
                 &cs->fw.ip.smsk.s_addr, sizeof(struct in_addr), op);
    }
    if (cs->fw.ip.dst.s_addr != 0) {
        op = nft_invflags2cmp(cs->fw.ip.invflags, IPT_INV_DSTIP);
        add_addr(r, offsetof(struct iphdr, daddr), &cs->fw.ip.dst.s_addr,
                 &cs->fw.ip.dmsk.s_addr, sizeof(struct in_addr), op);
    }
...
static bool nft_ipv4_is_same(const void *data_a, const void *data_b) {
    ...
    if (a->fw.ip.src.s_addr != b->fw.ip.src.s_addr ||
        ...
        a->fw.ip.proto != b->fw.ip.proto || a->fw.ip.flags != b->fw.ip.flags ||
...
static void nft_ipv4_parse_payload(struct nft_xt_ctx *ctx, struct nftnl_expr *e,
                                   void *data) {
    struct iptables_command_state *cs = data;
    struct in_addr addr;
    uint8_t proto;
...
static void nft_ipv4_print_firewall(struct nftnl_rule *r, unsigned int num,
    ...
    print_firewall_details(&cs, cs.jumpto, cs.fw.ip.flags, cs.fw.ip.invflags,
                           cs.fw.ip.proto, num, format);
...
static void nft_ipv4_save_firewall(const void *data, unsigned int format) {
    ...
    save_matches_and_target(cs->matches, cs->target, cs->jumpto,
                            cs->fw.ip.flags, &cs->fw);

    if (cs->target == NULL && strlen(cs->jumpto) > 0) {
        printf("-%c %s", cs->fw.ip.flags & IPT_F_GOTO ? 'g' : 'j', cs->jumpto);
    }
    printf("\n");
}
...
static void nft_ipv4_post_parse(int command, struct iptables_command_state *cs,
                                struct xtables_args *args) {
    ...
    memcpy(cs->fw.ip.iniface_mask, args->iniface_mask,
           IFNAMSIZ * sizeof(unsigned char));

    strncpy(cs->fw.ip.outiface, args->outiface, IFNAMSIZ);
    memcpy(cs->fw.ip.outiface_mask, args->outiface_mask,
           IFNAMSIZ * sizeof(unsigned char));
...
static bool nft_ipv4_rule_find(struct nft_family_ops *ops, struct nftnl_rule *r,
                               void *data) {
    struct iptables_command_state *cs = data;
...
static int nft_ipv4_xlate(const void *data, struct xt_xlate *xl) {
    ...
    if (cs->fw.ip.proto != 0) {
        const struct protoent *pent = getprotobynumber(cs->fw.ip.proto);
        char protonum[strlen("255") + 1];

        if (!xlate_find_match(cs, pent->p_name)) {
            snprintf(protonum, sizeof(protonum), "%u", cs->fw.ip.proto);
