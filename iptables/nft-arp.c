...
char *opcodes[] = {
    ...
    "DRARP_Error",   "InARP_Request", "ARP_NAK",
};
...
static void print_mac_and_mask(const unsigned char *mac,
                               const unsigned char *mask, int l) {
    int i;
...
static int nft_arp_add(struct nftnl_rule *r, void *data) {
    ...
    if (fw->arp.arhln != 0) {
        ...
        add_proto(r, offsetof(struct arphdr, ar_hln), 1, fw->arp.arhln, op);
    }
    ...
    if (fw->arp.src.s_addr != 0) {
        ...
        add_addr(r, sizeof(struct arphdr) + fw->arp.arhln, &fw->arp.src.s_addr,
                 &fw->arp.smsk.s_addr, sizeof(struct in_addr), op);
    }
...
static void nft_arp_parse_meta(struct nft_xt_ctx *ctx, struct nftnl_expr *e,
    ...
    parse_meta(e, ctx->meta.key, fw->arp.iniface, fw->arp.iniface_mask,
               fw->arp.outiface, fw->arp.outiface_mask, &flags);
...
static void nft_arp_parse_payload(struct nft_xt_ctx *ctx, struct nftnl_expr *e,
                                  void *data) {
    struct arptables_command_state *cs = data;
    struct arpt_entry *fw = &cs->fw;
    struct in_addr addr;
    ...
    default:
        ...
        if (ctx->payload.offset == sizeof(struct arphdr) + fw->arp.arhln) {
...
static void nft_arp_print_firewall(struct nftnl_rule *r, unsigned int num,
                                   unsigned int format) {
    struct arptables_command_state cs = {};
...
static bool nft_arp_is_same(const void *data_a, const void *data_b) {
    ...
    if (a->arp.src.s_addr != b->arp.src.s_addr ||
        ...
        a->arp.arpro != b->arp.arpro || a->arp.flags != b->arp.flags ||
    ...
    return is_same_interfaces(
        a->arp.iniface, a->arp.outiface, (unsigned char *)a->arp.iniface_mask,
        (unsigned char *)a->arp.outiface_mask, b->arp.iniface, b->arp.outiface,
