...
static void NFQUEUE_help_v1(void) {
    printf("NFQUEUE target options\n"
           ...
           "<value>.\n"
           "                               Valid queue numbers are 0-65535\n"
           "  --queue-balance first:last	Balance flows between queues "
           "<value> to <value>.\n");
}
...
static void NFQUEUE_help_v2(void) {
    printf(
        ...
        "  --queue-num value            Send packet to QUEUE number <value>.\n"
        "                               Valid queue numbers are 0-65535\n"
        "  --queue-balance first:last   Balance flows between queues <value> "
        "to <value>.\n"
        "  --queue-bypass		Bypass Queueing if no queue instance "
        "exists.\n"
...
static const struct xt_option_entry NFQUEUE_opts[] = {
    ...
    {.name = "queue-cpu-fanout",
     .id = O_QUEUE_CPU_FANOUT,
     .type = XTTYPE_NONE,
     .also = F_QUEUE_BALANCE},
    XTOPT_TABLEEND,
};
#undef s
...
static void NFQUEUE_parse_v1(struct xt_option_call *cb) {
    ...
    case O_QUEUE_BALANCE:
        if (cb->nvals != 2)
            xtables_error(PARAMETER_PROBLEM, "Bad range \"%s\"", cb->arg);
        if (r[0] >= r[1])
            xtables_error(PARAMETER_PROBLEM, "%u should be less than %u", r[0],
                          r[1]);
        info->queuenum = r[0];
...
static void NFQUEUE_print(const void *ip, const struct xt_entry_target *target,
                          int numeric) {
    const struct xt_NFQ_info *tinfo = (const struct xt_NFQ_info *)target->data;
...
static void NFQUEUE_save_v1(const void *ip,
                            const struct xt_entry_target *target) {
    const struct xt_NFQ_info_v1 *tinfo = (const void *)target->data;
    unsigned int last = tinfo->queues_total;
...
static void NFQUEUE_save_v2(const void *ip,
                            const struct xt_entry_target *target) {
    const struct xt_NFQ_info_v2 *info = (void *)target->data;
    unsigned int last = info->queues_total;
