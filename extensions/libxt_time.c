...
static const struct xt_option_entry time_opts[] = {
    ...
    {.name = "kerneltz", .id = O_KERNEL_TZ, .type = XTTYPE_NONE, .excl = F_UTC},
    XTOPT_TABLEEND,
};
...
static void time_help(void) {
    printf(
        "time match options:\n"
        "    --datestart time     Start and stop time, to be given in ISO "
        "8601\n"
        ...
        "comma\n"
        "                         (Possible days: Mon,Tue,Wed,Thu,Fri,Sat,Sun "
        "or 1 to 7\n"
        "                         Defaults to all weekdays.)\n"
        "    --kerneltz           Work with the kernel timezone instead of "
        "UTC\n");
}
...
out:
    xtables_error(PARAMETER_PROBLEM,
                  "Invalid date \"%s\" specified. Should "
...
out:
    xtables_error(PARAMETER_PROBLEM,
                  "invalid time \"%s\" specified, "
                  "should be hh:mm[:ss] format and within the boundaries",
                  s);
    return -1;
}

static const char *my_strseg(char *buf, unsigned int buflen, const char **arg,
                             char delim) {
    const char *sep;
...
static unsigned int time_parse_weekdays(const char *arg) {
    ...
    while (my_strseg(day, sizeof(day), &arg, ',') != NULL) {
        ...
        if (!valid)
            xtables_error(PARAMETER_PROBLEM, "%s is not a valid day specifier",
                          day);
    }
...
static void time_print_date(time_t date, const char *command) {
    ...
    if (command != NULL)
         ...
         */
        printf(" %s %04u-%02u-%02uT%02u:%02u:%02u", command, t->tm_year + 1900,
               t->tm_mon + 1, t->tm_mday, t->tm_hour, t->tm_min, t->tm_sec);
    else
        printf(" %04u-%02u-%02u %02u:%02u:%02u", t->tm_year + 1900,
               t->tm_mon + 1, t->tm_mday, t->tm_hour, t->tm_min, t->tm_sec);
}
...
static void time_check(struct xt_fcheck_call *cb) {
    ...
    if ((cb->xflags & F_TIME_CONTIGUOUS) &&
        ...
        xtables_error(PARAMETER_PROBLEM, "time: --contiguous only makes sense "
                                         "when stoptime is smaller than "
                                         "starttime");
}
...
void _init(void) { xtables_register_match(&time_match); }
