...
static void rpfilter_help(void) {
    printf("rpfilter match options:\n"
           "    --loose          permit reverse path via any interface\n"
           "    --validmark      use skb nfmark when performing route lookup\n"
           "    --accept-local   do not reject packets with a local source "
           "address\n"
           "    --invert         match packets that failed the reverse path "
           "test\n");
}
...
static void rpfilter_print_prefix(const void *ip, const void *matchinfo,
                                  const char *prefix) {
...
static void rpfilter_print(const void *ip, const struct xt_entry_match *match,
                           int numeric) {
    printf(" rpfilter");
