...
static void iprange_mt_help(void) {
    printf("iprange match options:\n"
           "[!] --src-range ip[-ip]    Match source IP in the specified range\n"
           "[!] --dst-range ip[-ip]    Match destination IP in the specified "
           "range\n");
}
...
static void iprange_parse_spec(const char *from, const char *to,
                               union nf_inet_addr *range, uint8_t family,
                               const char *optname) {
    ...
    if (family == NFPROTO_IPV6) {
        for (i = 0; i < ARRAY_SIZE(spec); ++i) {
            ...
            if (ia6 == NULL)
                xtables_param_act(XTF_BAD_VALUE, "iprange", optname, spec[i]);
            range[i].in6 = *ia6;
        }
    } else {
        for (i = 0; i < ARRAY_SIZE(spec); ++i) {
...
static void iprange_parse_range(const char *oarg, union nf_inet_addr *range,
    ...
    *dash = '\0';
    ...
    if (memcmp(&range[0], &range[1], sizeof(*range)) > 0)
        fprintf(stderr,
                "xt_iprange: range %s-%s is reversed and "
                "will never match\n",
                arg, dash + 1);
    free(arg);
}
...
static void iprange_mt_parse(struct xt_option_call *cb, uint8_t nfproto) {
    ...
    switch (cb->entry->id) {
    case O_SRC_RANGE:
        iprange_parse_range(cb->arg, &info->src_min, nfproto, "--src-range");
        info->flags |= IPRANGE_SRC;
        if (cb->invert)
            info->flags |= IPRANGE_SRC_INV;
        break;
    case O_DST_RANGE:
        iprange_parse_range(cb->arg, &info->dst_min, nfproto, "--dst-range");
        info->flags |= IPRANGE_DST;
        if (cb->invert)
            info->flags |= IPRANGE_DST_INV;
        break;
...
static void print_iprange(const struct ipt_iprange *range) {
    const unsigned char *byte_min, *byte_max;

    byte_min = (const unsigned char *)&range->min_ip;
    byte_max = (const unsigned char *)&range->max_ip;
    printf(" %u.%u.%u.%u-%u.%u.%u.%u", byte_min[0], byte_min[1], byte_min[2],
           byte_min[3], byte_max[0], byte_max[1], byte_max[2], byte_max[3]);
}
...
static void iprange_mt4_print(const void *ip,
...
static void iprange_mt6_print(const void *ip,
                              const struct xt_entry_match *match, int numeric) {
    const struct xt_iprange_mtinfo *info = (const void *)match->data;
...
static void iprange_mt4_save(const void *ip,
...
static void iprange_mt6_save(const void *ip,
                             const struct xt_entry_match *match) {
    const struct xt_iprange_mtinfo *info = (const void *)match->data;
...
static void print_iprange_xlate(const struct ipt_iprange *range,
                                struct xt_xlate *xl) {
    const unsigned char *byte_min, *byte_max;

    byte_min = (const unsigned char *)&range->min_ip;
    byte_max = (const unsigned char *)&range->max_ip;
    xt_xlate_add(xl, " %u.%u.%u.%u-%u.%u.%u.%u ", byte_min[0], byte_min[1],
                 byte_min[2], byte_min[3], byte_max[0], byte_max[1],
                 byte_max[2], byte_max[3]);
}
...
static int iprange_mt4_xlate(struct xt_xlate *xl,
                             const struct xt_xlate_mt_params *params) {
    const struct xt_iprange_mtinfo *info = (const void *)params->match->data;
    char *space = "";
    ...
    if (info->flags & IPRANGE_SRC) {
        ...
        xt_xlate_add(xl, "-%s", xtables_ipaddr_to_numeric(&info->src_max.in));
        space = " ";
    }
    if (info->flags & IPRANGE_DST) {
        ...
        xt_xlate_add(xl, "-%s", xtables_ipaddr_to_numeric(&info->dst_max.in));
    }
...
static int iprange_mt6_xlate(struct xt_xlate *xl,
    ...
    if (info->flags & IPRANGE_SRC) {
        xt_xlate_add(xl, "ip6 saddr%s %s",
                     info->flags & IPRANGE_SRC_INV ? " !=" : "",
                     xtables_ip6addr_to_numeric(&info->src_min.in6));
        xt_xlate_add(xl, "-%s", xtables_ip6addr_to_numeric(&info->src_max.in6));
        space = " ";
    }
    if (info->flags & IPRANGE_DST) {
        xt_xlate_add(xl, "%sip6 daddr%s %s", space,
                     info->flags & IPRANGE_DST_INV ? " !=" : "",
                     xtables_ip6addr_to_numeric(&info->dst_min.in6));
        xt_xlate_add(xl, "-%s", xtables_ip6addr_to_numeric(&info->dst_max.in6));
    }
