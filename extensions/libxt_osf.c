...
static void osf_help(void) {
    printf(
        "OS fingerprint match options:\n"
        "[!] --genre string     Match a OS genre by passive fingerprinting.\n"
        "--ttl level            Use some TTL check extensions to determine "
        "OS:\n"
        "       0                       true ip and fingerprint TTL "
        "comparison. Works for LAN.\n"
        "       1                       check if ip TTL is less than "
        "fingerprint one. Works for global addresses.\n"
        "       2                       do not compare TTL at all. Allows to "
        "detect NMAP, but can produce false results.\n"
        "--log level            Log determined genres into dmesg even if they "
        "do not match desired one:\n"
        "       0                       log all matched or unknown "
        "signatures.\n"
...
static void osf_print(const void *ip, const struct xt_entry_match *match,
                      int numeric) {
    const struct xt_osf_info *info = (const struct xt_osf_info *)match->data;

    printf(" OS fingerprint match %s%s",
           (info->flags & XT_OSF_INVERT) ? "! " : "", info->genre);
}
...
void _init(void) { xtables_register_match(&osf_match); }
