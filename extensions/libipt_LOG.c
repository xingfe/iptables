...
static void LOG_help(void) {
    printf("LOG target options:\n"
           ...
           " --log-macdecode		Decode MAC addresses and "
           "protocol.\n\n");
}
...
static const struct xt_option_entry LOG_opts[] = {
    {.name = "log-level",
     .id = O_LOG_LEVEL,
     .type = XTTYPE_SYSLOGLEVEL,
     .flags = XTOPT_PUT,
     XTOPT_POINTER(s, level)},
    {.name = "log-prefix",
     .id = O_LOG_PREFIX,
     .type = XTTYPE_STRING,
     .flags = XTOPT_PUT,
     XTOPT_POINTER(s, prefix),
     .min = 1},
...
static void LOG_print(const void *ip, const struct xt_entry_target *target,
    ...
    printf(" LOG");
    if (numeric)
        printf(" flags %u level %u", loginfo->logflags, loginfo->level);
    else {
...
static const struct ipt_log_xlate ipt_log_xlate_names[] = {
    ...
    {"emerg", LOG_EMERG},   {"err", LOG_ERR},     {"info", LOG_INFO},
...
static int LOG_xlate(struct xt_xlate *xl,
    ...
    for (i = 0; i < ARRAY_SIZE(ipt_log_xlate_names); ++i)
        if (loginfo->level != LOG_DEFAULT_LEVEL &&
            ...
            xt_xlate_add(xl, " level %s", ipt_log_xlate_names[i].name);
            break;
        }
