...
static void conntrack_mt_help(void) {
    printf(
        ...
        "                               Original/Reply source/destination "
        "address\n"
        "[!] --ctorigsrcport port\n"
        "[!] --ctorigdstport port\n"
        "[!] --ctreplsrcport port\n"
...
static int parse_state(const char *state, size_t len,
                       struct xt_conntrack_info *sinfo) {
...
static void parse_states(const char *arg, struct xt_conntrack_info *sinfo) {
    const char *comma;
...
static bool conntrack_ps_state(struct xt_conntrack_mtinfo3 *info,
                               const char *state, size_t z) {
...
static void conntrack_ps_states(struct xt_conntrack_mtinfo3 *info,
                                const char *arg) {
    const char *comma;
...
static int parse_status(const char *status, size_t len,
                        struct xt_conntrack_info *sinfo) {
...
static void parse_statuses(const char *arg, struct xt_conntrack_info *sinfo) {
    const char *comma;
...
static bool conntrack_ps_status(struct xt_conntrack_mtinfo3 *info,
                                const char *status, size_t z) {
...
static void conntrack_ps_statuses(struct xt_conntrack_mtinfo3 *info,
                                  const char *arg) {
    const char *comma;
...
static void conntrack_parse(struct xt_option_call *cb) {
    ...
    case O_CTPROTO:
        ...
        if (sinfo->tuple[IP_CT_DIR_ORIGINAL].dst.protonum == 0 &&
            ...
            xtables_error(PARAMETER_PROBLEM, "rule would never match protocol");
...
static void print_status(unsigned int statusmask) {
    const char *sep = " ";
...
static void print_addr(const struct in_addr *addr, const struct in_addr *mask,
                       int inv, int numeric) {
    char buf[BUFSIZ];
...
static void matchinfo_print(const void *ip, const struct xt_entry_match *match,
    ...
    if (sinfo->flags & XT_CONNTRACK_ORIGSRC) {
        ...
        print_addr((struct in_addr *)&sinfo->tuple[IP_CT_DIR_ORIGINAL].src.ip,
                   &sinfo->sipmsk[IP_CT_DIR_ORIGINAL], false, numeric);
    }
    ...
    if (sinfo->flags & XT_CONNTRACK_ORIGDST) {
        ...
        print_addr((struct in_addr *)&sinfo->tuple[IP_CT_DIR_ORIGINAL].dst.ip,
                   &sinfo->dipmsk[IP_CT_DIR_ORIGINAL], false, numeric);
    }
    ...
    if (sinfo->flags & XT_CONNTRACK_REPLSRC) {
        ...
        print_addr((struct in_addr *)&sinfo->tuple[IP_CT_DIR_REPLY].src.ip,
                   &sinfo->sipmsk[IP_CT_DIR_REPLY], false, numeric);
    }
    ...
    if (sinfo->flags & XT_CONNTRACK_REPLDST) {
        ...
        print_addr((struct in_addr *)&sinfo->tuple[IP_CT_DIR_REPLY].dst.ip,
                   &sinfo->dipmsk[IP_CT_DIR_REPLY], false, numeric);
    }
...
static void conntrack_dump_ports(const char *prefix, const char *opt,
...
static void conntrack_dump(const struct xt_conntrack_mtinfo3 *info,
                           const char *prefix, unsigned int family,
                           bool numeric, bool v3) {
    if (info->match_flags & XT_CONNTRACK_STATE) {
        if (info->invert_flags & XT_CONNTRACK_STATE)
            printf(" !");
        printf(" %s%s", prefix,
               info->match_flags & XT_CONNTRACK_STATE_ALIAS ? "state"
                                                            : "ctstate");
    ...
    if (info->match_flags & XT_CONNTRACK_ORIGSRC) {
        ...
        conntrack_dump_addr(&info->origsrc_addr, &info->origsrc_mask, family,
                            numeric);
    }
    ...
    if (info->match_flags & XT_CONNTRACK_ORIGDST) {
        ...
        conntrack_dump_addr(&info->origdst_addr, &info->origdst_mask, family,
                            numeric);
    }
    ...
    if (info->match_flags & XT_CONNTRACK_REPLSRC) {
        ...
        conntrack_dump_addr(&info->replsrc_addr, &info->replsrc_mask, family,
                            numeric);
    }
    ...
    if (info->match_flags & XT_CONNTRACK_REPLDST) {
        ...
        conntrack_dump_addr(&info->repldst_addr, &info->repldst_mask, family,
                            numeric);
    }
...
conntrack_print_name_alias(const struct xt_entry_match *match) {
    ...
    return info->match_flags & XT_CONNTRACK_STATE_ALIAS ? "state" : "conntrack";
}
...
static void conntrack1_mt4_print(const void *ip,
...
static void conntrack1_mt6_print(const void *ip,
                                 const struct xt_entry_match *match,
                                 int numeric) {
    const struct xt_conntrack_mtinfo1 *info = (void *)match->data;
    struct xt_conntrack_mtinfo3 up;
...
static void conntrack1_mt4_save(const void *ip,
                                const struct xt_entry_match *match) {
    const struct xt_conntrack_mtinfo1 *info = (void *)match->data;
    struct xt_conntrack_mtinfo3 up;
...
static void conntrack1_mt6_save(const void *ip,
                                const struct xt_entry_match *match) {
    const struct xt_conntrack_mtinfo1 *info = (void *)match->data;
    struct xt_conntrack_mtinfo3 up;
...
static unsigned int state_parse_state(const char *state, size_t len) {
    if (strncasecmp(state, "INVALID", len) == 0)
...
static unsigned int state_parse_states(const char *arg) {
    const char *comma;
...
static int _conntrack3_mt_xlate(struct xt_xlate *xl,
    ...
    if (sinfo->match_flags & XT_CONNTRACK_DIRECTION) {
        xt_xlate_add(xl, "ct direction %s",
                     sinfo->invert_flags & XT_CONNTRACK_DIRECTION ? "reply"
                                                                  : "original");
        space = " ";
    }
    ...
    if (sinfo->match_flags & XT_CONNTRACK_EXPIRES) {
        ...
        if (sinfo->expires_max == sinfo->expires_min)
            ...
            xt_xlate_add(xl, "%lu-%lu", sinfo->expires_min, sinfo->expires_max);
        space = " ";
    }
    ...
    if (sinfo->match_flags & XT_CONNTRACK_ORIGSRC) {
        ...
        addr_xlate_print(xl, &sinfo->origsrc_addr, &sinfo->origsrc_mask,
                         family);
        space = " ";
    }
    ...
    if (sinfo->match_flags & XT_CONNTRACK_ORIGDST) {
        ...
        addr_xlate_print(xl, &sinfo->origdst_addr, &sinfo->origdst_mask,
                         family);
        space = " ";
    }
    ...
    if (sinfo->match_flags & XT_CONNTRACK_REPLSRC) {
        ...
        addr_xlate_print(xl, &sinfo->replsrc_addr, &sinfo->replsrc_mask,
                         family);
        space = " ";
    }
    ...
    if (sinfo->match_flags & XT_CONNTRACK_REPLDST) {
        ...
        addr_xlate_print(xl, &sinfo->repldst_addr, &sinfo->repldst_mask,
                         family);
        space = " ";
    }
