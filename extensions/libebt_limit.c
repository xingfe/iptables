...
static void brlimit_print_help(void) {
    printf(
        ...
        "default " EBT_LIMIT_AVG "\n"
        "                                [Packets per second unless followed "
        "by \n"
        "                                /sec /minute /hour /day postfixes]\n"
        "--limit-burst number          : number to match in a burst, -1 < "
        "number < 10001,\n"
        "                                default %u\n",
        EBT_LIMIT_BURST);
}
...
static int brlimit_parse(int c, char **argv, int invert, unsigned int *flags,
    ...
    case ARG_LIMIT:
        ...
        if (invert)
            xtables_error(PARAMETER_PROBLEM, "Unexpected `!' after --limit");
...
static struct rates g_rates[] = {{"day", EBT_LIMIT_SCALE * 24 * 60 * 60},
...
void _init(void) { xtables_register_match(&brlimit_match); }
