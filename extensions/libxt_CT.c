...
static void ct_help(void) {
    printf(
        ...
        "connection\n"
        " --expevents event[,event...]	Generate specified expectation events "
        "for connection\n"
        " --zone {ID|mark}		Assign/Lookup connection in zone "
        "ID/packet nfmark\n"
        " --zone-orig {ID|mark}		Same as 'zone' option, but only "
        "applies to ORIGINAL direction\n"
        " --zone-reply {ID|mark} 	Same as 'zone' option, but only applies "
        "to REPLY direction\n");
}
...
static void ct_help_v1(void) {
    printf(
        ...
        " --helper name			Use conntrack helper 'name' for "
        "connection\n"
        " --timeout name 		Use timeout policy 'name' for "
        "connection\n"
        " --ctevents event[,event...]	Generate specified conntrack events for "
        "connection\n"
        " --expevents event[,event...]	Generate specified expectation events "
    ...
    if (strcasecmp(opt, "mark") == 0) {
        *flags |= XT_CT_ZONE_MARK;
        ...
        if (!xtables_strtoul(opt, NULL, &val, 0, UINT16_MAX))
            xtables_error(PARAMETER_PROBLEM, "Cannot parse %s as a zone ID\n",
                          opt);
...
static void ct_print_zone_id(const char *pfx, uint16_t zone_id,
                             uint16_t flags) {
    printf(" %s", pfx);

    if ((flags & (XT_CT_ZONE_DIR_ORIG | XT_CT_ZONE_DIR_REPL)) ==
        XT_CT_ZONE_DIR_ORIG)
        printf("-orig");
    if ((flags & (XT_CT_ZONE_DIR_ORIG | XT_CT_ZONE_DIR_REPL)) ==
        XT_CT_ZONE_DIR_REPL)
...
static void ct_parse(struct xt_option_call *cb) {
    ...
    case O_ZONE_REPLY:
    case O_ZONE:
        ct_parse_zone_id(cb->arg, cb->entry->id, &info->zone, &info->flags);
        break;
    case O_CTEVENTS:
        info->ct_events =
            ct_parse_events(ct_event_tbl, ARRAY_SIZE(ct_event_tbl), cb->arg);
        break;
    case O_EXPEVENTS:
        info->exp_events =
            ct_parse_events(exp_event_tbl, ARRAY_SIZE(exp_event_tbl), cb->arg);
        break;
    }
}
...
static void ct_print(const void *ip, const struct xt_entry_target *target,
    ...
    if (info->ct_events)
        ct_print_events("ctevents", ct_event_tbl, ARRAY_SIZE(ct_event_tbl),
                        info->ct_events);
    if (info->exp_events)
        ct_print_events("expevents", exp_event_tbl, ARRAY_SIZE(exp_event_tbl),
                        info->exp_events);
    if (info->flags & XT_CT_ZONE_MARK || info->zone)
        ct_print_zone_id("zone", info->zone, info->flags);
}

static void ct_print_v1(const void *ip, const struct xt_entry_target *target,
                        int numeric) {
...
static const char *ct_print_name_alias(const struct xt_entry_target *target) {
