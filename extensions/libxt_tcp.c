...
static void parse_tcp_ports(const char *portstring, uint16_t *ports) {
    char *buffer;
    char *cp;
...
static const struct tcp_flag_names tcp_flag_names[] = {
    ...
    {"ACK", 0x10}, {"URG", 0x20}, {"ALL", 0x3F}, {"NONE", 0},
};

static unsigned int parse_tcp_flag(const char *flags) {
    unsigned int ret = 0;
    char *ptr;
    char *buffer;
...
static void parse_tcp_flags(struct xt_tcp *tcpinfo, const char *mask,
                            const char *cmp, int invert) {
...
static void parse_tcp_option(const char *option, uint8_t *result) {
    unsigned int ret;
...
static int tcp_parse(int c, char **argv, int invert, unsigned int *flags,
    ...
    case '4':
        ...
        if (!argv[optind] || argv[optind][0] == '-' || argv[optind][0] == '!')
            xtables_error(PARAMETER_PROBLEM, "--tcp-flags requires two args.");
    ...
    case '5':
        if (*flags & TCP_OPTION)
            xtables_error(PARAMETER_PROBLEM, "Only one `--tcp-option' allowed");
...
static void print_option(uint8_t option, int invert, int numeric) {
    if (option || invert)
        printf(" option=%s%u", invert ? "!" : "", option);
}

static void print_tcpf(uint8_t flags) {
    int have_flag = 0;
...
static void print_flags(uint8_t mask, uint8_t cmp, int invert, int numeric) {
    if (mask || invert) {
...
static void tcp_print(const void *ip, const struct xt_entry_match *match,
    ...
    print_ports("spt", tcp->spts[0], tcp->spts[1],
                tcp->invflags & XT_TCP_INV_SRCPT, numeric);
    print_ports("dpt", tcp->dpts[0], tcp->dpts[1],
                tcp->invflags & XT_TCP_INV_DSTPT, numeric);
    print_option(tcp->option, tcp->invflags & XT_TCP_INV_OPTION, numeric);
    print_flags(tcp->flg_mask, tcp->flg_cmp, tcp->invflags & XT_TCP_INV_FLAGS,
                numeric);
    if (tcp->invflags & ~XT_TCP_INV_MASK)
        printf(" Unknown invflags: 0x%X", tcp->invflags & ~XT_TCP_INV_MASK);
}
...
static void tcp_save(const void *ip, const struct xt_entry_match *match) {
    ...
    if (tcpinfo->spts[0] != 0 || tcpinfo->spts[1] != 0xFFFF) {
        ...
        if (tcpinfo->spts[0] != tcpinfo->spts[1])
            printf(" --sport %u:%u", tcpinfo->spts[0], tcpinfo->spts[1]);
        else
            printf(" --sport %u", tcpinfo->spts[0]);
    }
    ...
    if (tcpinfo->dpts[0] != 0 || tcpinfo->dpts[1] != 0xFFFF) {
        ...
        if (tcpinfo->dpts[0] != tcpinfo->dpts[1])
            printf(" --dport %u:%u", tcpinfo->dpts[0], tcpinfo->dpts[1]);
        else
            printf(" --dport %u", tcpinfo->dpts[0]);
    }
...
static int tcp_xlate(struct xt_xlate *xl,
    ...
    const struct xt_tcp *tcpinfo = (const struct xt_tcp *)params->match->data;
    char *space = "";
...
void _init(void) { xtables_register_match(&tcp_match); }
