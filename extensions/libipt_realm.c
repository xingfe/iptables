...
static void realm_parse(struct xt_option_call *cb) {
    ...
    if (end != cb->arg && (*end == '/' || *end == '\0')) {
        ...
        if (*end != '\0' || end == cb->arg)
            xtables_error(PARAMETER_PROBLEM, "Bad realm value \"%s\"", cb->arg);
    } else {
        id = xtables_lmap_name2id(realms, cb->arg);
        if (id == -1)
            xtables_error(PARAMETER_PROBLEM, "Realm \"%s\" not found", cb->arg);
        realminfo->id = id;
        realminfo->mask = 0xffffffff;
    }
    if (cb->invert)
...
static void print_realm(unsigned long id, unsigned long mask, int numeric) {
...
static void print_realm_xlate(unsigned long id, unsigned long mask, int numeric,
    ...
    if (mask != 0xffffffff)
        ...
        if (numeric == 0)
            name = xtables_lmap_id2name(realms, id);
        if (name)
            xt_xlate_add(xl, " %s%s", op == XT_OP_EQ ? "" : "!= ", name);
        else
            xt_xlate_add(xl, " %s0x%lx", op == XT_OP_EQ ? "" : "!= ", id);
    }
}
...
void _init(void) { xtables_register_match(&realm_mt_reg); }
