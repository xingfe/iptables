...
static void dccp_help(void) {
    printf("dccp match options\n"
           ...
           "destination port(s)\n"
           " --dport ...\n"
           "[!] --dccp-types type[,...]                            match when "
           "packet is one of the given types\n"
           "[!] --dccp-option option                               match if "
           "option (by number!) is set\n");
}
...
static const char *const dccp_pkt_types[] = {
        [DCCP_PKT_REQUEST] = "REQUEST", [DCCP_PKT_RESPONSE] = "RESPONSE",
        [DCCP_PKT_DATA] = "DATA",       [DCCP_PKT_ACK] = "ACK",
        [DCCP_PKT_DATAACK] = "DATAACK", [DCCP_PKT_CLOSEREQ] = "CLOSEREQ",
        [DCCP_PKT_CLOSE] = "CLOSE",     [DCCP_PKT_RESET] = "RESET",
        [DCCP_PKT_SYNC] = "SYNC",       [DCCP_PKT_SYNCACK] = "SYNCACK",
        [DCCP_PKT_INVALID] = "INVALID",
};

static uint16_t parse_dccp_types(const char *typestring) {
    uint16_t typemask = 0;
    char *ptr, *buffer;
    ...
    for (ptr = strtok(buffer, ","); ptr; ptr = strtok(NULL, ",")) {
        ...
        if (i == ARRAY_SIZE(dccp_pkt_types))
            xtables_error(PARAMETER_PROBLEM, "Unknown DCCP type `%s'", ptr);
    }
...
static void print_types(uint16_t types, int inverted, int numeric) {
    int have_type = 0;
...
static void dccp_print(const void *ip, const struct xt_entry_match *match,
                       int numeric) {
    const struct xt_dccp_info *einfo = (const struct xt_dccp_info *)match->data;

    printf(" dccp");

    if (einfo->flags & XT_DCCP_SRC_PORTS) {
        print_ports("spt", einfo->spts[0], einfo->spts[1],
                    einfo->invflags & XT_DCCP_SRC_PORTS, numeric);
    }

    if (einfo->flags & XT_DCCP_DEST_PORTS) {
        print_ports("dpt", einfo->dpts[0], einfo->dpts[1],
                    einfo->invflags & XT_DCCP_DEST_PORTS, numeric);
    }

    if (einfo->flags & XT_DCCP_TYPE) {
        print_types(einfo->typemask, einfo->invflags & XT_DCCP_TYPE, numeric);
    }

    if (einfo->flags & XT_DCCP_OPTION) {
        print_option(einfo->option, einfo->invflags & XT_DCCP_OPTION, numeric);
    }
}
...
static void dccp_save(const void *ip, const struct xt_entry_match *match) {
    ...
    if (einfo->flags & XT_DCCP_TYPE) {
        printf("%s --dccp-types", einfo->invflags & XT_DCCP_TYPE ? " !" : "");
        print_types(einfo->typemask, false, 0);
    }

    if (einfo->flags & XT_DCCP_OPTION) {
        printf("%s --dccp-option %u",
               einfo->invflags & XT_DCCP_OPTION ? " !" : "", einfo->option);
    }
}
...
void _init(void) { xtables_register_match(&dccp_match); }
