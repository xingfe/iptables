    ...
    for (i = 0; i < ARRAY_SIZE(supported_types); ++i)
        if (supported_types[i].printhelp == 1)
            printf("\t%-14s\t\t%s\n", supported_types[i].name,
                   supported_types[i].help);
    printf("\n");
}

static void pkttype_help(void) {
    printf("pkttype match options:\n"
...
static void print_pkttype(const struct xt_pkttype_info *info) {
    ...
    printf(
        "%d",
        info->pkttype); /* in case we didn't find an entry in named-packtes */
}
...
void _init(void) { xtables_register_match(&pkttype_match); }
