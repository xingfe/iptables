...
static const struct xt_option_entry ipvs_mt_opts[] = {
    {.name = "ipvs", .id = O_IPVS, .type = XTTYPE_NONE, .flags = XTOPT_INVERT},
    {.name = "vproto",
     .id = O_VPROTO,
     .type = XTTYPE_STRING,
     .flags = XTOPT_INVERT | XTOPT_PUT,
     XTOPT_POINTER(s, l4proto)},
    {.name = "vaddr",
     .id = O_VADDR,
...
static void ipvs_mt_parse(struct xt_option_call *cb) {
    ...
    case O_VDIR:
        if (strcasecmp(cb->arg, "ORIGINAL") == 0) {
            ...
            xtables_param_act(XTF_BAD_VALUE, "ipvs", "--vdir", cb->arg);
        }
        break;
    case O_VMETHOD:
        if (strcasecmp(cb->arg, "GATE") == 0)
            ...
            xtables_param_act(XTF_BAD_VALUE, "ipvs", "--vmethod", cb->arg);
        break;
    }
...
static void ipvs_mt_check(struct xt_fcheck_call *cb) {
    ...
    if (info->bitmask & XT_IPVS_ONCE_MASK) {
        if (info->invert & XT_IPVS_IPVS_PROPERTY)
            xtables_error(PARAMETER_PROBLEM, "! --ipvs cannot be together with"
                                             " other options");
...
void _init(void) {
    xtables_register_matches(ipvs_matches_reg, ARRAY_SIZE(ipvs_matches_reg));
}
