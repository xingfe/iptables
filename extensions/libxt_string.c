...
static const struct xt_option_entry string_opts[] = {
    {.name = "from",
     ...
     .type = XTTYPE_UINT16,
     .flags = XTOPT_PUT,
     XTOPT_POINTER(s, from_offset)},
    {.name = "to",
     .id = O_TO,
     .type = XTTYPE_UINT16,
     .flags = XTOPT_PUT,
     XTOPT_POINTER(s, to_offset)},
    {.name = "algo",
     .id = O_ALGO,
     .type = XTTYPE_STRING,
     .flags = XTOPT_MAND | XTOPT_PUT,
     XTOPT_POINTER(s, algo)},
    {.name = "string",
     .id = O_STRING,
     .type = XTTYPE_STRING,
     .flags = XTOPT_INVERT,
...
static void parse_hex_string(const char *s, struct xt_string_info *info) {
    ...
    while (i < slen) {
        ...
        if (literal_f) {
            ...
            if (i + 1 >= slen) {
                xtables_error(PARAMETER_PROBLEM, "Odd number of hex digits");
            }
            if (i + 2 >= slen) {
                /* must end with a "|" */
            ...
            if (!isxdigit(s[i + 1])) /* check for valid hex char */
                xtables_error(PARAMETER_PROBLEM, "Invalid hex char '%c'",
                              s[i + 1]);
            hextmp[0] = s[i];
            hextmp[1] = s[i + 1];
            hextmp[2] = '\0';
...
static void string_parse(struct xt_option_call *cb) {
    ...
    case O_ICASE:
        if (revision == 0)
            xtables_error(VERSION_PROBLEM, "Kernel doesn't support --icase");
...
static void string_print(const void *ip, const struct xt_entry_match *match,
                         int numeric) {
