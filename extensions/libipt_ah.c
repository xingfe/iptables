...
static void ah_print(const void *ip, const struct xt_entry_match *match,
    ...
    printf(" ah ");
    print_spis("spi", ah->spis[0], ah->spis[1], ah->invflags & IPT_AH_INV_SPI);
    if (ah->invflags & ~IPT_AH_INV_MASK)
        printf(" Unknown invflags: 0x%X", ah->invflags & ~IPT_AH_INV_MASK);
}
...
static void ah_save(const void *ip, const struct xt_entry_match *match) {
    ...
    if (!(ahinfo->spis[0] == 0 && ahinfo->spis[1] == 0xFFFFFFFF)) {
        printf("%s --ahspi ", (ahinfo->invflags & IPT_AH_INV_SPI) ? " !" : "");
...
void _init(void) { xtables_register_match(&ah_mt_reg); }
