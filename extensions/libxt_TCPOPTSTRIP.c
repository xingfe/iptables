...
static void tcpoptstrip_tg_help(void) {
    ...
    printf("TCPOPTSTRIP target options:\n"
           "  --strip-options value     strip specified TCP options denoted by "
           "value\n"
           "                            (separated by comma) from TCP header\n"
           "  Instead of the numeric value, you can also use the following "
           "names:\n");
...
static void parse_list(struct xt_tcpoptstrip_target_info *info,
                       const char *arg) {
    unsigned int option;
    char *p;
    int i;
    ...
    while (true) {
        ...
        if (option == 0 && !xtables_strtoui(arg, NULL, &option, 0, UINT8_MAX))
            xtables_error(PARAMETER_PROBLEM, "Bad TCP option value \"%s\"",
                          arg);

        if (option < 2)
            xtables_error(PARAMETER_PROBLEM, "Option value may not be 0 or 1");

        if (tcpoptstrip_test_bit(info->strip_bmap, option))
            xtables_error(PARAMETER_PROBLEM, "Option \"%s\" already specified",
                          arg);
...
static void tcpoptstrip_tg_print(const void *ip,
                                 const struct xt_entry_target *target,
                                 int numeric) {
    const struct xt_tcpoptstrip_target_info *info = (const void *)target->data;

    printf(" TCPOPTSTRIP options ");
    tcpoptstrip_print_list(info, numeric);
}

static void tcpoptstrip_tg_save(const void *ip,
                                const struct xt_entry_target *target) {
    const struct xt_tcpoptstrip_target_info *info = (const void *)target->data;
...
void _init(void) { xtables_register_target(&tcpoptstrip_tg_reg); }
