...
static void multiport_help(void) {
    printf("multiport match options:\n"
...
static const char *proto_to_name(uint8_t proto) {
    switch (proto) {
    case IPPROTO_TCP:
        return "tcp";
    case IPPROTO_UDP:
...
static unsigned int parse_multi_ports(const char *portstring, uint16_t *ports,
                                      const char *proto) {
    char *buffer, *cp, *next;
    unsigned int i;
...
static void parse_multi_ports_v1(const char *portstring,
    ...
    for (cp = buffer, i = 0; cp && i < XT_MULTI_PORTS; cp = next, i++) {
        ...
        if (range) {
            ...
            if (multiinfo->ports[i - 1] >= multiinfo->ports[i])
                xtables_error(PARAMETER_PROBLEM, "invalid portrange specified");
        }
    }
    multiinfo->count = i;
    if (cp)
...
static const char *check_proto(uint16_t pnum, uint8_t invflags) {
    const char *proto;
...
static void __multiport_parse(struct xt_option_call *cb, uint16_t pnum,
    ...
    case O_SOURCE_PORTS:
        ...
        multiinfo->count = parse_multi_ports(cb->arg, multiinfo->ports, proto);
    ...
    if (cb->invert)
        xtables_error(PARAMETER_PROBLEM, "multiport.0 does not support invert");
}

static void multiport_parse(struct xt_option_call *cb) {
    const struct ipt_entry *entry = cb->xt_entry;
    return __multiport_parse(cb, entry->ip.proto, entry->ip.invflags);
}

static void multiport_parse6(struct xt_option_call *cb) {
    const struct ip6t_entry *entry = cb->xt_entry;
    return __multiport_parse(cb, entry->ipv6.proto, entry->ipv6.invflags);
}
...
static void multiport_parse_v1(struct xt_option_call *cb) {
    const struct ipt_entry *entry = cb->xt_entry;
    return __multiport_parse_v1(cb, entry->ip.proto, entry->ip.invflags);
}

static void multiport_parse6_v1(struct xt_option_call *cb) {
    const struct ip6t_entry *entry = cb->xt_entry;
    return __multiport_parse_v1(cb, entry->ipv6.proto, entry->ipv6.invflags);
}
...
static const char *port_to_service(int port, uint8_t proto) {
...
static void print_port(uint16_t port, uint8_t protocol, int numeric) {
    const char *service;
...
static void __multiport_print(const struct xt_entry_match *match, int numeric,
                              uint16_t proto) {
...
static int __multiport_xlate_v1(struct xt_xlate *xl,
    ...
    if (multiinfo->count > 2 || (multiinfo->count > 1 && !multiinfo->pflags[0]))
        xt_xlate_add(xl, "{ ");
