...
static void connlabel_open(void) {
    ...
    if (errno) {
        xtables_error(RESOURCE_PROBLEM, "cannot open %s: %s", fname,
                      strerror(errno));
    } else {
        xtables_error(RESOURCE_PROBLEM, "cannot parse %s: no labels found",
                      fname);
    }
}
...
static void connlabel_mt_print_op(const struct xt_connlabel_mtinfo *info,
                                  const char *prefix) {
    if (info->options & XT_CONNLABEL_OP_SET)
        printf(" %sset", prefix);
}

static void connlabel_mt_print(const void *ip,
...
static void connlabel_mt_save(const void *ip,
                              const struct xt_entry_match *match) {
    const struct xt_connlabel_mtinfo *info = (const void *)match->data;
    const char *name = connlabel_get_name(info->bit);
...
static int connlabel_mt_xlate(struct xt_xlate *xl,
    ...
    const struct xt_connlabel_mtinfo *info = (const void *)params->match->data;
    const char *name = connlabel_get_name(info->bit);
...
void _init(void) { xtables_register_match(&connlabel_mt_reg); }
