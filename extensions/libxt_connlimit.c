...
static void connlimit_help(void) {
    printf(
        ...
        "  --connlimit-upto n     match if the number of existing connections "
        "is 0..n\n"
        "  --connlimit-above n    match if the number of existing connections "
        "is >n\n"
        "  --connlimit-mask n     group hosts using prefix length (default: "
        "max len)\n"
...
static void connlimit_parse(struct xt_option_call *cb, uint8_t family) {
    ...
    case O_SADDR:
        if (revision < 1)
            xtables_error(PARAMETER_PROBLEM, "xt_connlimit.0 does not support "
                                             "--connlimit-daddr");
...
static void connlimit_print4(const void *ip, const struct xt_entry_match *match,
                             int numeric) {
...
static void connlimit_print6(const void *ip, const struct xt_entry_match *match,
                             int numeric) {
    const struct xt_connlimit_info *info = (const void *)match->data;
...
static void connlimit_save4(const void *ip,
...
static void connlimit_save6(const void *ip,
                            const struct xt_entry_match *match) {
    const struct xt_connlimit_info *info = (const void *)match->data;
    const int revision = match->u.user.revision;
