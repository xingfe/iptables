...
static void parse_to(const char *orig_arg, int portok,
    ...
    if (start == NULL) {
        start = arg;
        /* Lets assume one colon is port information. Otherwise its an IPv6
         * address */
        colon = strchr(arg, ':');
        ...
        if (end == NULL)
            xtables_error(PARAMETER_PROBLEM, "Invalid address format");
    ...
    if (colon) {
        ...
        dash = strchr(colon, '-');
        if (!dash) {
            range->min_proto.tcp.port = range->max_proto.tcp.port = htons(port);
        } else {
            int maxport;
...
static void DNAT_parse(struct xt_option_call *cb) {
    ...
    if (entry->ipv6.proto == IPPROTO_TCP || entry->ipv6.proto == IPPROTO_UDP ||
        ...
        entry->ipv6.proto == IPPROTO_DCCP || entry->ipv6.proto == IPPROTO_ICMP)
        portok = 1;
    else
        portok = 0;
...
static void print_range(const struct nf_nat_range *range) {
    if (range->flags & NF_NAT_RANGE_MAP_IPS) {
        ...
        if (memcmp(&range->min_addr, &range->max_addr, sizeof(range->min_addr)))
...
static void print_range_xlate(const struct nf_nat_range *range,
    ...
    if (range->flags & NF_NAT_RANGE_MAP_IPS) {
        xt_xlate_add(xl, "%s%s%s", proto_specified ? "[" : "",
