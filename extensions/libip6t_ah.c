...
static void print_spis(const char *name, uint32_t min, uint32_t max,
                       int invert) {
...
static void print_len(const char *name, uint32_t len, int invert) {
    const char *inv = invert ? "!" : "";
...
static void ah_print(const void *ip, const struct xt_entry_match *match,
    ...
    printf(" ah ");
    print_spis("spi", ah->spis[0], ah->spis[1], ah->invflags & IP6T_AH_INV_SPI);
    print_len("length", ah->hdrlen, ah->invflags & IP6T_AH_INV_LEN);

    if (ah->hdrres)
        printf(" reserved");

    if (ah->invflags & ~IP6T_AH_INV_MASK)
        printf(" Unknown invflags: 0x%X", ah->invflags & ~IP6T_AH_INV_MASK);
}
...
static void ah_save(const void *ip, const struct xt_entry_match *match) {
    ...
    if (!(ahinfo->spis[0] == 0 && ahinfo->spis[1] == 0xFFFFFFFF)) {
        printf("%s --ahspi ", (ahinfo->invflags & IP6T_AH_INV_SPI) ? " !" : "");
        if (ahinfo->spis[0] != ahinfo->spis[1])
            printf("%u:%u", ahinfo->spis[0], ahinfo->spis[1]);
        else
            printf("%u", ahinfo->spis[0]);
    }
...
void _init(void) { xtables_register_match(&ah_mt6_reg); }
