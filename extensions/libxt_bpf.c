...
static void bpf_help_v1(void) {
    printf(
        ...
        "--bytecode <program>	        : a bpf program as generated by\n"
        "                                 $(nfbpf_compile RAW '<filter>')\n"
        "--object-pinned <bpf object>	: a path to a pinned BPF object in bpf "
        "fs\n");
}
...
static const struct xt_option_entry bpf_opts_v1[] = {
    ...
    {.name = "object-pinned",
     ...
     .type = XTTYPE_STRING,
     .flags = XTOPT_PUT,
     XTOPT_POINTER(struct xt_bpf_info_v1, path)},
    XTOPT_TABLEEND,
};
...
static void bpf_parse_string(struct sock_filter *pc, __u16 *lenp, __u16 len_max,
    ...
    if (sscanf(bpf_program, "%hu%c", &len, &sp) != 2 || sp != separator)
        xtables_error(PARAMETER_PROBLEM, "bpf: error parsing program length");
    if (!len)
        xtables_error(PARAMETER_PROBLEM, "bpf: illegal zero length program");
    if (len > len_max)
    ...
    while ((token = strchr(token, separator)) && (++token)[0]) {
        if (i >= len)
            xtables_error(PARAMETER_PROBLEM, "bpf: real program length exceeds"
                                             " the encoded length parameter");
        if (sscanf(token, "%hu %hhu %hhu %u,", &pc->code, &pc->jt, &pc->jf,
                   &pc->k) != 4)
...
static void bpf_parse_obj_pinned(struct xt_bpf_info_v1 *bi,
    ...
    if (bi->fd < 0)
        xtables_error(PARAMETER_PROBLEM, "bpf: failed to get bpf object");

    /* Cannot close bi->fd explicitly. Rely on exit */
    if (fcntl(bi->fd, F_SETFD, FD_CLOEXEC) == -1) {
        xtables_error(OTHER_PROBLEM, "Could not set close on exec: %s\n",
                      strerror(errno));
    }
}
...
static void bpf_print_code(const struct sock_filter *pc, __u16 len, char tail) {
    for (; len; len--, pc++)
        printf("%hu %hhu %hhu %u%c", pc->code, pc->jt, pc->jf, pc->k,
               len > 1 ? ',' : tail);
}
...
static void bpf_fcheck(struct xt_fcheck_call *cb) {
    if (!(cb->xflags & (1 << O_BCODE_STDIN)))
        xtables_error(PARAMETER_PROBLEM, "bpf: missing --bytecode parameter");
}
