...
static const struct xt_option_entry recent_opts_v0[] = {
    ...
    {.name = "seconds",
     ...
     .type = XTTYPE_UINT32,
     .flags = XTOPT_PUT,
     XTOPT_POINTER(s, seconds),
     .min = 1},
    {.name = "reap", .id = O_REAP, .type = XTTYPE_NONE, .also = F_SECONDS},
    {.name = "hitcount",
     .id = O_HITCOUNT,
     .type = XTTYPE_UINT32,
     .flags = XTOPT_PUT,
     XTOPT_POINTER(s, hit_count)},
    {.name = "rttl",
     .id = O_RTTL,
     .type = XTTYPE_NONE,
     .excl = F_SET | F_REMOVE},
...
static void recent_help(void) {
    printf(
        ...
        "occur if source address last seen within\n"
        ...
        "                                Specifies that the match will only "
        "occur if the source address and the TTL\n"
        "                                match between this packet and the one "
        "which was set.\n"
        "                                Useful if you have problems with "
        "people spoofing their source address in order\n"
        ...
        "    --mask netmask              Netmask that will be applied to this "
        "recent list.\n"
...
static void recent_print(const void *ip, const struct xt_entry_match *match,
    ...
    if (info->check_set & XT_RECENT_REMOVE)
        printf(" REMOVE");
    if (info->seconds)
        printf(" seconds: %d", info->seconds);
    if (info->check_set & XT_RECENT_REAP)
        printf(" reap");
    if (info->hit_count)
        printf(" hit_count: %d", info->hit_count);
    ...
    switch (family) {
    case NFPROTO_IPV4:
        printf(" mask: %s", xtables_ipaddr_to_numeric(&info->mask.in));
        break;
    case NFPROTO_IPV6:
        printf(" mask: %s", xtables_ip6addr_to_numeric(&info->mask.in6));
        break;
    }
}
...
static void recent_save(const void *ip, const struct xt_entry_match *match,
    ...
    if (info->check_set & XT_RECENT_REAP)
        printf(" --reap");
    if (info->hit_count)
        printf(" --hitcount %d", info->hit_count);
    if (info->check_set & XT_RECENT_TTL)
        printf(" --rttl");
    if (info->name)
        printf(" --name %s", info->name);
