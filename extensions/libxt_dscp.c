...
static void dscp_help(void) {
    printf("dscp match options\n"
           "[!] --dscp value		Match DSCP codepoint with numerical "
           "value\n"
           "  		                This value can be in decimal (ex: 32)\n"
           "               		or in hex (ex: 0x20)\n"
           "[!] --dscp-class name		Match the DiffServ class. This "
           "value may\n"
           "				be any of the BE,EF, AFxx or CSx "
           "classes\n"
           "\n"
...
static void dscp_print(const void *ip, const struct xt_entry_match *match,
                       int numeric) {
    const struct xt_dscp_info *dinfo = (const struct xt_dscp_info *)match->data;
...
static int __dscp_xlate(struct xt_xlate *xl,
    ...
    xt_xlate_add(xl, "dscp %s0x%02x", dinfo->invert ? "!= " : "", dinfo->dscp);
