...
static void cluster_help(void) {
    printf(
        "cluster match options:\n"
        "  --cluster-total-nodes <num>		Set number of total nodes in "
        "cluster\n"
        "  [!] --cluster-local-node <num>	Set the local node number\n"
        "  [!] --cluster-local-nodemask <num>	Set the local node mask\n"
        "  --cluster-hash-seed <num>		Set seed value of the Jenkins "
        "hash\n");
}
...
static const struct xt_option_entry cluster_opts[] = {
    {.name = "cluster-total-nodes",
     ...
     .type = XTTYPE_UINT32,
     .min = 1,
     .max = XT_CLUSTER_NODES_MAX,
     .flags = XTOPT_MAND | XTOPT_PUT,
     XTOPT_POINTER(s, total_nodes)},
    {.name = "cluster-local-node",
     .id = O_CL_LOCAL_NODE,
     .excl = F_CL_LOCAL_NODEMASK,
     .flags = XTOPT_INVERT,
     .type = XTTYPE_UINT32,
     .min = 1,
     .max = XT_CLUSTER_NODES_MAX},
    {.name = "cluster-local-nodemask",
     .id = O_CL_LOCAL_NODEMASK,
     .excl = F_CL_LOCAL_NODE,
     .type = XTTYPE_UINT32,
     .min = 1,
     .max = XT_CLUSTER_NODES_MAX,
     .flags = XTOPT_INVERT | XTOPT_PUT,
     XTOPT_POINTER(s, node_mask)},
...
static void cluster_print(const void *ip, const struct xt_entry_match *match,
                          int numeric) {
    ...
    printf(" total_nodes=%u hash_seed=0x%08x", info->total_nodes,
           info->hash_seed);
}

static void cluster_save(const void *ip, const struct xt_entry_match *match) {
    const struct xt_cluster_match_info *info = (void *)match->data;
...
void _init(void) { xtables_register_match(&cluster_mt_reg); }
