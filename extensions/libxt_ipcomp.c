...
static void comp_print(const void *ip, const struct xt_entry_match *match,
    ...
    if (comp->invflags & ~XT_IPCOMP_INV_MASK)
        printf(" Unknown invflags: 0x%X", comp->invflags & ~XT_IPCOMP_INV_MASK);
}
...
static void comp_save(const void *ip, const struct xt_entry_match *match) {
    ...
    if (!(compinfo->spis[0] == 0 && compinfo->spis[1] == 0xFFFFFFFF)) {
        printf("%s --ipcompspi ",
               (compinfo->invflags & XT_IPCOMP_INV_SPI) ? " !" : "");
        if (compinfo->spis[0] != compinfo->spis[1])
            printf("%u:%u", compinfo->spis[0], compinfo->spis[1]);
        else
            printf("%u", compinfo->spis[0]);
    }
...
static int comp_xlate(struct xt_xlate *xl,
    ...
    const struct xt_ipcomp *compinfo = (struct xt_ipcomp *)params->match->data;

    xt_xlate_add(xl, "comp cpi %s",
                 compinfo->invflags & XT_IPCOMP_INV_SPI ? "!= " : "");
    if (compinfo->spis[0] != compinfo->spis[1])
        xt_xlate_add(xl, "%u-%u", compinfo->spis[0], compinfo->spis[1]);
    else
...
void _init(void) { xtables_register_match(&comp_mt_reg); };
