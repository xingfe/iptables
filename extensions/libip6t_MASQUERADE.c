...
static void MASQUERADE_help(void) {
    printf("MASQUERADE target options:\n"
           " --to-ports <port>[-<port>]\n"
...
/* Parses ports */
static void parse_ports(const char *arg, struct nf_nat_range *r) {
    char *end;
    ...
    switch (*end) {
    case '\0':
        r->min_proto.tcp.port = r->max_proto.tcp.port = htons(port);
        return;
    case '-':
...
static void MASQUERADE_print(const void *ip,
...
static void MASQUERADE_save(const void *ip,
                            const struct xt_entry_target *target) {
    const struct nf_nat_range *r = (const void *)target->data;
...
void _init(void) { xtables_register_target(&masquerade_tg_reg); }
