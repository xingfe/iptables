...
static void rt_help(void) {
    printf(
        ...
        " --rt-0-not-strict             List of Type=0 addresses not a strict "
        "list\n",
        IP6T_RT_HOPS);
}
...
static const struct xt_option_entry rt_opts[] = {
    {.name = "rt-type",
     ...
     XTOPT_POINTER(s, rt_type)},
    {.name = "rt-segsleft",
     .id = O_RT_SEGSLEFT,
     .type = XTTYPE_UINT32RC,
     .flags = XTOPT_INVERT | XTOPT_PUT,
     XTOPT_POINTER(s, segsleft)},
    {.name = "rt-len",
     .id = O_RT_LEN,
     .type = XTTYPE_UINT32,
     .flags = XTOPT_INVERT | XTOPT_PUT,
     XTOPT_POINTER(s, hdrlen)},
...
static const char *addr_to_numeric(const struct in6_addr *addrp) {
...
static int parse_addresses(const char *addrstr, struct in6_addr *addrp) {
    char *buffer, *cp, *next;
    unsigned int i;
...
static void print_nums(const char *name, uint32_t min, uint32_t max,
                       int invert) {
...
static void print_addresses(unsigned int addrnr, struct in6_addr *addrp) {
    unsigned int i;
...
static void rt_print(const void *ip, const struct xt_entry_match *match,
    ...
    if (rtinfo->flags & IP6T_RT_RES)
        printf(" reserved");
    if (rtinfo->flags & IP6T_RT_FST)
        printf(" 0-addrs");
    print_addresses(rtinfo->addrnr, (struct in6_addr *)rtinfo->addrs);
    if (rtinfo->flags & IP6T_RT_FST_NSTRICT)
        printf(" 0-not-strict");
    if (rtinfo->invflags & ~IP6T_RT_INV_MASK)
        printf(" Unknown invflags: 0x%X", rtinfo->invflags & ~IP6T_RT_INV_MASK);
}
...
static void rt_save(const void *ip, const struct xt_entry_match *match) {
    ...
    if (!(rtinfo->segsleft[0] == 0 && rtinfo->segsleft[1] == 0xFFFFFFFF)) {
        ...
        if (rtinfo->segsleft[0] != rtinfo->segsleft[1])
            printf("%u:%u", rtinfo->segsleft[0], rtinfo->segsleft[1]);
        else
            printf("%u", rtinfo->segsleft[0]);
    }
...
static int rt_xlate(struct xt_xlate *xl,
    ...
    if (!(rtinfo->segsleft[0] == 0 && rtinfo->segsleft[1] == 0xFFFFFFFF)) {
        ...
        if (rtinfo->segsleft[0] != rtinfo->segsleft[1])
            xt_xlate_add(xl, "%u-%u", rtinfo->segsleft[0], rtinfo->segsleft[1]);
        else
...
void _init(void) { xtables_register_match(&rt_mt6_reg); }
