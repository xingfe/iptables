...
static void hashlimit_help(void) {
    printf(
        ...
        "                                [Packets per second unless followed "
        "by \n"
        "                                /sec /minute /hour /day postfixes]\n"
        "--hashlimit-mode <mode>		mode is a comma-separated list "
        "of\n"
        "					dstip,srcip,dstport,srcport\n"
        "--hashlimit-name <name>		name for "
        "/proc/net/ipt_hashlimit/\n"
        "[--hashlimit-burst <num>]	number to match in a burst, default "
        "%u\n"
        "[--hashlimit-htable-size <num>]	number of hashtable buckets\n"
        "[--hashlimit-htable-max <num>]	number of hashtable entries\n"
        "[--hashlimit-htable-gcinterval]	interval between garbage "
        "collection runs\n"
...
static void hashlimit_mt_help(void) {
    printf(
        ...
        "                                   [Packets per second unless "
        "followed by \n"
        "                                   /sec /minute /hour /day "
        "postfixes]\n"
        "  --hashlimit-above <avg>          min average match rate\n"
        "  --hashlimit-mode <mode>          mode is a comma-separated list of\n"
        "                                   dstip,srcip,dstport,srcport (or "
        "none)\n"
        "  --hashlimit-srcmask <length>     source address grouping prefix "
        "length\n"
        "  --hashlimit-dstmask <length>     destination address grouping "
        "prefix length\n"
...
static const struct xt_option_entry hashlimit_opts[] = {
    {.name = "hashlimit", .id = O_UPTO, .excl = F_ABOVE, .type = XTTYPE_STRING},
    {.name = "hashlimit-burst",
     .id = O_BURST,
     .type = XTTYPE_UINT32,
     .min = 1,
     .max = XT_HASHLIMIT_BURST_MAX_v1,
     .flags = XTOPT_PUT,
     XTOPT_POINTER(s, cfg.burst)},
    {.name = "hashlimit-htable-size",
     .id = O_HTABLE_SIZE,
     .type = XTTYPE_UINT32,
     .flags = XTOPT_PUT,
     XTOPT_POINTER(s, cfg.size)},
    {.name = "hashlimit-htable-max",
     .id = O_HTABLE_MAX,
     .type = XTTYPE_UINT32,
     .flags = XTOPT_PUT,
     XTOPT_POINTER(s, cfg.max)},
    {.name = "hashlimit-htable-gcinterval",
     .id = O_HTABLE_GCINT,
     .type = XTTYPE_UINT32,
     .flags = XTOPT_PUT,
     XTOPT_POINTER(s, cfg.gc_interval)},
    {.name = "hashlimit-htable-expire",
     .id = O_HTABLE_EXPIRE,
     .type = XTTYPE_UINT32,
     .flags = XTOPT_PUT,
...
static int cfg_copy(struct hashlimit_cfg2 *to, const void *from, int revision) {
    if (revision == 1) {
...
static uint64_t parse_burst(const char *burst, int revision) {
    ...
    uint64_t burst_max =
        (revision == 1) ? XT_HASHLIMIT_BURST_MAX_v1 : XT_HASHLIMIT_BURST_MAX;
...
static bool parse_bytes(const char *rate, void *val,
                        struct hashlimit_mt_udata *ud, int revision) {
...
static int parse_rate(const char *rate, void *val,
                      struct hashlimit_mt_udata *ud, int revision) {
    const char *delim;
    uint64_t tmp, r;
    uint64_t scale =
        (revision == 1) ? XT_HASHLIMIT_SCALE : XT_HASHLIMIT_SCALE_v2;
...
static void hashlimit_parse(struct xt_option_call *cb) {
    ...
    case O_UPTO:
        if (!parse_rate(cb->arg, &info->cfg.avg, cb->udata, 1))
            xtables_param_act(XTF_BAD_VALUE, "hashlimit", "--hashlimit-upto",
                              cb->arg);
        break;
    case O_MODE:
        if (parse_mode(&info->cfg.mode, cb->arg) < 0)
            xtables_param_act(XTF_BAD_VALUE, "hashlimit", "--hashlimit-mode",
                              cb->arg);
        break;
    }
}
...
static void hashlimit_mt_parse_v1(struct xt_option_call *cb) {
    ...
    case O_ABOVE:
        ...
        if (parse_bytes(cb->arg, &info->cfg.avg, cb->udata, 1))
            ...
            xtables_param_act(XTF_BAD_VALUE, "hashlimit", "--hashlimit-above",
                              cb->arg);
        break;
    case O_MODE:
        if (parse_mode(&info->cfg.mode, cb->arg) < 0)
...
static void hashlimit_check(struct xt_fcheck_call *cb) {
    ...
    if (!(cb->xflags & (F_UPTO | F_ABOVE)))
        xtables_error(PARAMETER_PROBLEM, "You have to specify --hashlimit");
...
static void hashlimit_print(const void *ip, const struct xt_entry_match *match,
                            int numeric) {
...
static void hashlimit_mt_print(const struct hashlimit_cfg2 *cfg,
                               unsigned int dmask, int revision) {
    uint32_t quantum;
...
static void hashlimit_mt4_print_v1(const void *ip,
...
static void hashlimit_mt6_print_v1(const void *ip,
                                   const struct xt_entry_match *match,
                                   int numeric) {
    const struct xt_hashlimit_mtinfo1 *info = (const void *)match->data;
    struct hashlimit_cfg2 cfg;
...
static void hashlimit_mt4_print(const void *ip,
                                const struct xt_entry_match *match,
                                int numeric) {
...
static void hashlimit_mt6_print(const void *ip,
                                const struct xt_entry_match *match,
                                int numeric) {
    const struct xt_hashlimit_mtinfo2 *info = (const void *)match->data;
...
static void hashlimit_mt_save(const struct hashlimit_cfg2 *cfg,
                              const char *name, unsigned int dmask,
                              int revision) {
    uint32_t quantum;
...
static void hashlimit_mt4_save_v1(const void *ip,
                                  const struct xt_entry_match *match) {
    const struct xt_hashlimit_mtinfo1 *info = (const void *)match->data;
    struct hashlimit_cfg2 cfg;
    int ret;
...
static void hashlimit_mt6_save_v1(const void *ip,
                                  const struct xt_entry_match *match) {
    const struct xt_hashlimit_mtinfo1 *info = (const void *)match->data;
    struct hashlimit_cfg2 cfg;
    int ret;
...
static void hashlimit_mt4_save(const void *ip,
                               const struct xt_entry_match *match) {
    const struct xt_hashlimit_mtinfo2 *info = (const void *)match->data;
...
static void hashlimit_mt6_save(const void *ip,
                               const struct xt_entry_match *match) {
    const struct xt_hashlimit_mtinfo2 *info = (const void *)match->data;
...
static void print_packets_rate_xlate(struct xt_xlate *xl, uint64_t avg,
    ...
    unsigned int i;
    const struct rates *_rates = (revision == 1) ? rates_v1_xlate : rates_xlate;

    for (i = 1; i < ARRAY_SIZE(rates); ++i)
        if (avg > _rates[i].mult || _rates[i].mult / avg < _rates[i].mult % avg)
            break;

    xt_xlate_add(xl, " %llu/%s burst %lu packets", _rates[i - 1].mult / avg,
                 _rates[i - 1].name, burst);
}
...
static void hashlimit_print_subnet_xlate(struct xt_xlate *xl, uint32_t nsub,
                                         int family) {
...
static int hashlimit_mode_xlate(struct xt_xlate *xl, uint32_t mode, int family,
                                unsigned int nsrc, unsigned int ndst) {
    const char *const *_modes = (family == NFPROTO_IPV4)
                                    ? hashlimit_modes4_xlate
                                    : hashlimit_modes6_xlate;
    bool prevopt = false;
    unsigned int mask;
...
static int hashlimit_mt_xlate(struct xt_xlate *xl, const char *name,
    ...
    ret =
        hashlimit_mode_xlate(xl, cfg->mode, family, cfg->srcmask, cfg->dstmask);
...
static int hashlimit_mt4_xlate_v1(struct xt_xlate *xl,
                                  const struct xt_xlate_mt_params *params) {
    const struct xt_hashlimit_mtinfo1 *info = (const void *)params->match->data;
    struct hashlimit_cfg2 cfg;
...
static int hashlimit_mt4_xlate(struct xt_xlate *xl,
                               const struct xt_xlate_mt_params *params) {
    const struct xt_hashlimit_mtinfo2 *info = (const void *)params->match->data;
