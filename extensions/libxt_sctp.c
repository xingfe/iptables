...
static void print_chunk(uint32_t chunknum, int numeric);
...
static void sctp_help(void) {
    printf("sctp match options\n"
           "[!] --source-port port[:port]                          match "
           "source port(s)\n"
           " --sport ...\n"
           "[!] --destination-port port[:port]                     match "
           "destination port(s)\n"
           " --dport ...\n"
           "[!] --chunk-types (all|any|none) (chunktype[:flags])+	match "
           "if all, any or none of\n"
           "						        chunktypes are "
           "present\n"
           "chunktypes - DATA INIT INIT_ACK SACK HEARTBEAT HEARTBEAT_ACK ABORT "
           "SHUTDOWN SHUTDOWN_ACK ERROR COOKIE_ECHO COOKIE_ACK ECN_ECNE "
           "ECN_CWR SHUTDOWN_COMPLETE ASCONF ASCONF_ACK FORWARD_TSN ALL "
           "NONE\n");
}
...
static void parse_sctp_ports(const char *portstring, uint16_t *ports) {
    char *buffer;
    char *cp;
    ...
    if ((cp = strchr(buffer, ':')) == NULL) {
        ...
        *cp = '\0';
        ...
        if (ports[0] > ports[1])
            xtables_error(PARAMETER_PROBLEM, "invalid portrange (min > max)");
    }
    free(buffer);
}
...
static void save_chunk_flag_info(struct xt_sctp_flag_info *flag_info,
                                 int *flag_count, int chunktype, int bit,
                                 int set) {
    int i;
...
static void parse_sctp_chunk(struct xt_sctp_info *einfo, const char *chunks) {
    char *ptr;
    char *buffer;
    unsigned int i, j;
    int found = 0;
    ...
    for (ptr = strtok(buffer, ","); ptr; ptr = strtok(NULL, ",")) {
        ...
        if (!found)
            xtables_error(PARAMETER_PROBLEM, "Unknown sctp chunk `%s'", ptr);
        ...
        if (chunk_flags) {
            ...
            for (j = 0; j < strlen(chunk_flags); j++) {
                ...
                if ((p = strchr(sctp_chunk_names[i].valid_flags,
                    ...
                    save_chunk_flag_info(einfo->flag_info, &(einfo->flag_count),
                                         i, bit, isupper(chunk_flags[j]));
                } else {
...
static void parse_sctp_chunks(struct xt_sctp_info *einfo,
                              const char *match_type, const char *chunks) {
...
static int sctp_parse(int c, char **argv, int invert, unsigned int *flags,
                      const void *entry, struct xt_entry_match **match) {
    struct xt_sctp_info *einfo = (struct xt_sctp_info *)(*match)->data;
    ...
    case '3':
        ...
        if (!argv[optind] || argv[optind][0] == '-' || argv[optind][0] == '!')
            xtables_error(PARAMETER_PROBLEM, "--chunk-types requires two args");
...
static const char *port_to_service(int port) {
...
static void print_port(uint16_t port, int numeric) {
    const char *service;
...
static void print_ports(const char *name, uint16_t min, uint16_t max,
...
static void print_chunk_flags(uint32_t chunknum, uint8_t chunk_flags,
                              uint8_t chunk_flags_mask) {
    int i;
    ...
    for (i = 7; i >= 0; i--) {
        if (chunk_flags_mask & (1 << i)) {
            if (chunk_flags & (1 << i)) {
                ...
                printf("%c",
                       tolower(sctp_chunk_names[chunknum].valid_flags[7 - i]));
            }
        }
    }
}
...
static void print_chunks(const struct xt_sctp_info *einfo, int numeric) {
...
static void sctp_print(const void *ip, const struct xt_entry_match *match,
                       int numeric) {
    const struct xt_sctp_info *einfo = (const struct xt_sctp_info *)match->data;

    printf(" sctp");

    if (einfo->flags & XT_SCTP_SRC_PORTS) {
        print_ports("spt", einfo->spts[0], einfo->spts[1],
                    einfo->invflags & XT_SCTP_SRC_PORTS, numeric);
    }

    if (einfo->flags & XT_SCTP_DEST_PORTS) {
        print_ports("dpt", einfo->dpts[0], einfo->dpts[1],
                    einfo->invflags & XT_SCTP_DEST_PORTS, numeric);
    }
...
static void sctp_save(const void *ip, const struct xt_entry_match *match) {
    ...
    if (einfo->flags & XT_SCTP_SRC_PORTS) {
        ...
        if (einfo->spts[0] != einfo->spts[1])
            printf(" --sport %u:%u", einfo->spts[0], einfo->spts[1]);
        else
    ...
    if (einfo->flags & XT_SCTP_DEST_PORTS) {
        ...
        if (einfo->dpts[0] != einfo->dpts[1])
            printf(" --dport %u:%u", einfo->dpts[0], einfo->dpts[1]);
        else
...
void _init(void) { xtables_register_match(&sctp_match); }
