...
static uint32_t parse_opts_num(const char *idstr, const char *typestr) {
    ...
    if (idstr == ep) {
        xtables_error(PARAMETER_PROBLEM, "hbh: no valid digits in %s `%s'",
                      typestr, idstr);
    }
    if (id == ULONG_MAX && errno == ERANGE) {
        xtables_error(PARAMETER_PROBLEM,
                      "%s `%s' specified too big: would overflow", typestr,
                      idstr);
    }
    if (*idstr != '\0' && *ep != '\0') {
        xtables_error(PARAMETER_PROBLEM, "hbh: error parsing %s `%s'", typestr,
                      idstr);
    }
    return id;
}

static int parse_options(const char *optsstr, uint16_t *opts) {
...
static void print_options(unsigned int optsnr, uint16_t *optsp) {
    unsigned int i;
...
static int hbh_xlate(struct xt_xlate *xl,
    ...
    const struct ip6t_opts *optinfo = (struct ip6t_opts *)params->match->data;

    if (!(optinfo->flags & IP6T_OPTS_LEN) || (optinfo->flags & IP6T_OPTS_OPTS))
        return 0;
...
void _init(void) { xtables_register_match(&hbh_mt6_reg); }
