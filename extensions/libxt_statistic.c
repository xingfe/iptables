...
static void statistic_help(void) {
    printf("statistic match options:\n"
           " --mode mode                    Match mode (random, nth)\n"
           " random mode:\n"
           "[!] --probability p		 Probability\n"
           " nth mode:\n"
           "[!] --every n			 Match every nth packet\n"
           " --packet p			 Initial counter value (0 <= p "
           "<= n-1, default 0)\n");
}

#define s struct xt_statistic_info
static const struct xt_option_entry statistic_opts[] = {
    {.name = "mode", .id = O_MODE, .type = XTTYPE_STRING, .flags = XTOPT_MAND},
    {.name = "probability",
     .id = O_PROBABILITY,
     .type = XTTYPE_DOUBLE,
     .flags = XTOPT_INVERT,
...
static void statistic_parse(struct xt_option_call *cb) {
    ...
    case O_MODE:
        if (strcmp(cb->arg, "random") == 0)
            ...
            xtables_error(PARAMETER_PROBLEM, "Bad mode \"%s\"", cb->arg);
        break;
    case O_PROBABILITY:
...
static void statistic_check(struct xt_fcheck_call *cb) {
    ...
    if (info->mode == XT_STATISTIC_MODE_RANDOM && !(cb->xflags & F_PROBABILITY))
...
static void print_match(const struct xt_statistic_info *info, char *prefix) {
    ...
    case XT_STATISTIC_MODE_RANDOM:
        printf(" %smode random%s %sprobability %.11f", prefix,
               (info->flags & XT_STATISTIC_INVERT) ? " !" : "", prefix,
...
static void statistic_print(const void *ip, const struct xt_entry_match *match,
                            int numeric) {
...
static int statistic_xlate(struct xt_xlate *xl,
    ...
    case XT_STATISTIC_MODE_RANDOM:
        return 0;
    case XT_STATISTIC_MODE_NTH:
        xt_xlate_add(xl, "numgen inc mod %u %s%u", info->u.nth.every + 1,
                     info->flags & XT_STATISTIC_INVERT ? "!= " : "",
                     info->u.nth.packet);
        break;
    }
