...
static const struct xt_option_entry idletimer_tg_opts[] = {
    ...
    {.name = "label",
     ...
     .type = XTTYPE_STRING,
     .flags = XTOPT_MAND | XTOPT_PUT,
     XTOPT_POINTER(s, label)},
    XTOPT_TABLEEND,
};
#undef s

static void idletimer_tg_help(void) {
    printf("IDLETIMER target options:\n"
           " --timeout time	Timeout until the notification is sent (in "
           "seconds)\n"
...
static void idletimer_tg_print(const void *ip,
                               ...
                               int numeric) {
    struct idletimer_tg_info *info = (struct idletimer_tg_info *)target->data;
...
void _init(void) { xtables_register_target(&idletimer_tg_reg); }
