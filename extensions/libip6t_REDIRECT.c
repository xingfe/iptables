...
static void REDIRECT_help(void) {
    printf("REDIRECT target options:\n"
           " --to-ports <port>[-<port>]\n"
...
/* Parses ports */
static void parse_ports(const char *arg, struct nf_nat_range *range) {
    char *end = "";
...
static void REDIRECT_save(const void *ip,
...
static int REDIRECT_xlate(struct xt_xlate *xl,
    ...
    if (range->flags & NF_NAT_RANGE_PROTO_SPECIFIED) {
        xt_xlate_add(xl, "redirect to :%hu", ntohs(range->min_proto.tcp.port));
        if (range->max_proto.tcp.port != range->min_proto.tcp.port)
...
void _init(void) { xtables_register_target(&redirect_tg_reg); }
