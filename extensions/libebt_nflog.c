...
static int brnflog_parse(int c, char **argv, int invert, unsigned int *flags,
    ...
    if (invert)
        xtables_error(PARAMETER_PROBLEM, "The use of '!' makes no sense for the"
                                         " nflog watcher");
    ...
    case NFLOG_GROUP:
        ...
        if (!xtables_strtoui(optarg, NULL, &i, 1, UINT32_MAX))
            xtables_error(PARAMETER_PROBLEM, "--nflog-group must be a number!");
        info->group = i;
        break;
    case NFLOG_RANGE:
        EBT_CHECK_OPTION(flags, NFLOG_RANGE);
        if (!xtables_strtoui(optarg, NULL, &i, 1, UINT32_MAX))
            xtables_error(PARAMETER_PROBLEM, "--nflog-range must be a number!");
        info->len = i;
        break;
...
static void brnflog_print(const void *ip, const struct xt_entry_target *target,
                          int numeric) {
    struct ebt_nflog_info *info = (struct ebt_nflog_info *)target->data;
...
void _init(void) { xtables_register_target(&brnflog_watcher); }
