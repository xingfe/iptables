...
static const struct xt_option_entry LED_opts[] = {
    {.name = "led-trigger-id",
     ...
     .type = XTTYPE_STRING,
     .min = 0,
     .max = sizeof(((struct xt_led_info *)NULL)->id) - sizeof("netfilter-")},
    {.name = "led-delay", .id = O_LED_DELAY, .type = XTTYPE_STRING},
    {.name = "led-always-blink", .id = O_LED_ALWAYS_BLINK, .type = XTTYPE_NONE},
    XTOPT_TABLEEND,
};
#undef s
    ...
    case O_LED_DELAY:
        if (strncasecmp(cb->arg, "inf", 3) == 0)
            ...
            xtables_error(PARAMETER_PROBLEM,
                          "Delay value must be within range 0..%u", UINT32_MAX);
        break;
...
void _init(void) { xtables_register_target(&led_tg_reg); }
