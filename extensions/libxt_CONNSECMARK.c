...
static void CONNSECMARK_help(void) {
    printf("CONNSECMARK target options:\n"
           "  --save                   Copy security mark from packet to "
           "conntrack\n"
           "  --restore                Copy security mark from connection to "
           "packet\n");
}

static const struct xt_option_entry CONNSECMARK_opts[] = {
    {.name = "save", .id = O_SAVE, .excl = F_RESTORE, .type = XTTYPE_NONE},
    {.name = "restore", .id = O_RESTORE, .excl = F_SAVE, .type = XTTYPE_NONE},
    XTOPT_TABLEEND,
};
...
static void CONNSECMARK_print(const void *ip,
...
static void CONNSECMARK_save(const void *ip,
                             const struct xt_entry_target *target) {
    const struct xt_connsecmark_target_info *info =
        (struct xt_connsecmark_target_info *)target->data;
...
void _init(void) { xtables_register_target(&connsecmark_target); }
