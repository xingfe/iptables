...
static const struct numflag chain_flags[] = {
    {IPPROTO_HOPOPTS, MASK_HOPOPTS}, {IPPROTO_DSTOPTS, MASK_DSTOPTS},
    {IPPROTO_ROUTING, MASK_ROUTING}, {IPPROTO_FRAGMENT, MASK_FRAGMENT},
    {IPPROTO_AH, MASK_AH},           {IPPROTO_ESP, MASK_ESP},
    {IPPROTO_NONE, MASK_NONE},       {IPPROTO_RAW, MASK_PROTO},
};

static const char *proto_to_name(uint8_t proto, int nolookup) {
    unsigned int i;
...
static uint16_t name_to_proto(const char *s) {
    ...
    if ((pent = getprotobyname(s)))
        ...
        if (i == ARRAY_SIZE(chain_protos))
            xtables_error(PARAMETER_PROBLEM, "unknown header `%s' specified",
                          s);
    }
...
static unsigned int add_proto_to_mask(int proto) {
    ...
    if (i == ARRAY_SIZE(chain_flags))
        xtables_error(PARAMETER_PROBLEM, "unknown header `%d' specified",
                      proto);
...
static void ipv6header_help(void) {
    printf(
        ...
        "--soft                    The header CONTAINS the specified "
        "extensions\n");
}
...
static unsigned int parse_header(const char *flags) {
    unsigned int ret = 0;
    char *ptr;
    char *buffer;
...
static void print_header(uint8_t flags) {
    int have_flag = 0;
...
static void ipv6header_print(const void *ip, const struct xt_entry_match *match,
                             int numeric) {
    const struct ip6t_ipv6header_info *info =
        (const struct ip6t_ipv6header_info *)match->data;
    printf(" ipv6header");
...
static void ipv6header_save(const void *ip,
...
void _init(void) { xtables_register_match(&ipv6header_mt6_reg); }
