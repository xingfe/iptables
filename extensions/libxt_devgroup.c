...
static void devgroup_help(void) {
    printf(
        ...
        "[!] --dst-group value[/mask]	Match device group of outgoing "
        "device\n");
}
...
static void devgroup_parse_groupspec(const char *arg, unsigned int *group,
    ...
    if (ok && (*end == '/' || *end == '\0')) {
        if (*end == '/')
            ok = xtables_strtoui(end + 1, NULL, mask, 0, UINT32_MAX);
        else
            *mask = ~0U;
        if (!ok)
            xtables_error(PARAMETER_PROBLEM, "Bad group value \"%s\"", arg);
    } else {
        *group = xtables_lmap_name2id(devgroups, arg);
        if (*group == -1)
            xtables_error(PARAMETER_PROBLEM, "Device group \"%s\" not found",
                          arg);
        *mask = ~0U;
    }
}
...
static void print_devgroup(unsigned int id, unsigned int mask, int numeric) {
...
static void print_devgroup_xlate(unsigned int id, uint32_t op,
                                 unsigned int mask, struct xt_xlate *xl,
                                 int numeric) {
    const char *name = NULL;
...
static void devgroup_show_xlate(const struct xt_devgroup_info *info,
    ...
    if (info->flags & XT_DEVGROUP_MATCH_SRC) {
        ...
        print_devgroup_xlate(info->src_group, op, info->src_mask, xl, numeric);
        space = " ";
    }
    ...
    if (info->flags & XT_DEVGROUP_MATCH_DST) {
        ...
        print_devgroup_xlate(info->dst_group, op, info->dst_mask, xl, numeric);
    }
}
...
void _init(void) { xtables_register_match(&devgroup_mt_reg); }
