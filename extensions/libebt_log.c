...
static int brlog_parse(int c, char **argv, int invert, unsigned int *flags,
    ...
    case LOG_PREFIX:
        ...
        if (strlen(optarg) > sizeof(loginfo->prefix) - 1)
            xtables_error(PARAMETER_PROBLEM, "Prefix too long");
        if (strchr(optarg, '\"'))
            xtables_error(PARAMETER_PROBLEM, "Use of \\\" is not allowed"
                                             " in the prefix");
    ...
    case LOG_LEVEL:
        ...
        if (loginfo->loglevel == 9)
            xtables_error(PARAMETER_PROBLEM, "Problem with the log-level");
        break;
    case LOG_IP:
        if (invert)
            xtables_error(PARAMETER_PROBLEM, "Unexpected `!' after --log-ip");
        loginfo->bitmask |= EBT_LOG_IP;
        break;
    case LOG_ARP:
        if (invert)
            xtables_error(PARAMETER_PROBLEM, "Unexpected `!' after --log-arp");
    ...
    case LOG_IP6:
        if (invert)
            xtables_error(PARAMETER_PROBLEM, "Unexpected `!' after --log-ip6");
...
static void brlog_print(const void *ip, const struct xt_entry_target *target,
    ...
    printf("--log-level %s --log-prefix \"%s\"",
           eight_priority[loginfo->loglevel].c_name, loginfo->prefix);
...
void _init(void) { xtables_register_target(&brlog_target); }
