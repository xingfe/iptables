...
static void esp_print(const void *ip, const struct xt_entry_match *match,
                      int numeric) {
    ...
    if (esp->invflags & ~XT_ESP_INV_MASK)
        printf(" Unknown invflags: 0x%X", esp->invflags & ~XT_ESP_INV_MASK);
}
...
static void esp_save(const void *ip, const struct xt_entry_match *match) {
    ...
    if (!(espinfo->spis[0] == 0 && espinfo->spis[1] == 0xFFFFFFFF)) {
        ...
        if (espinfo->spis[0] != espinfo->spis[1])
            printf("%u:%u", espinfo->spis[0], espinfo->spis[1]);
        else
            printf("%u", espinfo->spis[0]);
    }
}
...
static int esp_xlate(struct xt_xlate *xl,
    ...
    if (!(espinfo->spis[0] == 0 && espinfo->spis[1] == 0xFFFFFFFF)) {
        xt_xlate_add(xl, "esp spi%s",
                     (espinfo->invflags & XT_ESP_INV_SPI) ? " !=" : "");
        if (espinfo->spis[0] != espinfo->spis[1])
            xt_xlate_add(xl, " %u-%u", espinfo->spis[0], espinfo->spis[1]);
        else
...
void _init(void) { xtables_register_match(&esp_match); }
