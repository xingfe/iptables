...
static void brip_print_help(void) {
    printf("ip options:\n"
           ...
           "--ip-dport  [!] port[:port]   : tcp/udp destination port or port "
           "range\n");
}
...
static void parse_port_range(const char *protocol, const char *portstring,
                             uint16_t *ports) {
    char *buffer;
    char *cp;
...
static void ebt_parse_ip_address(char *address, uint32_t *addr, uint32_t *msk) {
    ...
    if ((p = strrchr(address, '/')) != NULL) {
        *p = '\0';
        if (ip_mask(p + 1, (unsigned char *)msk)) {
            xtables_error(PARAMETER_PROBLEM, "Problem with the IP mask '%s'",
                          p + 1);
            return;
        }
    } else
    ...
    if (undot_ip(address, (unsigned char *)addr)) {
        xtables_error(PARAMETER_PROBLEM, "Problem with the IP address '%s'",
                      address);
        return;
    }
    *addr = *addr & *msk;
...
static int brip_parse(int c, char **argv, int invert, unsigned int *flags,
    ...
    case IP_EBT_TOS:
        ...
        if (!xtables_strtoul(optarg, NULL, (uintmax_t *)&info->tos, 0, 255))
            xtables_error(PARAMETER_PROBLEM, "Problem with specified IP tos");
    ...
    case IP_PROTO:
        ...
        if (info->protocol == -1)
            xtables_error(PARAMETER_PROBLEM,
                          "Unknown specified IP protocol - %s", optarg);
...
static void brip_final_check(unsigned int flags) {
    if (!flags)
        xtables_error(PARAMETER_PROBLEM, "You must specify proper arguments");
}
