...
static void addrtype_help_v0(void) {
    printf("Address type match options:\n"
...
static void addrtype_help_v1(void) {
    printf("Address type match options:\n"
           " [!] --src-type type[,...]      Match source address type\n"
           " [!] --dst-type type[,...]      Match destination address type\n"
           "     --limit-iface-in           Match only on the packet's "
           "incoming device\n"
           "     --limit-iface-out          Match only on the packet's "
           "outgoing device\n"
           "\n"
           "Valid types:           \n");
    addrtype_help_types();
...
static int parse_type(const char *name, size_t len, uint16_t *mask) {
    int i;
...
static void addrtype_print_v0(const void *ip,
...
static void addrtype_print_v1(const void *ip,
                              const struct xt_entry_match *match, int numeric) {
...
static void addrtype_save_v0(const void *ip,
...
static void addrtype_save_v1(const void *ip,
                             const struct xt_entry_match *match) {
    const struct xt_addrtype_info_v1 *info = (const void *)match->data;
...
static const char *const rtn_lnames[] = {
    "unspec",    "unicast",   "local",       "broadcast", "anycast",
    "multicast", "blackhole", "unreachable", "prohibit",  NULL,
};
...
static int addrtype_xlate(struct xt_xlate *xl,
    ...
    const struct xt_addrtype_info_v1 *info = (const void *)params->match->data;
    const char *sep = "";
    bool need_braces;
    uint16_t val;
    int i;
...
static const struct xt_option_entry addrtype_opts_v1[] = {
    ...
    {.name = "limit-iface-in",
     .id = O_LIMIT_IFACE_IN,
     .type = XTTYPE_NONE,
     .excl = F_LIMIT_IFACE_OUT},
    {.name = "limit-iface-out",
     .id = O_LIMIT_IFACE_OUT,
     .type = XTTYPE_NONE,
     .excl = F_LIMIT_IFACE_IN},
    XTOPT_TABLEEND,
};
