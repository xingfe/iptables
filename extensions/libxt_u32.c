    ...
    for (testind = 0; testind < data->ntests; ++testind) {
        ...
        for (i = 0; i < ct->nvalues; ++i) {
            ...
            if (ct->value[i].min == ct->value[i].max)
                ...
                printf("0x%x:0x%x", ct->value[i].min, ct->value[i].max);
        }
    }
    putchar('\"');
}
...
static uint32_t parse_number(const char **s, int pos) {
    ...
    if (!xtables_strtoui(*s, &end, &number, 0, UINT32_MAX) || end == *s)
    ...
    while (1) {
        ...
        if (state == 0) {
                ...
                if (locind != 0) {
                    ...
                    if (*arg == '&') {
                        ...
                        if (*++arg != '<')
                            xtables_error(
                                PARAMETER_PROBLEM,
                                "u32: at char %u: a second '<' was expected",
                                (unsigned int)(arg - start));
                        ...
                        xtables_error(PARAMETER_PROBLEM,
                                      "u32: at char %u: operator expected",
                                      (unsigned int)(arg - start));
                    }
                    ++arg;
                }
                /* now a number; string_to_number skips white space? */
                ct->location[locind].number = parse_number(&arg, arg - start);
            ...
            if (*arg == '&') {
                ...
                ct->value[valind].min = parse_number(&arg, arg - start);
                ...
                if (*arg == ':') {
                    ++arg;
                    ct->value[valind].max = parse_number(&arg, arg - start);
                } else {
                    ct->value[valind].max = ct->value[valind].min;
                }
...
void _init(void) { xtables_register_match(&u32_match); }
