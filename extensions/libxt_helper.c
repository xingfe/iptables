...
static void helper_print(const void *ip, const struct xt_entry_match *match,
                         int numeric) {
...
static int helper_xlate(struct xt_xlate *xl,
    ...
    if (params->escape_quotes)
        xt_xlate_add(xl, "ct helper%s \\\"%s\\\"", info->invert ? " !=" : "",
                     info->name);
    else
        xt_xlate_add(xl, "ct helper%s \"%s\"", info->invert ? " !=" : "",
                     info->name);
...
void _init(void) { xtables_register_match(&helper_match); }
