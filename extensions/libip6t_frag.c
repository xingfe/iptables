...
static void print_ids(const char *name, uint32_t min, uint32_t max,
                      int invert) {
...
static void frag_print(const void *ip, const struct xt_entry_match *match,
    ...
    if (frag->flags & IP6T_FRAG_LEN) {
        printf(" length:%s%u", frag->invflags & IP6T_FRAG_INV_LEN ? "!" : "",
               frag->hdrlen);
    }
    ...
    if (frag->invflags & ~IP6T_FRAG_INV_MASK)
        printf(" Unknown invflags: 0x%X", frag->invflags & ~IP6T_FRAG_INV_MASK);
}
...
static void frag_save(const void *ip, const struct xt_entry_match *match) {
    ...
    if (!(fraginfo->ids[0] == 0 && fraginfo->ids[1] == 0xFFFFFFFF)) {
        ...
        if (fraginfo->ids[0] != fraginfo->ids[1])
            printf("%u:%u", fraginfo->ids[0], fraginfo->ids[1]);
        else
            printf("%u", fraginfo->ids[0]);
    }
...
static int frag_xlate(struct xt_xlate *xl,
    ...
    const struct ip6t_frag *fraginfo = (struct ip6t_frag *)params->match->data;
    char *space = "";
    ...
    if (!(fraginfo->ids[0] == 0 && fraginfo->ids[1] == 0xFFFFFFFF)) {
        xt_xlate_add(xl, "frag id %s",
                     (fraginfo->invflags & IP6T_FRAG_INV_IDS) ? "!= " : "");
        if (fraginfo->ids[0] != fraginfo->ids[1])
            xt_xlate_add(xl, "%u-%u", fraginfo->ids[0], fraginfo->ids[1]);
        else
...
void _init(void) { xtables_register_match(&frag_mt6_reg); }
