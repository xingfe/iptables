...
static int brmark_parse(int c, char **argv, int invert, unsigned int *flags,
    ...
    case MARK_TARGET: {
        ...
        if (ebt_fill_target(optarg, &tmp))
            xtables_error(PARAMETER_PROBLEM, "Illegal --mark-target target");
        /* the 4 lsb are left to designate the target */
        info->target =
            (info->target & ~EBT_VERDICT_BITS) | (tmp & EBT_VERDICT_BITS);
    }
        return 1;
    case MARK_SETMARK:
        ...
        info->target = (info->target & EBT_VERDICT_BITS) | MARK_SET_VALUE;
        break;
    case MARK_ORMARK:
        ...
        if (*flags & mask)
            xtables_error(PARAMETER_PROBLEM,
                          ...
                          " specific --mark option");
        info->target = (info->target & EBT_VERDICT_BITS) | MARK_OR_VALUE;
        break;
    case MARK_ANDMARK:
        ...
        if (*flags & mask)
            xtables_error(PARAMETER_PROBLEM,
                          ...
                          " specific --mark option");
        info->target = (info->target & EBT_VERDICT_BITS) | MARK_AND_VALUE;
        break;
    case MARK_XORMARK:
        ...
        if (*flags & mask)
            xtables_error(PARAMETER_PROBLEM,
                          ...
                          " specific --mark option");
        info->target = (info->target & EBT_VERDICT_BITS) | MARK_XOR_VALUE;
        break;
    default:
        return 0;
    }
...
static void brmark_final_check(unsigned int flags) {
    ...
    if (!flags)
        xtables_error(PARAMETER_PROBLEM, "You must specify some option");
}
...
void _init(void) { xtables_register_target(&brmark_target); }
