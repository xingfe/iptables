...
static void HMARK_help(void) {
    printf(
        ...
        " Alternatively, fine tuning of what will be included in hash "
        "calculation\n"
...
#define HMARK_OPT_PKT_MASK                                                     \
    ((1 << O_HMARK_SADDR_MASK) | (1 << O_HMARK_DADDR_MASK) |                   \
     (1 << O_HMARK_SPI_MASK) | (1 << O_HMARK_SPORT_MASK) |                     \
     (1 << O_HMARK_DPORT_MASK) | (1 << O_HMARK_PROTO_MASK) |                   \
     (1 << O_HMARK_SPI_MASK) | (1 << O_HMARK_SPORT) | (1 << O_HMARK_DPORT) |   \
     (1 << O_HMARK_SPI))

static const struct xt_option_entry HMARK_opts[] = {
    {
        .name = "hmark-tuple", .type = XTTYPE_STRING, .id = O_HMARK_TYPE,
    },
...
static int hmark_parse(const char *type, size_t len, struct xt_hmark_info *info,
                       unsigned int *xflags) {
    if (strncasecmp(type, "ct", len) == 0) {
        ...
        *xflags |= (1 << O_HMARK_DADDR_MASK);
        ...
        memset(&info->port_mask.p16.src, 0xff, sizeof(info->port_mask.p16.src));
        info->flags |= XT_HMARK_FLAG(XT_HMARK_SPORT_MASK);
        *xflags |= (1 << O_HMARK_SPORT_MASK);
    } else if (strncasecmp(type, "dport", len) == 0) {
        memset(&info->port_mask.p16.dst, 0xff, sizeof(info->port_mask.p16.dst));
...
static void hmark_parse_type(struct xt_option_call *cb) {
    const char *arg = cb->arg;
    struct xt_hmark_info *info = cb->data;
    const char *comma;

    while ((comma = strchr(arg, ',')) != NULL) {
        if (comma == arg || !hmark_parse(arg, comma - arg, info, &cb->xflags))
    ...
    if (strlen(arg) == 0 || !hmark_parse(arg, strlen(arg), info, &cb->xflags))
        xtables_error(PARAMETER_PROBLEM, "Bad type \"%s\"", arg);
}
...
static void HMARK_ip4_parse(struct xt_option_call *cb) { HMARK_parse(cb, 32); }
static void HMARK_ip6_parse(struct xt_option_call *cb) { HMARK_parse(cb, 128); }
...
static void HMARK_check(struct xt_fcheck_call *cb) {
    ...
    if (cb->xflags & (1 << O_HMARK_SPI_MASK) &&
        (cb->xflags & ((1 << O_HMARK_SPORT_MASK) | (1 << O_HMARK_DPORT_MASK))))
        xtables_error(PARAMETER_PROBLEM,
                      "you cannot use "
                      "--hmark-spi-mask and --hmark-?port-mask,"
                      "at the same time");
    if (!((cb->xflags & HMARK_OPT_PKT_MASK) || cb->xflags & (1 << O_HMARK_CT)))
...
static void HMARK_ip4_print(const void *ip,
    ...
    if (info->flags & XT_HMARK_FLAG(XT_HMARK_CT))
        printf("ct, ");
    if (info->flags & XT_HMARK_FLAG(XT_HMARK_SADDR_MASK))
        printf("src-prefix %u ", xtables_ipmask_to_cidr(&info->src_mask.in));
    if (info->flags & XT_HMARK_FLAG(XT_HMARK_DADDR_MASK))
        printf("dst-prefix %u ", xtables_ipmask_to_cidr(&info->dst_mask.in));
    HMARK_print(info);
}

static void HMARK_save(const struct xt_hmark_info *info) {
    if (info->flags & XT_HMARK_FLAG(XT_HMARK_SPORT_MASK))
        printf(" --hmark-sport-mask 0x%04x", htons(info->port_mask.p16.src));
    if (info->flags & XT_HMARK_FLAG(XT_HMARK_DPORT_MASK))
        printf(" --hmark-dport-mask 0x%04x", htons(info->port_mask.p16.dst));
    if (info->flags & XT_HMARK_FLAG(XT_HMARK_SPI_MASK))
        printf(" --hmark-spi-mask 0x%08x", htonl(info->port_mask.v32));
    if (info->flags & XT_HMARK_FLAG(XT_HMARK_SPORT))
        printf(" --hmark-sport 0x%04x", htons(info->port_set.p16.src));
    if (info->flags & XT_HMARK_FLAG(XT_HMARK_DPORT))
        printf(" --hmark-dport 0x%04x", htons(info->port_set.p16.dst));
    if (info->flags & XT_HMARK_FLAG(XT_HMARK_SPI))
...
static void HMARK_ip6_save(const void *ip,
...
static void HMARK_ip4_save(const void *ip,
                           const struct xt_entry_target *target) {
    const struct xt_hmark_info *info =
        (const struct xt_hmark_info *)target->data;
    int ret;
