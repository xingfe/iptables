...
static uint32_t parse_opts_num(const char *idstr, const char *typestr) {
    ...
    if (idstr == ep) {
        xtables_error(PARAMETER_PROBLEM, "dst: no valid digits in %s `%s'",
                      typestr, idstr);
    }
    ...
    if (*idstr != '\0' && *ep != '\0') {
        xtables_error(PARAMETER_PROBLEM, "dst: error parsing %s `%s'", typestr,
                      idstr);
    }
    return id;
}
...
static int parse_options(const char *optsstr, uint16_t *opts) {
    ...
    for (cp = buffer, i = 0; cp && i < IP6T_OPTS_OPTSNR; cp = next, i++) {
        ...
        if (range) {
            if (opts[i] == 0)
                xtables_error(PARAMETER_PROBLEM, "PAD0 hasn't got length");
...
static void dst_print(const void *ip, const struct xt_entry_match *match,
    ...
    if (optinfo->flags & IP6T_OPTS_LEN)
        printf(" length:%s%u", optinfo->invflags & IP6T_OPTS_INV_LEN ? "!" : "",
               optinfo->hdrlen);
...
void _init(void) { xtables_register_match(&dst_mt6_reg); }
