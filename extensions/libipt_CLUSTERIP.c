...
enum {
    ...
    F_FULL = F_NEW | F_HASHMODE | F_CLUSTERMAC | F_TOTAL_NODES | F_LOCAL_NODE,
};
...
static const struct xt_option_entry CLUSTERIP_opts[] = {
    ...
    {.name = "clustermac",
     .id = O_CLUSTERMAC,
     .type = XTTYPE_ETHERMAC,
     .also = O_NEW,
     .flags = XTOPT_PUT,
     XTOPT_POINTER(s, clustermac)},
    {.name = "total-nodes",
     .id = O_TOTAL_NODES,
     .type = XTTYPE_UINT16,
     .flags = XTOPT_PUT,
     XTOPT_POINTER(s, num_total_nodes),
     .also = O_NEW,
     .max = CLUSTERIP_MAX_NODES},
    {.name = "local-node",
     .id = O_LOCAL_NODE,
     .type = XTTYPE_UINT16,
     .flags = XTOPT_PUT,
     XTOPT_POINTER(s, local_nodes[0]),
     .also = O_NEW,
     .max = CLUSTERIP_MAX_NODES},
    {.name = "hash-init",
     .id = O_HASH_INIT,
...
static void CLUSTERIP_print(const void *ip,
    ...
    printf(" CLUSTERIP hashmode=%s clustermac=%s total_nodes=%u local_node=%u "
           "hash_init=%u",
           hashmode2str(cipinfo->hash_mode), mac2str(cipinfo->clustermac),
           cipinfo->num_total_nodes, cipinfo->local_nodes[0],
           cipinfo->hash_initval);
}

static void CLUSTERIP_save(const void *ip,
    ...
    printf(" --new --hashmode %s --clustermac %s --total-nodes %d --local-node "
           "%d --hash-init %u",
           hashmode2str(cipinfo->hash_mode), mac2str(cipinfo->clustermac),
           cipinfo->num_total_nodes, cipinfo->local_nodes[0],
           cipinfo->hash_initval);
