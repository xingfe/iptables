...
static void comment_print(const void *ip, const struct xt_entry_match *match,
                          int numeric) {
...
static void comment_save(const void *ip, const struct xt_entry_match *match) {
    struct xt_comment_info *commentinfo = (void *)match->data;
...
static int comment_xlate(struct xt_xlate *xl,
    ...
    if (params->escape_quotes)
        ...
        snprintf(comment, XT_MAX_COMMENT_LEN, "\"%s\"", commentinfo->comment);
...
void _init(void) { xtables_register_match(&comment_match); }
