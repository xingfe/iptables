...
static void limit_help(void) {
    printf(
        "limit match options:\n"
        "--limit avg			max average match rate: "
        "default " XT_LIMIT_AVG "\n"
        "                                [Packets per second unless followed "
        "by \n"
        "                                /sec /minute /hour /day postfixes]\n"
        "--limit-burst number		number to match in a burst, default "
        "%u\n",
        XT_LIMIT_BURST);
}
...
static const struct xt_option_entry limit_opts[] = {
    ...
    {.name = "limit-burst",
     ...
     .type = XTTYPE_UINT32,
     .flags = XTOPT_PUT,
     XTOPT_POINTER(struct xt_rateinfo, burst),
     .min = 0,
     .max = 10000},
    XTOPT_TABLEEND,
};
...
static void limit_parse(struct xt_option_call *cb) {
    ...
    case O_LIMIT:
        if (!parse_rate(cb->arg, &r->avg))
            xtables_error(PARAMETER_PROBLEM, "bad rate \"%s\"'", cb->arg);
        break;
    }
    if (cb->invert)
        xtables_error(PARAMETER_PROBLEM, "limit does not support invert");
}
...
static void limit_print(const void *ip, const struct xt_entry_match *match,
                        int numeric) {
...
void _init(void) { xtables_register_match(&limit_match); }
