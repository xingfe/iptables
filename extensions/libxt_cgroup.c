...
static void cgroup_help_v0(void) {
    printf("cgroup match options:\n"
           "[!] --cgroup classid            Match cgroup classid\n");
}

static void cgroup_help_v1(void) {
    printf("cgroup match options:\n"
           "[!] --path path                 Recursively match path relative to "
           "cgroup2 root\n"
           "[!] --cgroup classid            Match cgroup classid, can't be "
           "used with --path\n");
}
...
static void cgroup_print_v0(const void *ip, const struct xt_entry_match *match,
                            int numeric) {
...
static void cgroup_print_v1(const void *ip, const struct xt_entry_match *match,
                            int numeric) {
...
static int cgroup_xlate_v0(struct xt_xlate *xl,
    ...
    xt_xlate_add(xl, "meta cgroup %s%u", info->invert ? "!= " : "", info->id);
    return 1;
}
...
static int cgroup_xlate_v1(struct xt_xlate *xl,
    ...
    if (info->has_classid)
        xt_xlate_add(xl, "meta cgroup %s%u", info->invert_classid ? "!= " : "",
                     info->classid);
