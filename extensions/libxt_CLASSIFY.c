...
static void CLASSIFY_help(void) {
    printf("CLASSIFY target options:\n"
           "--set-class MAJOR:MINOR    Set skb->priority value (always "
           "hexadecimal!)\n");
}
...
static void CLASSIFY_parse(struct xt_option_call *cb) {
    ...
    if (CLASSIFY_string_to_priority(cb->arg, &clinfo->priority))
        xtables_error(PARAMETER_PROBLEM, "Bad class value \"%s\"", cb->arg);
}

static void CLASSIFY_print_class(unsigned int priority, int numeric) {
    printf(" %x:%x", TC_H_MAJ(priority) >> 16, TC_H_MIN(priority));
}

static void CLASSIFY_print(const void *ip, const struct xt_entry_target *target,
                           int numeric) {
...
static void CLASSIFY_save(const void *ip,
                          const struct xt_entry_target *target) {
    const struct xt_classify_target_info *clinfo =
        (const struct xt_classify_target_info *)target->data;

    printf(" --set-class %.4x:%.4x", TC_H_MAJ(clinfo->priority) >> 16,
           TC_H_MIN(clinfo->priority));
}

static void arpCLASSIFY_print(const void *ip,
                              const struct xt_entry_target *target,
                              int numeric) {
...
static int CLASSIFY_xlate(struct xt_xlate *xl,
    ...
    case TC_H_UNSPEC:
        ...
        break;
    default:
        xt_xlate_add(xl, "%0x:%0x", TC_H_MAJ(handle) >> 16, TC_H_MIN(handle));
        break;
    }
