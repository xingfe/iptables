...
static void connbytes_help(void) {
    printf("connbytes match options:\n"
           " [!] --connbytes from:[to]\n"
...
static void connbytes_parse(struct xt_option_call *cb) {
    ...
    case O_CONNBYTES_DIR:
        if (strcmp(cb->arg, "original") == 0)
            ...
            xtables_error(PARAMETER_PROBLEM, "Unknown --connbytes-dir `%s'",
                          cb->arg);
        break;
    case O_CONNBYTES_MODE:
        if (strcmp(cb->arg, "packets") == 0)
            ...
            xtables_error(PARAMETER_PROBLEM, "Unknown --connbytes-mode `%s'",
                          cb->arg);
        break;
    }
}
...
static void print_from_to(const struct xt_connbytes_info *sinfo,
                          const char *prefix) {
...
static void connbytes_print(const void *ip, const struct xt_entry_match *match,
                            int numeric) {
...
void _init(void) { xtables_register_match(&connbytes_match); }
