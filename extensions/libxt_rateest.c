...
static void rateest_help(void) {
    printf("rateest match options:\n"
           " --rateest1 name		Rate estimator name\n"
           " --rateest2 name		Rate estimator name\n"
           " --rateest-delta		Compare difference(s) to given "
           "rate(s)\n"
           " --rateest-bps1 [bps]		Compare bps\n"
           " --rateest-pps1 [pps]		Compare pps\n"
           " --rateest-bps2 [bps]		Compare bps\n"
           " --rateest-pps2 [pps]		Compare pps\n"
           " [!] --rateest-lt		Match if rate is less than given "
           "rate/estimator\n"
           " [!] --rateest-gt		Match if rate is greater than given "
           "rate/estimator\n"
           " [!] --rateest-eq		Match if rate is equal to given "
           "rate/estimator\n");
}
...
static const struct option rateest_opts[] = {
    ...
    {.name = "rateest",
     .has_arg = true,
     .val = OPT_RATEEST1}, /* alias for absolute mode */
    ...
    {.name = "rateest-bps",
     .has_arg = false,
     .val = OPT_RATEEST_BPS2}, /* alias for absolute mode */
    {.name = "rateest-pps",
     .has_arg = false,
     .val = OPT_RATEEST_PPS2}, /* alias for absolute mode */
...
static int rateest_get_rate(uint32_t *rate, const char *str) {
    char *p;
...
static int rateest_parse(int c, char **argv, int invert, unsigned int *flags,
    ...
    case OPT_RATEEST_BPS1:
        ...
        *flags |= 1 << c;
        ...
        if (rateest_get_rate(&info->bps1, argv[optind]) < 0)
            xtables_error(PARAMETER_PROBLEM,
                          "rateest: could not parse rate `%s'", argv[optind]);
        optind++;
        break;
    ...
    case OPT_RATEEST_PPS1:
        ...
        *flags |= 1 << c;
        ...
        if (!xtables_strtoui(argv[optind], NULL, &val, 0, UINT32_MAX))
            xtables_error(PARAMETER_PROBLEM,
                          "rateest: could not parse pps `%s'", argv[optind]);
        info->pps1 = val;
        optind++;
        break;
...
static void rateest_print_rate(uint32_t rate, int numeric) {
...
static void rateest_print_mode(const struct xt_rateest_match_info *info,
                               const char *prefix) {
...
static void rateest_print(const void *ip, const struct xt_entry_match *match,
                          int numeric) {
...
static void rateest_save(const void *ip, const struct xt_entry_match *match) {
    const struct xt_rateest_match_info *info = (const void *)match->data;
...
void _init(void) { xtables_register_match(&rateest_mt_reg); }
