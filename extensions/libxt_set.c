...
static void set_check_v0(unsigned int flags) {
    if (!flags)
...
static int set_parse_v0(int c, char **argv, int invert, unsigned int *flags,
    ...
    switch (c) {
    case '2':
        fprintf(stderr, "--set option deprecated, please use --match-set\n");
    case '1': /* --match-set <set> <flag>[,<flag> */
        ...
        if (!argv[optind] || argv[optind][0] == '-' || argv[optind][0] == '!')
            xtables_error(PARAMETER_PROBLEM, "--match-set requires two args.");

        if (strlen(optarg) > IPSET_MAXNAMELEN - 1)
            xtables_error(PARAMETER_PROBLEM,
                          "setname `%s' too long, max %d characters.", optarg,
                          IPSET_MAXNAMELEN - 1);
...
static void print_match_v0(const char *prefix,
                           const struct xt_set_info_v0 *info) {
    int i;
    char setname[IPSET_MAXNAMELEN];

    get_set_byid(setname, info->index);
    printf("%s %s %s", (info->u.flags[0] & IPSET_MATCH_INV) ? " !" : "", prefix,
           setname);
...
/* Revision 1 */
static int set_parse_v1(int c, char **argv, int invert, unsigned int *flags,
                        const void *entry, struct xt_entry_match **match) {
...
static void print_match(const char *prefix, const struct xt_set_info *info) {
    int i;
    char setname[IPSET_MAXNAMELEN];

    get_set_byid(setname, info->index);
    printf("%s %s %s", (info->flags & IPSET_INV_MATCH) ? " !" : "", prefix,
           setname);
...
/* Prints out the matchinfo. */
static void set_print_v1(const void *ip, const struct xt_entry_match *match,
                         int numeric) {
...
static void set_save_v1(const void *ip, const struct xt_entry_match *match) {
    const struct xt_set_info_match_v1 *info = (const void *)match->data;
...
static int set_parse_v2(int c, char **argv, int invert, unsigned int *flags,
                        const void *entry, struct xt_entry_match **match) {
    struct xt_set_info_match_v1 *myinfo =
        (struct xt_set_info_match_v1 *)(*match)->data;
    struct xt_set_info *info = &myinfo->match_set;
...
/* Prints out the matchinfo. */
static void set_print_v2(const void *ip, const struct xt_entry_match *match,
                         int numeric) {
    const struct xt_set_info_match_v1 *info = (const void *)match->data;
...
static void set_save_v2(const void *ip, const struct xt_entry_match *match) {
    const struct xt_set_info_match_v1 *info = (const void *)match->data;
...
static uint64_t parse_counter(const char *opt) {
    uintmax_t value;

    if (!xtables_strtoul(opt, NULL, &value, 0, UINT64_MAX))
        xtables_error(PARAMETER_PROBLEM, "Cannot parse %s as a counter value\n",
                      opt);
    return (uint64_t)value;
}

static int set_parse_v3(int c, char **argv, int invert, unsigned int *flags,
                        const void *entry, struct xt_entry_match **match) {
...
static void set_printv3_counter(const struct ip_set_counter_match0 *c,
...
static void set_print_v3_matchinfo(const struct xt_set_info_match_v3 *info,
...
/* Prints out the matchinfo. */
static void set_print_v3(const void *ip, const struct xt_entry_match *match,
                         int numeric) {
...
static void set_save_v3(const void *ip, const struct xt_entry_match *match) {
    const struct xt_set_info_match_v3 *info = (const void *)match->data;
...
/* Revision 4 */
static int set_parse_v4(int c, char **argv, int invert, unsigned int *flags,
                        const void *entry, struct xt_entry_match **match) {
...
static void set_printv4_counter(const struct ip_set_counter_match *c,
                                const char *name, const char *sep) {
    switch (c->op) {
    case IPSET_COUNTER_EQ:
        printf(" %s%s-eq %llu", sep, name, c->value);
...
static void set_print_v4_matchinfo(const struct xt_set_info_match_v4 *info,
                                   const char *opt, const char *sep) {
    print_match(opt, &info->match_set);
    if (info->flags & IPSET_FLAG_RETURN_NOMATCH)
        printf(" %sreturn-nomatch", sep);
...
/* Prints out the matchinfo. */
static void set_print_v4(const void *ip, const struct xt_entry_match *match,
                         int numeric) {
...
static void set_save_v4(const void *ip, const struct xt_entry_match *match) {
    const struct xt_set_info_match_v4 *info = (const void *)match->data;
