...
static void connmark_print(const void *ip, const struct xt_entry_match *match,
                           int numeric) {
...
static void connmark_mt_print(const void *ip,
...
static void connmark_mt_save(const void *ip,
...
static void print_mark_xlate(unsigned int mark, unsigned int mask,
    ...
    if (mask != 0xffffffffU)
        ...
        xt_xlate_add(xl, " %s0x%x", op == XT_OP_EQ ? "" : "!= ", mark);
}
...
static int connmark_mt_xlate(struct xt_xlate *xl,
                             const struct xt_xlate_mt_params *params) {
    const struct xt_connmark_mtinfo1 *info = (const void *)params->match->data;
    enum xt_op op = XT_OP_EQ;
