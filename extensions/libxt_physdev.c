...
static void physdev_help(void) {
    printf(
        ...
        " [!] --physdev-in inputname[+]		bridge port name ([+] "
        "for wildcard)\n"
        " [!] --physdev-out outputname[+]	bridge port name ([+] for "
        "wildcard)\n"
...
static const struct xt_option_entry physdev_opts[] = {
    {.name = "physdev-in",
     ...
     XTOPT_POINTER(s, physindev)},
    {.name = "physdev-out",
     .id = O_PHYSDEV_OUT,
     .type = XTTYPE_STRING,
     .flags = XTOPT_INVERT | XTOPT_PUT,
     XTOPT_POINTER(s, physoutdev)},
    {.name = "physdev-is-in",
     .id = O_PHYSDEV_IS_IN,
     .type = XTTYPE_NONE,
     .flags = XTOPT_INVERT},
    {.name = "physdev-is-out",
     ...
     .type = XTTYPE_NONE,
     .flags = XTOPT_INVERT},
    {.name = "physdev-is-bridged",
     .id = O_PHYSDEV_IS_BRIDGED,
     .type = XTTYPE_NONE,
     .flags = XTOPT_INVERT},
    XTOPT_TABLEEND,
};
...
static void physdev_print(const void *ip, const struct xt_entry_match *match,
                          int numeric) {
...
void _init(void) { xtables_register_match(&physdev_match); }
