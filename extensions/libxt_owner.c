...
static void owner_mt_print_item_v0(const struct ipt_owner_info *info,
...
static void owner_mt6_print_item_v0(const struct ip6t_owner_info *info,
                                    const char *label, uint8_t flag,
                                    bool numeric) {
    if (!(info->match & flag))
        return;
...
static void owner_mt_print_item(const struct xt_owner_match_info *info,
                                const char *label, uint8_t flag, bool numeric) {
    if (!(info->match & flag))
        return;
    if (info->invert & flag)
        printf(" !");
...
static void owner_mt_print_v0(const void *ip,
...
static void owner_mt6_print_v0(const void *ip,
...
static void owner_mt_save_v0(const void *ip,
                             const struct xt_entry_match *match) {
    const struct ipt_owner_info *info = (void *)match->data;
...
static void owner_mt6_save_v0(const void *ip,
                              const struct xt_entry_match *match) {
    const struct ip6t_owner_info *info = (void *)match->data;
...
static int owner_mt_print_uid_xlate(const struct xt_owner_match_info *info,
                                    struct xt_xlate *xl) {
...
static int owner_mt_print_gid_xlate(const struct xt_owner_match_info *info,
                                    struct xt_xlate *xl) {
